...
.PHONY: clean mrclean distclean configcheck boa docs apmib utility utils system defconfig html tools cgi
...
ifeq ($(CONFIG_RTL_83XX_QOS_SUPPORT),y)
    COMMON_CFLAGS += -DCONFIG_RTL_83XX_QOS_SUPPORT
endif
...
ifeq ($(CONFIG_APP_RTL_QUAGGA), y)
COMMON_CFLAGS += -DCONFIG_APP_RTL_QUAGGA
endif
ifeq ($(CONFIG_APP_PIMD), y)
COMMON_CFLAGS += -DCONFIG_APP_PIMD
endif
ifeq ($(CONFIG_APP_MROUTED), y)
COMMON_CFLAGS += -DCONFIG_APP_MROUTED
endif
...
COMMON_CFLAGS += -DCONFIG_RTL_11AC_SUPPORT
COMMON_CFLAGS += -DCONFIG_RTL_92D_SUPPORT
COMMON_CFLAGS += -DCONFIG_WLAN_HAL_8812FE
endif
...
ifeq ($(CONFIG_RTK_CROSSBAND_REPEATER),y)
...
ifeq ($(CONFIG_LOGIN_URL),y)
COMMON_CFLAGS += -DLOGIN_URL
COMMON_CFLAGS += -DLOGIN_URL_MAX_USER=$(CONFIG_LOGIN_URL_MAX_USER)
COMMON_CFLAGS += -DLOGIN_URL_ACCESS_TIMEOUT=$(CONFIG_LOGIN_URL_ACCESS_TIMEOUT)
endif
# support WPS works on VAP interface
#COMMON_CFLAGS += -DWSC_SUPPORT_VAP_INFERTACE

ifeq ($(CONFIG_APP_THERMAL_CONTROL), y)
COMMON_CFLAGS += -DCONFIG_APP_THERMAL_CONTROL
endif
...
TARGET = tools defconfig apmib utility utils system boa cgi html
endif
endif
...
ifeq ($(CONFIG_ELINKSDK_SUPPORT),y)
	$(ROMFSINST) $(DIR_ROOT)/users/lib/lib/libelinksdk.so /lib/libelinksdk.so
endif
	$(ROMFSINST) utility/$(UTILITY_LIB) /lib/libutility.so
...
ifeq  ($(CONFIG_IPV6_MANAGE_INET),y)
...
ifneq ($(CONFIG_RTL_DEF_SETTING_IN_FW),y)
ifeq ($(CONFIG_RTL_DOWNLOAD_UPLOAD_DEFAULT_SETTING),y)
	$(ROMFSINST) -s /var/defconfig.dat /web/defconfig.dat
endif
endif
ifeq ($(CONFIG_APP_OPENVPN), y)
...
ifeq ($(CONFIG_APP_TXT_CONFIG_SUPPORT),y)
	$(ROMFSINST) utils/cvcfg-gw /bin/cvcfg-gw
endif
ifneq ($(CONFIG_RTL_NFJROM_MP),y)
...
ifeq ($(CONFIG_USER_CLUSTER_MANAGE), y)
	$(ROMFSINST) src/cluster_server /bin/cluster_server
	$(ROMFSINST) src/cluster_client /bin/cluster_client
endif
