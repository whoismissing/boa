...
void printf_usage(void)
{
    ...
    printf("<option>: root|root-jffs2|linux|boot|all|vmlinux|vmlinuxhdr|signature\n");
    printf("[signature]: user-specified signature (4 characters)\n");
#ifdef CONFIG_FWD_UNIFIED_SUPPORT_PLUGIN
    printf("Usage: cvimg plugin   input-filename output-filename $plugin_name\n");
    printf("$plugin_name: user-specified plugin name (<16 characters)\n");
#endif
}
...
int main(int argc, char** argv)
{
...
#ifdef CONFIG_FWD_UNIFIED_SUPPORT_PLUGIN
	int is_plugin = 0;
	PLUGIN_HEADER_Tp pPluginHdr = NULL;
#endif
	...
	if(argc == 2 && !strcmp(argv[1], "mib_flash_size_chk")){
		...
		if(sizeof(BLUETOOTH_HW_SETTING_T)+sizeof(PARAM_HEADER_T)>=BLUETOOTH_HW_SETTING_SECTOR_LEN){		
			printf("==============================================\n");
			printf("Bluetooth hardware setting over size!!\nFlash left %x but need %x\n",BLUETOOTH_HW_SETTING_SECTOR_LEN,sizeof(BLUETOOTH_HW_SETTING_T)+sizeof(PARAM_HEADER_T));
			exit(1);
		}
#endif
...
//#endif
...
#ifdef CONFIG_FWD_UNIFIED_SUPPORT_PLUGIN
	if(argc == 5 && !strcmp(argv[1], "plugin")) {
		is_plugin = 1;
	}
#endif
	...
	if (is_vmlinuxhdr) {
		...
		size = sizeof(PLUGIN_HEADER_T) + status.st_size;
#ifdef CONFIG_MTD_NAND
		if (size%4)
			size=size+(size%4); // pad
#else
		if (size%2)
			size++; // padding for short-type cksum
#endif
	}
#endif
	else if (!is_vmlinux) {
    ...
    if (is_vmlinuxhdr)
		...
		pPluginHdr = (PLUGIN_HEADER_Tp)buf;
		buf += sizeof(PLUGIN_HEADER_T);
	}
#endif
	else if (!is_vmlinux)
	...
	if (is_vmlinuxhdr) {
		...
		*((unsigned int *)((char *)pHeader+4)) = start_addr;
		...
		//Plugin Header
		memcpy(pPluginHdr->signature, PLUGIN_SIGNATURE, SIGNATURE_LEN);
		strncpy(pPluginHdr->name, argv[4], PLUGIN_HDR_NAME_LEN-1);	//reserve last '\0'
		pPluginHdr->len = DWORD_SWAP(status.st_size); //payload len
		pPluginHdr->padding = WORD_SWAP(size - sizeof(PLUGIN_HEADER_T) - status.st_size);

		//Plugin Checksum
		checksum = pPluginHdr->cksum = WORD_SWAP(calculateChecksum(buf, status.st_size));//add cksum after padding bytes
	}
#endif
	else if (!is_vmlinux) {
		if( !strcmp("root", argv[1]))
			memcpy(pHeader->signature, ROOT_HEADER, SIGNATURE_LEN);
		else if ( !strcmp("root-jffs2", argv[1]))
      memcpy(pHeader->signature, ROOT_HEADER, SIGNATURE_LEN);
	...
	if (is_vmlinuxhdr)
		...
		printf("Generate plugin image successfully, length=0x%x, checksum=0x%x\n", (int)DWORD_SWAP(pPluginHdr->len), checksum);
#endif
	else if (!is_vmlinux)
