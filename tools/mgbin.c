...
/////////////////////////////////////////////////////////////////////////////
/* Input file type */
typedef enum { BOOT_CODE=0, CONFIG, WEB_PAGES, ROOT, SYS, PLUG, ALL, INVALID_FILE=-1 } SECTOR_TYPE_T;
...
static SECTOR_TYPE_T checkInputFile(char *filename, int *pWith_header)
{
	...
	*pWith_header = 1;
	...
	else if ( !memcmp(signature, PLUGIN_SIGNATURE, 4) )
		return PLUG;
#endif
	return INVALID_FILE;
}
...
#ifdef CONFIG_FWD_UNIFIED_SUPPORT_PLUGIN
	for (i=BOOT_CODE; i<=ALL ; i++)
#else
	for (i=BOOT_CODE; i<=SYS ; i++) 
#endif
	{
#ifdef COMPACT_FILENAME_BUFFER
