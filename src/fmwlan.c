...
#ifdef CONFIG_WPS_TRIGGER_SINGLE_INTERFACE
...
static void _Start_Wlan_Applications(void)
{
	...
	{						
		pid=getPid("/var/run/udhcpc-br0.pid");
		if(pid>0)
		{			
			kill(pid, SIGUSR1);
		}
...
//displayPostDate(wp->post_data);	
	...
	for(i=1; i<=MAX_SCHEDULE_NUM ; i++)
	{
		...
		*((char *)&entry) = (char)i;
		...
		//
		strTmp= req_get_cstream_var(wp, ("wlsch_mode"), "");
		if(strTmp[0]&&(atoi(strTmp)==1))
		{
		...
		memset(tmpBuf,0x00, sizeof(tmpBuf));			
				sprintf(tmpBuf,"wlsch_action_%d",index);
				strTmp = req_get_cstream_var(wp, tmpBuf, "");
				if(strTmp[0]&&(atoi(strTmp)==1))
				{
...
int wlanHandler(request *wp, char *tmpBuf, int *mode, int wlan_id)
{
  char *strSSID, *strChan, *strDisabled, *strVal, strtmp[80];
	...
	if(disabled)
	{
		//shutdown root vdev and all vap vdev
		wlan_vdev_action_all(wlan_id, 0, 1);
	}else{	
		//enable root vdev and all vap vdev
		wlan_vdev_action_all(wlan_id, 1, 1);
	}
#endif
...
//printf("\r\n __[%s-%u]\r\n",__FILE__,__LINE__);	
	...
	if (mode != 1) { // AP mode
		...
		if(encrypt == ENCRYPT_WPA){//marked by tesia(bcz wpa encrypt is displayed in webpage)
/*
			intVal =   WPA_CIPHER_AES ;
			if ( apmib_set(MIB_WLAN_WPA2_CIPHER_SUITE, (void *)&intVal) == 0) {
				strcpy(tmpBuf, ("<script>dw(wl_Set_MIB_WLAN_WPA2_UNICIPHER_fai)</script>"));
...
//printf("\r\n auahType=[%d],__[%s-%u]\r\n",auahType,__FILE__,__LINE__);				
	...
	snprintf(varName, sizeof(varName), "wepEnabled%d", wlan_id);
	...
	if ( enabled ) {
		snprintf(varName, sizeof(varName), "length%d", wlan_id);
	...
	snprintf(varName, sizeof(varName), "authType%d", wlan_id);
	...
	snprintf(varName, sizeof(varName), "format%d", wlan_id);
	...
	snprintf(varName, sizeof(varName), "key%d", wlan_id);
...
#endif
	{
		snprintf(varName, sizeof(varName), "wps_clear_configure_by_reg%d", wlan_id);
...
int wpaHandler(request *wp, char *tmpBuf, int wlan_id)
{
	...
	snprintf(varName, sizeof(varName), "method%d", wlan_id);
   	strEncrypt = req_get_cstream_var(wp, varName, "");
	if (!strEncrypt[0]) {
 		strcpy(tmpBuf, ("<script>dw(wl_Err_no_enc)</script>"));
		goto setErr_encrypt;
	...
	if (encrypt == ENCRYPT_DISABLED || encrypt == ENCRYPT_WEP) {
		snprintf(varName, sizeof(varName),"use1x%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if ( !strcmp(strVal, "ON")) {
...
//					printf("%s(%d): WPA-RADIUS can be used when device is set to client mode\n",__FUNCTION__,__LINE__);//Added for test 
	...
	{
		/*WAPI handle*/
		snprintf(varName, sizeof(varName), "wapiAuth%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) 
		{
		...
		if ( getPSK ) {
			...
			snprintf(varName, sizeof(varName), "wapiPskFormat%d", wlan_id);
   			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
	 			strcpy(tmpBuf, ("<script>dw(wl_Err_no_psk)</script>"));
				goto setErr_encrypt;
			...
			snprintf(varName, sizeof(varName), "wapiPskValue%d", wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			len = strlen(strVal);
	...
	wapi_end:
		...
		{ 
			int old_vwlan_idx,i;
			snprintf(varName, sizeof(varName), "wapiASIP%d", wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
			...
			snprintf(varName, sizeof(varName), "wapiCertSel%d", wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
		...
		snprintf(varName, sizeof(varName), "nonWpaSupp%d", wlan_id);
 		strVal = req_get_cstream_var(wp, varName, "");
		...
		if ( intVal ) {
			...
			snprintf(varName, sizeof(varName), "nonWpaWep%d", wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if ( !strcmp(strVal, "ON"))
				suppNonWPA |= SUPP_NONWPA_WEP;

			snprintf(varName, sizeof(varName), "nonWpa1x%d", wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if ( !strcmp(strVal, "ON")) {
		...
		// WPA authentication
		snprintf(varName, sizeof(varName), "wpaAuth%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( !strcmp(strVal, ("eap"))) {
				apmib_get( MIB_WLAN_MODE, (void *)&intVal);
...
// get cipher suite from user setting, for wpa-aes -------------------		
		...
		//if ((encrypt == ENCRYPT_WPA) || (encrypt == ENCRYPT_WPA2_MIXED)) 
		{
			snprintf(varName, sizeof(varName), "ciphersuite%d", wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");	 	
			if (strVal[0]) {
				intVal = 0;				
      ...
      if (encrypt == ENCRYPT_WPA2_MIXED && val == CLIENT_MODE)
		...
		//if ((encrypt == ENCRYPT_WPA2) || (encrypt == ENCRYPT_WPA2_MIXED)) 
		{
			snprintf(varName, sizeof(varName), "wpa2ciphersuite%d", wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");	 	
    ...
    if(encrypt == ENCRYPT_WPA2_MIXED && val == CLIENT_MODE)
    {
      sprintf(varName, "is_wpa3_packet%d", wlan_id);
      strVal = req_get_cstream_var(wp, varName, "");
      if ( !strcmp(strVal, "1"))
        intVal =1;
      else
        intVal = 0;
      apmib_set(MIB_WLAN_WPA3, (void *)&intVal);
    }
...
//-------------------------------------------------- david, 2005-8-03	
		...
		if( ((encrypt ==  ENCRYPT_WPA2) || (encrypt == ENCRYPT_WPA2_MIXED)) &&
		    enableRS == 1){
			snprintf(varName, sizeof(varName), "preAuth%d", wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if ( !strcmp(strVal, "ON"))
				intVal = 1 ;
			else
		...
		if ( getPSK ) {
			...
			snprintf(varName, sizeof(varName), "pskFormat%d", wlan_id);
   			strVal = req_get_cstream_var(wp, varName, "");
			if (!strVal[0]) {
	 			strcpy(tmpBuf, ("<script>dw(wl_Err_no_psk)</script>"));
				goto setErr_encrypt;
			...
			snprintf(varName, sizeof(varName), "pskValue%d", wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
...
rekey_time:
		// group key rekey time
		reKeyTime = 0;
		snprintf(varName, sizeof(varName), "groupKeyTimeDay%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
		...
		snprintf(varName, sizeof(varName), "groupKeyTimeHr%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
		...
		snprintf(varName, sizeof(varName), "groupKeyTimeMin%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
		...
		snprintf(varName, sizeof(varName), "groupKeyTimeSec%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
	...
	if (enableRS == 1) { // if 1x enabled, get RADIUS server info
		...
		if (wlan_mode == CLIENT_MODE) { // wlan client mode
			...
			snprintf(varName, sizeof(varName), "eapType%d", wlan_id);
			strVal = req_get_cstream_var(wp, varName, "");
			if (strVal[0]) {
				if ( !string_to_dec(strVal, &intVal) ) {
			...
			if(intVal == EAP_MD5){
				snprintf(varName, sizeof(varName), "eapUserId%d", wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
				...
				snprintf(varName, sizeof(varName), "radiusUserName%d", wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
				...
				snprintf(varName, sizeof(varName), "radiusUserPass%d", wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
				...
				snprintf(varName, sizeof(varName), "radiusUserCertPass%d", wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
				...
				snprintf(varName, sizeof(varName), "eapInsideType%d", wlan_id);
				strVal = req_get_cstream_var(wp, varName, "");
				if (strVal[0]) {
...
//					}
...
#endif
		{
		snprintf(varName, sizeof(varName), "radiusPort%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (!strVal[0]) {
		...
		snprintf(varName, sizeof(varName), "radiusIP%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (!strVal[0]) {
		...
		snprintf(varName, sizeof(varName), "radiusPass%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		...
		snprintf(varName, sizeof(varName), "radiusRetry%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
		...
		snprintf(varName, sizeof(varName), "radiusTime%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
		...
		snprintf(varName, sizeof(varName), "useAccount%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if ( !strcmp(strVal, "ON"))
			intVal = 1;
		else
		...
		snprintf(varName, sizeof(varName), "accountPort%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (!strVal[0]) {
		...
		snprintf(varName, sizeof(varName), "accountIP%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (!strVal[0]) {
		...
		snprintf(varName, sizeof(varName), "accountPass%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strlen(strVal) > (MAX_RS_PASS_LEN -1) ) {
		...
		snprintf(varName, sizeof(varName), "accountRetry%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
		...
		snprintf(varName, sizeof(varName), "accountTime%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
		...
		snprintf(varName, sizeof(varName), "accountUpdateEnabled%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if ( !strcmp(strVal, "ON"))
			intVal = 1;
		else
		...
		snprintf(varName, sizeof(varName), "accountUpdateTime%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( !string_to_dec(strVal, &intVal) ) {
...
get_wepkey:
		...
		snprintf(varName, sizeof(varName), "wepKeyLen%d", wlan_id);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
	...
	#ifndef WSC_SUPPORT_VAP_INFERTACE
	#ifdef MBSSID
	if (vwlan_idx == 0)
	#endif
	#endif
...
setOK:
	...
	apmib_update_web(CURRENT_SETTING);
#ifdef MODULES_REINIT
	set_reinit_flag(REINIT_FLAG_WLAN|REINIT_FLAG_WLAN_APP);
#endif
...
setac_ret:
	apmib_update_web(CURRENT_SETTING);
#ifdef MODULES_REINIT
	set_reinit_flag(REINIT_FLAG_WLAN);
#endif
...
// for WMM move to basic setting
	...
	if (strPower[0]) {		
		if (!strcmp(strPower, ("0")))
			...
			val = 4;
#ifdef CONFIG_CMCC
		else if (!strcmp(strPower, ("80")))
			val = 80;
		else if (!strcmp(strPower, ("60")))
			val = 60;
		else if (!strcmp(strPower, ("40")))
			val = 40;
		else if (!strcmp(strPower, ("20")))
			val = 20;		
#endif
		else {
...
void formWlanMultipleAP(request *wp, char *path, char *query)
{
	...
	int wanIndex=0, portIndex=0;
	char ifName[16]={0}, wanBrName[8]={0};
	...
	for (idx=1; idx<=4; idx++) {
		...
		snprintf(varName, sizeof(varName), "wl_disable%d", idx);		
		strVal = req_get_cstream_var(wp, varName, "");
		if ( !strcmp(strVal, "ON"))
			disabled = 0;
		else
		...
		portIndex = getLanPortIndex(ifName);
		snprintf(wanBrName, sizeof(wanBrName), "br%d", wanBindingLanPorts[portIndex]);
		
		//printf("[%s:%d] portIndex:%d, wlan_idx:%d, vwlan_idx:%d, ifname:%s, wanbrname:%s\n", __FUNCTION__, __LINE__, 
		//	portIndex, wlan_idx, vwlan_idx, ifName, wanBrName);

		if((wanIndex=wanBindingLanPorts[portIndex])!=0)
		{
			//this vap has been binded to wan bridge
			if(disabled)
			{
				//shutdown vdev
				wlan_vdev_disable(ifName, wanBrName, 1);
			}else{	
				//startup vdev
				wlan_vdev_enable(ifName, wanBrName);
			}
		}
#endif
		...
		snprintf(varName, sizeof(varName), "wl_band%d", idx);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			int wlan_onoff_tkip;
			band_no = strtol( strVal, (char **)NULL, 10);
		...
		snprintf(varName, sizeof(varName), "wl_ssid%d", idx);
		strVal = req_get_cstream_var(wp, varName, "");
		if(strlen(strVal)>=MAX_SSID_LEN)
		{
		...
		snprintf(varName, sizeof(varName), "wl_hide_ssid%d", idx);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( strVal[0] == '0')
				val = 0;
...
#endif
		snprintf(varName, sizeof(varName), "wl_wmm_capable%d", idx);
		strVal= req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( strVal[0] == '0')
				val = 0;
		...
		snprintf(varName, sizeof(varName), "wl_access%d", idx);
		strVal= req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( strVal[0] == '0')
				val = 0;
		...
		snprintf(varName, sizeof(varName), "tx_restrict%d", idx);
		strVal= req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			val = atoi(strVal);
		...
		snprintf(varName, sizeof(varName), "rx_restrict%d", idx);
		strVal= req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			val = atoi(strVal);
	...
	snprintf(redirectUrl,sizeof(redirectUrl), "/boafrm/formWlanRedirect?redirect-url=%s&wlan_id=%d",submitUrl,wlan_idx);
	if(tmpBuf[0]!=0)
		ERR_FOR_VAP(tmpBuf);
	OK_MSG(redirectUrl);
}
...
{
	if(reason_code == 14 || reason_code == 15 || reason_code == 2 || reason_code == 6 || reason_code == 37)
		return 1;
	else
		return 0;
}
...
void formElink(request *wp,char *path, char *query)
{
	char *str_enable_elink,*str_config_yes,*submitUrl;
	int enabled,sync_config;
	int pid=-1;
	char strPID[10];

	str_enable_elink = req_get_cstream_var(wp, ("elink_enable"), "");
	str_config_yes=req_get_cstream_var(wp, ("elink_sync"), "");
	...
	apmib_set(MIB_RTL_LINK_ENABLE,(void *)&enabled);
	apmib_set(MIB_RTL_LINK_SYNC,(void *)&sync_config);
	apmib_update_web(CURRENT_SETTING);
	if(enabled){
...
void formElinkSDKUpload(request *wp, char * path, char * query)
{
	...
	numLeft = wp->upload_len;
	while(numLeft){
		ret = fwrite(wp->upload_data+head_offset+offset, 1, numLeft, fp);
		numLeft -= ret;
		offset += ret;
	}
	fclose(fp);
...
void formWlSiteSurvey(request *wp, char *path, char *query)
{
    ...
    int wait_time, max_wait_time=5, wait_time1=0, max_wait_time1=5;
    char tmpBuf[MAX_MSG_BUFFER_SIZE]={0};
#if defined(WLAN_PROFILE)
	...
	//bss_info bss;
	int vxd_wisp_wan=0;
	char wlanifp_bak[32],wlanifp[32];
...
//displayPostDate(wp->post_data);
	...
	if ( refresh[0] ) {
		...
		while (1) {
			...
			if(strVal[0])
			{
				snprintf(WLAN_IF, WLAN_IFACE_SIZE, "%s",strVal);				
			}
...
*/
    ...
    if ( connect[0] ) 
    {
		...
		wlanifp_tmp = req_get_cstream_var(wp, ("wlanif"), "");
		snprintf(wlanifp, sizeof(wlanifp), "%s", wlanifp_tmp);
...
//printf("\r\n ++++++++++++wlanifp=[%s],__[%s-%u]\r\n",wlanifp,__FILE__,__LINE__);
		...
		if(vxd_wisp_wan== 1)
		{
			snprintf(wlanifp_bak, sizeof(wlanifp_bak), "%s", wlanifp);
			snprintf(wlanifp, sizeof(wlanifp), "%s-vxd", wlanifp_bak);
		}		
#endif
		...
		if (strSel[0]) {
			...
			{ 
                                ...
                                snprintf(varName, sizeof(varName), "method%d", wlan_idx);
...
#if 1//def CONFIG_SMART_REPEATER
						if(vxd_wisp_wan){
							snprintf(command, sizeof(command), "ifconfig %s-vxd down",wlanif);
                        	system(command);
                        }
#endif
                        snprintf(command, sizeof(command), "ifconfig %s down",wlanif);
                        system(command);
                        snprintf(command, sizeof(command), "flash set_mib %s",wlanif);
                        system(command);
                        snprintf(command, sizeof(command), "ifconfig %s up",wlanif);
                        system(command);
#if 1//def CONFIG_SMART_REPEATER
						if(vxd_wisp_wan){
							snprintf(command, sizeof(command), "flash set_mib %s-vxd",wlanif);                      
	                        system(command);
							snprintf(command, sizeof(command), "ifconfig %s-vxd up",wlanif);
	                        system(command);
                        }
#endif                        
...
#else
			{
				...
				while (1) 
				{
						...
						if(vxd_wisp_wan && !strstr(WLAN_IF,"-vxd")){
							strncat(WLAN_IF,"-vxd", WLAN_IFACE_SIZE-strlen(WLAN_IF)-1);
							WLAN_IF[WLAN_IFACE_SIZE-1] = '\0';
						}
					...
					else
					{
						if (encrypt == ENCRYPT_WPA || encrypt == ENCRYPT_WPA2 || encrypt == ENCRYPT_WPA2_MIXED ||encrypt == ENCRYPT_WAPI) {
							...
							{
								status = 1;
								pMsg =	(unsigned char *)"<script>dw(wlsurvey_network_key_mismatch)</script>";
								break;
							}
						}
						...
						while (wait_time++ < max_wait_time) {
							...
							if(wait_time >= max_wait_time)
							{
								...
								if(reason_code == 25)
									...
									pMsg =	(unsigned char *)"<script>dw(wlsurvey_connect_timeout)</script>";
							}
						}
						
					}
					else if(res==STATE_Bss  || res==STATE_Ibss_Idle || res==STATE_Ibss_Active){
						pMsg =  (unsigned char *)"<script>dw(wlsurvey_connect_successfully)</script>";//"Connect failed 5!";//Input negative AES password can trigger here
						status = 0;
					}
					else
						pMsg =  (unsigned char *)"<script>dw(wlsurvey_connect_timeout)</script>";
...
#endif
	getWlAuthResult(WLAN_IF, &status);
	if(status == 14)
		strcpy(tmpBuf, ("<script>dw(wlsurvey_mic_error)</script>"));
	else
		strcpy(tmpBuf, ("<script>dw(wlsurvey_connect_timeout)</script>"));
...
//#ifdef CONFIG_RTK_MESH
	...
	for (i=0; i<pStatus->number && pStatus->number!=0xff; i++) {
        ...
        if(pBss->bdTstamp[1] & 0x00000800)
        {
          idx += sprintf(tmp2Buf+idx, "WPA3");
          wpa3_exist = 1;
        }
        if ((pBss->bdTstamp[0] & 0xf0000000) || (!(pBss->bdTstamp[1] & 0x00000800) && (pBss->bdTstamp[0] & 0xffff0000))) {
...
void sigHandler_autoconf(int signo)
{
	...
	if(NULL != (fp = fopen("/tmp/map_wps_interface", "r"))) {
		...
		if (strstr(map_wps_interface, "vxd")) {
		    skip_init = 1;
			sprintf(cmd, "flash set_mib %s; ifconfig %s up; echo 1 > /tmp/map_soft_reinit; reinitCli -e 3 &", map_wps_interface, map_wps_interface);
			system(cmd);
		}
		fclose(fp);
	}
	...
	snprintf(cmdbuf, 20, "reinitCli -e %d &",REINIT_EVENT_APPLYCAHNGES);
#else
	snprintf(cmdbuf, 20, "reinitCli -e %d",REINIT_EVENT_APPLYCAHNGES);
#endif
...
//#if defined(FOR_DUAL_BAND) //  ; both reset two unterface (wlan0 AND wlan1)
		...
		if(dual_wps_icons)
		{
			if(wlanMode[wlan_idx] != CLIENT_MODE)
			{
				sprintf(tmpbuf,"wlan%d",wlan_idx);
				apmib_reset_wlan_to_default(tmpbuf);
				pMib->wlan[wlan_idx][0].wlanDisabled = wlanDisabled[wlan_idx];
				pMib->wlan[wlan_idx][0].macPhyMode = wlanMacPhy[wlan_idx];
			}
		}
		else
		{
...
//			printf("Reset wlan to default fail!! No wlan name. %s,%d\n",__FUNCTION__ , __LINE__);
		...
		apmib_update_web(CURRENT_SETTING);
#ifdef MODULES_REINIT
		set_reinit_flag(REINIT_FLAG_WLAN_APP|REINIT_FLAG_APP);
#endif
	...
	if(unlockclicked[0])
	{
		snprintf(tmpbuf, sizeof(tmpbuf), "%s -sig_unlock wlan%d", _WSC_DAEMON_PROG,wlan_idx);
		printf("%s\n",tmpbuf);
		system(tmpbuf);	
	...
	if (strVal[0]) {
		...
		set_reinit_flag(REINIT_FLAG_WLAN_APP|REINIT_FLAG_APP);
			...
			run_init_script("bridge");
				...
				if(wlan_idx)
				{
					if(open(WSCD_IND_ONLY_INTERFACE1, 0777, O_RDWR | O_CREAT ) == -1)
						printf("create files for only trriger interface wlan1 (pbc) fail \n");
				}
				else
				{
					if(open(WSCD_IND_ONLY_INTERFACE0, 0777, O_RDWR | O_CREAT ) == -1)
						printf("create files for only trriger interface wlan0 (pbc) fail \n");
				}
#endif
	...
	if (strVal[0]) {
		...
		if (intVal) {
			...
			run_init_script("bridge");
				...
				{
					if(open(WSCD_IND_ONLY_INTERFACE1, 0777, O_RDWR | O_CREAT ) == -1)
						printf("create files for only trriger interface wlan1-vxd (pbc) fail \n");
				}
				else
				{
					if(open(WSCD_IND_ONLY_INTERFACE0, 0777, O_RDWR | O_CREAT ) == -1)
						printf("create files for only trriger interface wlan0-vxd (pbc) fail \n");
				}
#endif
			snprintf(tmpbuf, sizeof(tmpbuf), "%s -sig_pbc wlan%d-vxd", _WSC_DAEMON_PROG,wlan_idx);
			system(tmpbuf);
#endif
		}
		OK_MSG2(START_PBC_MSG, ((mode==AP_MODE) ? "<script>dw(wlwps_client)</script>" : "AP"), submitUrl);
...
#ifdef MODULES_REINIT
			set_reinit_flag(REINIT_FLAG_WLAN|REINIT_FLAG_WLAN_APP|REINIT_FLAG_APP);
#endif
			...
			if (local_pin_changed) {
				...
				if(targetAPSsid[0]){					
					if(strlen(targetAPSsid)<= 32){
						...
						snprintf(tmpbuf, sizeof(tmpbuf), "iwpriv wlan%d set_mib wsc_specssid=\"%s\" ",wlan_idx, targetAPSsid);
						system(tmpbuf);						
					}else{					
				...
				if( (wlan0_mode == 0) && (wlan1_mode == 0) && (wlan0_disable == 0) && (wlan1_disable == 0))
						snprintf(tmpbuf, sizeof(tmpbuf), "%s -sig_start %s", _WSC_DAEMON_PROG, "wlan0-wlan1");
				else 
						snprintf(tmpbuf, sizeof(tmpbuf), "%s -sig_start wlan%d", _WSC_DAEMON_PROG,wlan_idx);
				system(tmpbuf);
#else
				snprintf(tmpbuf, sizeof(tmpbuf), "%s -sig_start wlan%d", _WSC_DAEMON_PROG,wlan_idx);
				system(tmpbuf);
	...
	if (strVal[0]) {
		...
		if (intVal) {
			...
			if (local_pin_changed) {
				...
				snprintf(tmpbuf, sizeof(tmpbuf), "%s -sig_start wlan%d-vxd", _WSC_DAEMON_PROG,wlan_idx);
				system(tmpbuf);
			}			
#endif
		}
	...
	if (strVal[0]) {		
		...
		if (strVal[0]) {
			...
			if (intVal) {
				...
				{
					if(open(WSCD_IND_ONLY_INTERFACE1, 0777, O_RDWR | O_CREAT ) == -1)
						printf("create files for only trriger interface wlan1 fail \n");
				}
				else
				{
					if(open(WSCD_IND_ONLY_INTERFACE0, 0777, O_RDWR | O_CREAT ) == -1)
						printf("create files for only trriger interface wlan0 fail \n");
				}
#endif
				//printf("after filter pin code =%s , len =%d\n", pincodestr_b ,strlen(pincodestr_b));				
				snprintf(tmpbuf, sizeof(tmpbuf), "iwpriv %s set_mib pin=%s", WLAN_IF, pincodestr_b);				
				//printf("tmpbuf=%s\n",tmpbuf);
				system(tmpbuf);
				// WPS2DOTX ; 2011-0428 ; support the format pin code 1234-5670 (include "-")				
#endif
	...
	if (strVal[0]) {		
		...
		if (strVal[0]) {
			...
			if (intVal) {
				...
				{
					if(open(WSCD_IND_ONLY_INTERFACE1, 0777, O_RDWR | O_CREAT ) == -1)
						printf("create files for only trriger interface wlan1-vxd fail \n");
				}
				else
				{
					if(open(WSCD_IND_ONLY_INTERFACE0, 0777, O_RDWR | O_CREAT ) == -1)
						printf("create files for only trriger interface wlan0-vxd fail \n");
				}
#endif
				snprintf(tmpbuf, sizeof(tmpbuf), "iwpriv wlan%d-vxd set_mib pin=%s", wlan_idx, strVal);
				system(tmpbuf);
#endif
			}
			OK_MSG1(SET_PIN_MSG, submitUrl);			
