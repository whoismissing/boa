...
#ifdef CONFIG_IPV6
#define _IPV6_FIREWALL_SCRIPT_PROG "ipv6firewall.sh"
#endif
...
#define FORM_ELINKSDK_UPLOAD "formElinkSDKUpload"
#endif
...
#define REBOOT_WAIT(url) { \
	snprintf(lastUrl, URL_MAX_LEN,"%s",url); \
	snprintf(okMsg, MSG_MAX_LEN,"%s",APPLY_OK_MSG); \
...
#define OK_MSG(url) { \
	...
	if(req_get_cstream_var(wp, "save_apply", "")[0]==0){ \
       req_format_write(wp, "<meta http-equiv=\"refresh\" content=\"0;url=%s\"></head></html>", (strlen(url)>0)?url:"/wizard.htm"); \
    }else{\
     ...
     req_format_write(wp, "<input type='hidden' value='%s' name='submit-url'>",(strlen(url)>0)?url:"/wizard.htm"); \
...
#define OK_MSG(url) { \
	...
	req_format_write(wp, "<input id='restartLater' type='button' value='Reboot Later' OnClick=window.location.replace(\"%s\")>", (strlen(url)>0)?url:"/wizard.htm"); \
...
#define RET_SURVEY_PAGE(pMsg, url, connectOK, wlan_id, isWizard) { \
	...
	if(connectOK) req_format_write(wp, "<td><font size=2><b><input type=\"checkbox\" name=wizardAddProfile%d value=\"ON\">&nbsp;&nbsp;<script>dw(wlsurvey_connect_addto_profile)</script></b><br><br></td>", wlan_id); \
	...
	if(connectOK) req_format_write(wp, "<input id='restartLater' name='restartLater' type='submit' value='Save' OnClick=window.location.replace(\"%s\");return true>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;", (strlen(url)>0)?url:"/wizard.htm"); \
...
#define RET_SURVEY_PAGE(pMsg, url, connectOK, wlan_id, isWizard) { \
	...
	if(connectOK) req_format_write(wp, "<input id='restartLater' name='restartLater' type='submit' value='Reboot Later' OnClick=window.location.replace(\"%s\");return true>", (strlen(url)>0)?url:"/wizard.htm"); \
	req_format_write(wp, "</form></blockquote></body></html>");\
}
#define SUCCESS_INFO(info,url){\
	req_format_write(wp, "<html><head>"); \
...
#define OK_MSG1(msg, url) { \
	...
	if (url) req_format_write(wp, "<form>\n<input type=button value=\"  OK  \" OnClick=window.location.replace(\"%s\")>\n</form>\n</blockquote></body>\n</html>", url);\
	else req_format_write(wp, "<form><input type=button value=\"  OK  \" OnClick=window.close()></form></blockquote></body></html>");\
}
#ifndef CONFIG_CMCC
...
#define OK_MSG_FW(msg, url, c, ip) { \
	...
	req_format_write(wp, "<P align=left><h4><script>dw(saveconf_wait)</script><B><SPAN id=show_sec></SPAN></B>&nbsp;<script>dw(saveconf_seconds)</script></h4></P>");\
...
extern int FilteredSysLogList(request *wp, int argc, char **argv);
extern void formPasswordSetup(request *wp, char *path, char *query);

#ifdef SUPER_NAME_SUPPORT
extern void formSuperPasswordSetup(request *wp, char *path, char *query);
#endif
...
extern void formSetSyslogDisplayLevel(request *wp, char *path, char *query);
extern void formSaveClrSysLog(request *wp, char *path, char *query);
...
extern void formAWiFiMacList(request *wp, char *path, char *query);
extern void formAWiFiUrlList(request *wp, char *path, char *query);
...
extern int rtl_upgrade_proceed(char *upload_data, int upload_len, int flag, char *buffer);
extern int rtl_watchdog_reboot(unsigned char *msg);
#else
...
extern int getQosWanPort(request *wp, int argc, char **argv);
extern int getWanPortEnable(request *wp, int argc, char **argv);
extern int getQoSWan(request *wp, int argc, char **argv);
#endif
extern int getQosQueue(request *wp, int argc, char **argv);
extern int getQosClass(request *wp, int argc, char **argv);
extern int getQosType(request *wp, int argc, char **argv);
extern int getQosUpCar(request *wp, int argc, char **argv);

#elif defined(MULTI_WAN_QOS)
extern int getQoSWanTraffic(request *wp, int argc, char **argv);
extern int getQoSWanPolicy(request *wp, int argc, char **argv);
#endif
...
#ifdef CONFIG_APP_AWIFI_SUPPORT
extern void wanListForAWiFi(request *wp, int argc, char **argv);
extern int aWiFiTrustedMacList(request *wp, int argc, char **argv);
extern int aWiFiTrustedUrlList(request *wp, int argc, char **argv);
#endif
...
extern void formElink(request *wp,char *path, char *query);
...
extern void formIgmpMldSnoopingProxy(request *wp, char *path, char *query);
extern void formL2tpCfg(request *wp, char *path, char *query);
...
extern void formpimdroute(request * wp, char *path, char *query);
extern int PimdRpList(request *wp, int argc, char **argv);
extern int PimdInterfaceList(request *wp, int argc, char **argv);
#endif 
#ifdef CONFIG_APP_MROUTED
extern void formmroutedroute(request * wp, char *path, char *query);
extern int mroutedIfList(request *wp, int argc, char **argv);
extern int mroutedTunnelList(request *wp, int argc, char **argv);
extern int mroutedAltnetList(request *wp, int argc, char **argv);
#endif
...
extern int vlanBindingList(request *wp, int argc, char **argv);
extern int showLanDeviceList(request *wp, int argc, char **argv);
extern void formVlanBinding(request *wp, char *path, char *query);
#endif

#if defined(CONFIG_RTL_ACCESS_INTERNET_USER_NUM_LIMIT)
extern void formUserLimit(request *wp, char *path, char *query);
#endif
...
extern int interface_list(request *wp, int argc, char **argv);
...
#if defined(CONFIG_CMCC_QOS)
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
	extern void formMultiPhyWanUplinkQoS(request *wp, char *path, char *query);
	extern void formMultiPhyWan(request *wp, char *path, char *query);
	extern void formUplinkMultiPhyWanQosRule(request *wp, char *path, char *query);
#else
	extern void formUplinkQoS(request *wp, char *path, char *query);
	extern void formUplinkQosRule(request *wp, char *path, char *query);
#endif
	extern void formShapingQoS(request *wp, char *path, char *query);
#elif defined(MULTI_WAN_QOS)
	extern void formQosTraffic(request *wp, char *path, char *query);
	extern void formQosPolicy(request *wp, char *path, char *query);
	extern void formQosRule(request *wp, char *path, char *query); 
	
	extern int qosTrafficList(request *wp, int argc, char **argv);
	extern int qosPolicyList(request *wp, int argc, char **argv);
	extern int qosRuleList(request *wp, int argc, char **argv);  
...
#ifdef MULTI_WAN_SUPPORT
extern void wanListForPortMapping(request *wp, int argc, char **argv);
#endif
#endif
...
extern void formCluMgmt(request *wp, char *path, char *query);
...
#ifndef WLAN_IFACE_SIZE
#define WLAN_IFACE_SIZE 20
#endif
