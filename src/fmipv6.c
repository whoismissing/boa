...
void get_multiIpv6Wan_info(char *argv,char *phy_interface,char *real_interface,int *enabled, int *wan_index, int *link_type, int *wan_type)
{
	WANIFACE_T WanIfaceEntry;
	if(NULL == argv) 	return;
	if(!strcmp(argv,"first"))
		*wan_index = 1;
	else if(!strcmp(argv,"second"))
		*wan_index = 2;
	else if(!strcmp(argv,"third"))
		*wan_index = 3;
	else if(!strcmp(argv,"forth"))
		*wan_index = 4;
	else
		return;
	...
	*enabled = WanIfaceEntry.ipv6Enable;
	*link_type = WanIfaceEntry.ipv6LinkType;
	*wan_type = WanIfaceEntry.ipv6Origin;
	getWanIfName(*wan_index, phy_interface,IFNAMESIZE);	
	if(*link_type ==IPV6_LINKTYPE_PPP)
		getPPPIfName (*wan_index, real_interface,IFNAMESIZE);
	else
...
int getIPv6Status(request *wp, int argc, char **argv)
{
	...
	int i, tmpval;
	#if defined(MULTI_WAN_SUPPORT)
	int enabled=0,wan_index=0,link_type=0,wan_type=0;
	char phy_interface[IFNAMESIZE],real_interface[IFNAMESIZE];
	#endif
	name = argv[0];
	if (name == NULL) {
   		fprintf(stderr, "Insufficient args\n");
	...
	#if defined(MULTI_WAN_SUPPORT)
	if(!strcmp(name,"ipv6WanEnabled")){
		get_multiIpv6Wan_info(argv[1],phy_interface,real_interface,&enabled,&wan_index,&link_type,&wan_type);
		val=wan_type;
		if(val != IPV6_ORIGIN_BRIDGE)
			val = enabled;
		else
			val = 0;
		req_format_write(wp,"%d",val);
	...
	if(!strcmp(name,"ipv6Origin")){
		...
		if(val==IPV6_ORIGIN_AUTO)
			req_format_write(wp,"%s","AUTO");
		else if(val==IPV6_ORIGIN_STATIC)
			req_format_write(wp,"%s","STATIC");
		else if(val==IPV6_ORIGIN_SLAAC)
			req_format_write(wp,"%s","SLAAC");
		else if(val==IPV6_ORIGIN_DHCP6C)
			req_format_write(wp,"%s","DHCP6C");
		else if(val==IPV6_ORIGIN_DSLITE)
			req_format_write(wp,"%s","DSLITE");
		#else
		if(!apmib_get(MIB_IPV6_ORIGIN_TYPE,&val)){
			fprintf(stderr, "Read MIB_IPV6_ORIGIN_TYPE Error\n");
			return -1;			
	...
	#if defined(MULTI_WAN_SUPPORT)
	get_multiIpv6Wan_info(argv[1],phy_interface,real_interface,&enabled,&wan_index,&link_type,&wan_type);
	memcpy(ifname,real_interface,sizeof(real_interface));
	#else
	if(val==IPV6_LINKTYPE_IP)
		sprintf(ifname,"eth1");
	else
