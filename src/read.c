...
int write_body(request * req)
{
    ...
    if(strstr(req->logline, FORM_ELINKSDK_UPLOAD))  //add david_zhang -- upload elinksdk, don't kill daemon
        req->daemon_killed=1;
#endif
#endif
	...
	if (req->upload_data) {
		...
		if(get_shm_id()==0)
		{
			clear_fwupload_shm(get_shm_id());
			req->upload_data = (unsigned char *)setup_fwupload_shm("/bin/boa", content_length);
			if (req->upload_data==NULL) {
				printf("upload image too big!\n");
				boa_perror(req, "upload image too big!");
				return -2;
			}else{
				//printf("upload_data:%p, shmid:%d\n", req->upload_data, get_shm_id());
			}
		}
#else
		if(shm_id == 0)
