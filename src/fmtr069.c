...
///////////////////////////////////////////////////////////////////
static void get_wan_interface_name(int index, char *name, int name_size)
{
	FILE *fp = NULL;
	char *ptr=NULL, filename[32]={0};

	if(name==NULL || name_size==0 || index<1 || index>WANIFACE_NUM)
		return ;
	
	snprintf(filename, sizeof(filename), "/var/wan_interface_%d", index);
	fp = fopen(filename, "r");
	if(fp)
	{
		fread(name, name_size, sizeof(char), fp);
		//format ifname
...
//displayPostDate(wp->post_data);
...
#if defined(MULTI_WAN_SUPPORT)
			sprintf((char *)tmpStr,"echo \"tr069_port %s\" > /proc/wan_vlan", strData);
			system((char *)tmpStr);
#endif
		}
	}
...
#if defined(MULTI_WAN_SUPPORT)
	/* File Transfer */
	strData = req_get_cstream_var(wp, "intf_enable", "");
	if( strData[0] )
	{
		unsigned char vChar=0;

		if( apmib_get( MIB_CWMP_DLUL_INTF_SPECIFY_ENABLE, (void *)&vChar ) )
		{
			if(strData[0]=='0')
			{
				if( vChar==1 )
				{
					...
					changeflag = 1;
					if ( !apmib_set( MIB_CWMP_DLUL_INTF_SPECIFY_ENABLE, (void *)&newVal)) {
						strcpy(tmpBuf, ("Set specify filetrans interface to disable error!"));
						goto setErr_tr069;
					}
				}
			}else{
				if( vChar==0 )
				{
					...
					changeflag = 1;
					if ( !apmib_set( MIB_CWMP_DLUL_INTF_SPECIFY_ENABLE, (void *)&newVal)) {
						strcpy(tmpBuf, ("Set specify filetrans interface to enable error!"));
						goto setErr_tr069;
					}
				}
			}
		}else{
			strcpy(tmpBuf, ("get MIB_CWMP_DLUL_INTF_SPECIFY_ENABLE error!"));
			goto setErr_tr069;
		}
	}
	
	strData = req_get_cstream_var(wp, "dns_intf", "");
	if ( strData[0] )
	{
		char intf_name[MAX_NAME_LEN]={0}, orig_ifname[MAX_NAME_LEN]={0};
		int wan_idx = atoi(strData);

		apmib_get(MIB_CWMP_DLUL_DNS_INTF, (void *)orig_ifname);
		get_wan_interface_name(wan_idx, intf_name, sizeof(intf_name));

		if( strcmp(orig_ifname, intf_name)!=0 )
		{
			if(!apmib_set(MIB_CWMP_DLUL_DNS_INTF, (void *)intf_name)){
				strcpy(tmpBuf, ("Set DNS interface error!"));
				goto setErr_tr069;
			}
			changeflag = 1;
		}
	...
	strData = req_get_cstream_var(wp, "data_intf", "");
	if ( strData[0] )
	{
		char intf_name[MAX_NAME_LEN]={0}, orig_ifname[MAX_NAME_LEN]={0};

		apmib_get(MIB_CWMP_DLUL_INTF, (void *)orig_ifname);
		get_wan_interface_name(atoi(strData), intf_name, sizeof(intf_name));

		if( strcmp(orig_ifname, intf_name)!=0 )
		{
			if(!apmib_set(MIB_CWMP_DLUL_INTF, (void *)intf_name)){
				strcpy(tmpBuf, ("Set Transfer Data interface error!"));
				goto setErr_tr069;
			}
			changeflag = 1;
		}
