...
#ifndef CONFIG_FWD_UNIFIED_FRAMEWORK
...
#endif //ifndef CONFIG_FWD_UNIFIED_FRAMEWORK
...
void formSaveConfig(request *wp, char *path, char *query)
{
	...
	buf = malloc(len);
	if ( buf == NULL ) {
		strcpy(tmpBuf, "<script>dw(saveconf_buffer_failed)</script>");		
		goto back;
	}
	...
	strRequest = req_get_cstream_var(wp, ("save-default-setting"), "");
	if(strRequest[0])
	{
		copy_ds_from_flash_to_file();
		send_redirect_perm(wp, "/defconfig.dat");
		return;
	}
#endif
	...
	if ((strRequest[0] && strcmp(strRequest,"Reset") == 0) || (strRequest[0] && strcmp(strRequest,"重置") == 0)) {
		...
		if ( !apmib_updateDef() ) {
			free(ptr);
			strcpy(tmpBuf, "<script>dw(saveconf_current_setting_failed)</script>");
			free(buf);
			goto back;
		}
#endif
...
#define TMP_TXT_CONFIG_FILE "/tmp/config.txt"
#define TMP_DAT_CONFIG_FILE "/tmp/config.dat"
static int	upload_txtConfig_into_flash(unsigned char *txtBuf, unsigned int txtBufLen, int *pType, int *pStatus)
{
	unsigned char *binBuf=NULL;
	unsigned int binBufLen=0;
	unsigned char cmdBuf[256]={0};
	unsigned char tmpBuf[512]={0};
	struct stat fileStat={0};

	FILE * fp=fopen(TMP_TXT_CONFIG_FILE,"w");
	if(!fp) { 
		fprintf(stderr,"open %s fail!\n",TMP_TXT_CONFIG_FILE);
		return -1;
	}
	if(fwrite(txtBuf,txtBufLen,1,fp)!=1){
		fprintf(stderr,"write %d to %s fail!\n",txtBufLen,TMP_TXT_CONFIG_FILE);
		fclose(fp);
		return -1;
	}
	fclose(fp);
	sprintf(cmdBuf,"cvcfg-gw -no_hw %s %s",TMP_TXT_CONFIG_FILE,TMP_DAT_CONFIG_FILE);
	system(cmdBuf);
	fprintf(stderr,"%s\n",cmdBuf);
	sleep(5);
	if(stat(TMP_DAT_CONFIG_FILE,&fileStat)!=0){
		fprintf(stderr,"stat %s fail!\n",TMP_DAT_CONFIG_FILE);
		return -1;
	}
	binBufLen=fileStat.st_size;
	//fprintf(stderr,"binBufLen=%d\n",binBufLen);
	binBuf=(unsigned char*)malloc(binBufLen);
	if(!binBuf){
		fprintf(stderr,"malloc %d fail!\n",binBufLen);
		return -1;
	}
	bzero(binBuf,binBufLen);
	fp=fopen(TMP_DAT_CONFIG_FILE,"r");
	if(!fp) { 
		fprintf(stderr,"open %s fail!\n",TMP_DAT_CONFIG_FILE);
		return -1;
	}
	if(fread(binBuf,binBufLen,1,fp)!=1){
		fprintf(stderr,"read %d from %s fail!\n",binBufLen,TMP_DAT_CONFIG_FILE);
		fclose(fp);
		return -1;
	}
//	fprintf(stderr,"%s:%d binBufLen=%d\n",__FUNCTION__,__LINE__,binBufLen);

	updateConfigIntoFlash(binBuf,binBufLen,pType,pStatus);
	return 0;
}
#endif
...
#define FWD_CONF "/var/fwd.conf"
	...
	if (head_offset == -1) {
		strcpy(tmpBuf, "<script>dw(saveconf_invaild_format)</script>");
		goto back;
	}
#ifdef RTK_MIB_TAG_CHECK
	...
	if(upload_txtConfig_into_flash(&wp->upload_data[head_offset],(wp->upload_len-head_offset), (int *)&type, &status)<0)
		return;
	...
	if (status == 0 || type == 0) { // checksum error
		strcpy(tmpBuf, "<script>dw(saveconf_invaild_config)</script>");
		goto back;
	}
	else {
...
#ifdef HOME_GATEWAY
		sprintf(tmpBuf, ("%s"), "<script>dw(saveconf_update_sucessful)</script><br><br><script>dw(saveconf_updating)</script><br><script>dw(saveconf_not_turnoff_reboot)</script><br>");
#else
		sprintf(tmpBuf, ("%s"), "<script>dw(saveconf_update_sucessful)</script><br><br><script>dw(saveconf_updating)</script><br><script>dw(saveconf_not_turnoff_ap)</script>");
#endif			
		Reboot_Wait = 45;
		submitUrl = req_get_cstream_var(wp, "submit-url", "");   // hidden page
		...
		sprintf(lan_ip,"%s",inet_ntoa(*((struct in_addr *)lan_ip_buf)) );
#ifdef REBOOT_CHECK
		snprintf(lastUrl,URL_MAX_LEN,"%s",submitUrl);
		snprintf(okMsg,MSG_MAX_LEN,"%s",tmpBuf);
...
#include <sys/shm.h>
	...
	{
#ifdef CONFIG_FWD_UNIFIED_FRAMEWORK
		if(NULL==(char *)setup_fwupload_shm("/bin/boa", wp->upload_len))
		{
			printf("upload too big!(%d bytes)", wp->upload_len);
			return ;
		}
#else
...
#ifdef REBOOT_CHECK
	snprintf(lastUrl,URL_MAX_LEN,"%s","/status.htm");
	snprintf(okMsg,MSG_MAX_LEN,"%s",tmpBuf);
	countDownTime = Reboot_Wait;
	send_redirect_perm(wp, COUNTDOWN_PAGE);
#else
...
void formPasswordSetup(request *wp, char *path, char *query)
{
	...
	if ( !strUser[0]  )
	{
		strcpy(tmpBuf, ("<script>dw(password_err_username)</script>"));
		goto setErr_pass;
	}
	if ( !strPassword[0] ) {
		strcpy(tmpBuf, ("<script>dw(password_err_password)</script>"));
		goto setErr_pass;
	}

	if (!apmib_set(MIB_USER_NAME, (void *)strUser) ||
	 	!apmib_set(MIB_USER_PASSWORD, (void *)strPassword) ) {
		strcpy(tmpBuf, ("<script>dw(super_password_set_mib_fail)</script>"));
		goto setErr_pass;
	}
...
#ifdef REBOOT_CHECK
	{
		...
		char cmdBuf[64]={0};
#endif		
		sprintf(tmpMsg, "%s","<script>dw(password_change_setting)</script><br><br><script>dw(password_not_turnoff_router)</script>");
		apmib_get( MIB_IP_ADDR,  (void *)lan_ip_buf) ;
		sprintf(lan_ip,"%s",inet_ntoa(*((struct in_addr *)lan_ip_buf)) );
#ifdef RTK_REINIT_SUPPORT
		sprintf(cmdBuf,"reinitCli -e %d",REINIT_EVENT_APPLYCAHNGES);
...
#ifdef SUPER_NAME_SUPPORT
void formSuperPasswordSetup(request *wp, char *path, char *query)
{
	char *submitUrl, *strUser, *strPassword;
	char tmpBuf[100];
	...
	if (!superName[0])
	{
		strcpy(tmpBuf, ("<script>dw(super_password_err_super_username)</script>"));
		goto setErr_pass;
	}
	if (!superPassword[0]) {
		strcpy(tmpBuf, ("<script>dw(super_password_err_super_password)</script>"));
		goto setErr_pass;
	}
	...
	if (!strUser[0])
	{
		strcpy(tmpBuf, ("<script>dw(super_password_err_username)</script>"));
		goto setErr_pass;
	}
	if (!strPassword[0]) {
		strcpy(tmpBuf, ("<script>dw(super_password_err_password)</script>"));
		goto setErr_pass;
	}


	if(!strcmp(strUser, tmpBuf)){
		strcpy(tmpBuf, ("<script>dw(super_password_users_invalid)</script>"));
		goto setErr_pass;
	}
	
	if ( !apmib_set(MIB_SUPER_NAME, (void *)superName) ||
		 !apmib_set(MIB_SUPER_PASSWORD, (void *)superPassword) ||
		 !apmib_set(MIB_USER_NAME, (void *)strUser) ||
		 !apmib_set(MIB_USER_PASSWORD, (void *)strPassword) ) {
		strcpy(tmpBuf, ("<script>dw(super_password_set_mib_fail)</script>"));
		goto setErr_pass;
	}
...
//Brad add end		
	...
	if(enabled){
		...
		if(tmpStr[0]){
			if ( apmib_set(MIB_NTP_SERVER_IP3,(void *)tmpStr ) == 0) {
				strcpy(tmpBuf, ("Set NTP server IP error!"));
				goto setErr_ntp;
			}
		}
...
//Brad modify for system re-init method
	...
	set_reinit_flag(REINIT_FLAG_SYSTEM|REINIT_FLAG_APP);
#endif
...
void formOpMode(request *wp, char *path, char *query)
{
	...
	apmib_update_web(CURRENT_SETTING);
		...
		rtk_multicast_set_proxy_enable(0);
#ifdef CONFIG_IPV6
		rtk_multicast_set_mld_proxy_enable(0);
...
#endif
#ifdef HOME_GATEWAY
		snprintf(tmpBuf,sizeof(tmpBuf), "%s","<script>dw(saveconf_router_booting)</script><br><script>dw(saveconf_not_turnoff_reboot)</script><br>");
#else
		snprintf(tmpBuf,sizeof(tmpBuf), "%s", "<script>dw(saveconf_aprouter_booting)</script><br>");
#endif
		apmib_update_web(CURRENT_SETTING);	// update configuration to flash
		apmib_get( MIB_IP_ADDR,  (void *)lan_ip_buf);
...
void formRebootCheck(request *wp, char *path, char *query)
{
...
#ifdef RTK_REINIT_SUPPORT
	{
		...
		/*
		printf("========%s:%s:%d lan_ip_orig = %s, lan_mask_orig = %s , remote_ip_addr = %s ========\n",__FILE__,__FUNCTION__,__LINE__,
			inet_ntoa(lan_ip_orig),inet_ntoa(lan_mask_orig),
			wp->remote_ip_addr);
		*/
...
//displayPostDate(wp->post_data);	
	...
	printf("\r\n Add wireless profile fail__[%s-%u]\r\n",__FILE__,__LINE__);
		...
		for(profileIdx=1; profileIdx<=entryNum;profileIdx++)
		{
			...
			*((char *)&entry) = (char)profileIdx;
			...
			translate_control_code_sprintf(entry.ssid);
			translate_control_code_sprintf(entry.wpaPSK);
	...
	if(strTmp[0])
	{
		...
		run_init_script("all");
#endif
	//	printf("%s:%d submitUrl=%s APPLY_COUNTDOWN_TIME=%d\n",__FUNCTION__,__LINE__,submitUrl,APPLY_COUNTDOWN_TIME);
		OK_MSG(submitUrl);
		needReboot = 0;
	}
	else
...
void formTr069Diagnose(request *wp, char *path, char *query)
{
	...
	if(fp)
	{
		...
		snprintf(errMsg, sizeof(errMsg), "cwmpClient process does not exist!");
		ERR_MSG(errMsg);
		return ;
	}
...
void formSetSyslogDisplayLevel(request *wp, char *path, char *query)
{
	char *submitUrl, *tmpStr;
	char tmpBuf[64];
	char level=0;
	...
	tmpStr = req_get_cstream_var(wp, ("dispLevel"), "");  
	if(tmpStr[0]) {
		level = (char)atoi(tmpStr);
		snprintf(tmpBuf, sizeof(tmpBuf), "echo %d > %s", level, SCRLOG_DISPLAY_CONFFILE);
		system(tmpBuf);
	}
...
void formSaveClrSysLog(request *wp, char *path, char *query)
{
	char *submitUrl, *tmpStr;
	char tmpBuf[64], cmd[128]={0};
	char level=0;
	
	submitUrl = req_get_cstream_var(wp, "submit-url", "");   // hidden page
	
	tmpStr = req_get_cstream_var(wp, ("saverec"), "");  
	if(tmpStr[0]) {
		//save log
		int lognum = 0;
		FILE *fp = NULL;
		...
		snprintf(cmd, sizeof(cmd), "rm -f %s", SCRLOG_SAVED_FILENAME);//clear old saved logfile
		system(cmd);
#ifdef RINGLOG
		fp = fopen("/var/log/log_split", "r");
		if (fp == NULL)
		{
			snprintf(tmpBuf, sizeof(tmpBuf), "Open log_split file failed!\n");
			goto setErr;
		}
		...
		while (lognum >= 0)
		{
			...
			if (fp != NULL)
			{
				snprintf(cmd, sizeof(cmd), "cat %s >> %s", logname, SCRLOG_SAVED_FILENAME);
				system(cmd);
				fclose(fp);
				fp = NULL;
			}
...
#else
		snprintf(cmd, sizeof(cmd), "cp /var/log/messages %s", SCRLOG_SAVED_FILENAME);//clear old saved logfile
		system(cmd);
#endif
	}

	tmpStr = req_get_cstream_var(wp, ("clrrec"), "");
	if(tmpStr[0]) {
		//clear log
		int lognum = 0;
		FILE *fp = NULL;
		...
		while (lognum >= 0)
		{
			snprintf(cmd, sizeof(cmd), "rm -f /var/log/messages.%d", lognum);
			system(cmd);
			lognum--;
		}
#endif
		snprintf(cmd, sizeof(cmd), "echo \" \" > %s", "/var/log/messages");
		system(cmd);
	}
...
int FilteredSysLogList(request *wp, int argc, char **argv)
{
	FILE *fp;
	char  buf[200];
	int level=0;
	int nBytesSent=0;
	int enabled;
	char *LEVEL_PRIO[] = {"emerg", "alert", "crit", "err", "warn", "notice", "info", "debug"};
...
//### end
	...
	while(fgets(buf,200,fp)){
		int ret=0;
		if (strstr(buf, LEVEL_PRIO[level]))
			nBytesSent += req_format_write(wp, ("%s"), buf);
	}
	fclose(fp);
...
void formDosCfg(request *wp, char *path, char *query)
{
...
#ifdef CONFIG_IPV6
	tmpStr = req_get_cstream_var(wp, ("ipv6DosEnabled"), "");
	if(!strcmp(tmpStr, "ON")) {
		ipv6Enabled |= 1;

		tmpStr = req_get_cstream_var(wp, ("Ipv6SmurfFloodEnable"), "");
		if(!strcmp(tmpStr, "ON")) {
			ipv6Enabled |=2;
			tmpStr = req_get_cstream_var(wp, ("Ipv6SmurfFloodcount"), "");
			string_to_dec(tmpStr,&floodCount);
			if ( apmib_set(MIB_IPV6_DOS_SMURF_FLOOD, (void *)&floodCount) == 0) {
				strcpy(tmpBuf, ("Set Ipv6 Smurf flood error!"));
				goto setErr;
			}
		}
		else{
		...
		tmpStr = req_get_cstream_var(wp, ("Ipv6IcmpTooBigFloodEnable"), "");
		if(!strcmp(tmpStr, "ON")) {
			ipv6Enabled |=4;
			tmpStr = req_get_cstream_var(wp, ("Ipv6IcmpTooBigFloodcount"), "");
			string_to_dec(tmpStr,&floodCount);
			if ( apmib_set(MIB_IPV6_DOS_ICMP_TOO_BIG_FLOOD, (void *)&floodCount) == 0) {
				strcpy(tmpBuf, ("Set Ipv6 icmp too big flood error!"));
				goto setErr;
			}
		}
		else{
		...
		tmpStr = req_get_cstream_var(wp, ("Ipv6NSFloodEnable"), "");
		if(!strcmp(tmpStr, "ON")) {
			ipv6Enabled |=8;
			tmpStr = req_get_cstream_var(wp, ("Ipv6NSFloodcount"), "");
			string_to_dec(tmpStr,&floodCount);
			if ( apmib_set(MIB_IPV6_DOS_NS_FLOOD, (void *)&floodCount) == 0) {
				strcpy(tmpBuf, ("Set Ipv6 Neighbor Solicitation flood error!"));
				goto setErr;
			}
		}
		else{
	...
	if ( apmib_set(MIB_IPV6_DOS_ENABLE, (void *)&ipv6Enabled) == 0) {
		strcpy(tmpBuf, ("Set IPV6 DoS enable error!"));
		goto setErr;
	}
#endif
	apmib_update_web(CURRENT_SETTING);
...
void formFirewall(request *wp, char *path, char *query)
{
	...
	char	tmpBuf[100];
	int dos_level = 0;
	int	floodCount=100, blockTimer=300;
	long	enabled = 0;
	tmpStr = req_get_cstream_var(wp, ("filterLevel"), "");

	string_to_dec(tmpStr,&dos_level);
	if ( apmib_set(MIB_DOS_FIREWALL_LEVEL, (void *)&dos_level) == 0) {
		strcpy(tmpBuf, ("Set DoS firewall level error!"));
		goto setErr;
	}

	enabled |= 1;  //dos default enable
	if(!strcmp(tmpStr, "0")) {
		enabled |= CMCC_FIREWARE_LEVEL_LOW;
	}
	else if(!strcmp(tmpStr, "1")) {
		enabled |= CMCC_FIREWARE_LEVEL_MIDDLE;
		enabled |= CMCC_FIREWARE_LEVEL_LOW;
		if ( apmib_set(MIB_DOS_SYSSYN_FLOOD, (void *)&floodCount) == 0) {
			strcpy(tmpBuf, ("Set DoS SYSSYN_FLOOD error!"));
			goto setErr;
		...
		enabled |= 0x800000;	//sensitivity high
		//enabled &= ~0x800000;	//sensitivity low
	}
	else if(!strcmp(tmpStr, "2")) {
		enabled |= CMCC_FIREWARE_LEVEL_HIGH;
		enabled |= CMCC_FIREWARE_LEVEL_MIDDLE;
		enabled |= CMCC_FIREWARE_LEVEL_LOW;
		if ( apmib_set(MIB_DOS_SYSSYN_FLOOD, (void *)&floodCount) == 0) {
			strcpy(tmpBuf, ("Set DoS SYSSYN_FLOOD error!"));
...
void formCluMgmt(request *wp, char *path, char *query)
{
#ifdef CONFIG_USER_CLUSTER_MANAGE
	char *strData = NULL, *submitUrl;
	char tmpBuf[100] = {0};
    unsigned int enable = 0, peri_enable=0, peri_interval=0, clu_pid = 0;
	
    // get cluster enable value
	submitUrl = req_get_cstream_var(wp, "submit-url", "");   // hidden page
	strData = req_get_cstream_var(wp, ("enable_clu_mgmt"), (""));
	if( strData[0] )
    {
        if(!strcmp(strData, "ON"))
            enable = 1 ;
        else 
            enable = 0 ;
    }else
	...
	// get cluster periodic enable value
	strData = req_get_cstream_var(wp, ("clu_periodic_enable"), (""));
	if( strData[0] )
    {
        if(!strcmp(strData, "ON"))
            peri_enable = 1 ;
	...
	// get cluster periodic interval value
	strData = req_get_cstream_var(wp, ("clu_periodic_interval"), (""));
	if( strData[0] )
	{
		peri_interval = atoi(strData);
	}else
		peri_interval = 0;

    // set enable to mib
	if ( apmib_set( MIB_CLUSTER_MANAGE_ENABLE, (void *)&enable) == 0) {
		strcpy(tmpBuf, "Set cluster management enable mib error!");
		goto setErr_clu_mgmt;
	}
	if ( apmib_set( MIB_CLUSTER_MANAGE_PERIODIC_ENABLE, (void *)&peri_enable) == 0) {
		strcpy(tmpBuf, "Set cluster periodic enable mib error!");
		goto setErr_clu_mgmt;
	}
	if ( apmib_set( MIB_CLUSTER_MANAGE_PERIODIC_INTERVAL, (void *)&peri_interval) == 0) {
		strcpy(tmpBuf, "Set cluster periodic interval mib error!");
		goto setErr_clu_mgmt;
	}
    ...
    if(1==enable)
    {
        clu_pid = read_pid(CLU_CLIENT_RUNFILE);
        if(clu_pid > 0)
        {
			printf("cluster_client exist, killall cluster_client\n");
			system("killall cluster_client");
        }
		printf("start cluster_client\n");
		system("cluster_client &");
		printf("start cluster_client done\n");
	}else{
		printf("killall cluster_client\n");
		system("killall cluster_client");
    }
...
void formPing(request *wp, char *path, char *query)
{
	...
	if(wan_ifac&&wan_ifac[0]&&memcmp(wan_ifac,"default",strlen("default"))) {
		...
		//wan_num = (wan_num==1)?1:wan_num+6;// wan1->eth1,wan2->eth(8+0),wan3->eth(8+1)
		if(getWanIfNameByWanIdx(wan_num, wan_ifname)<0)
		{			
			printf("\n%s:%d get wan interface name fail!!!\n",__FUNCTION__,__LINE__);
			return;
		}
		//rtl_gethostbyname() just used for dns address is ipv4, so if rtl_gethostbyname() fail, it also use gethostbyname 
		if(rtl_gethostbyname(wan_ifname, target_addr, target_ip)<0)
			snprintf(buf, sizeof(buf), "ping -I %s -c %d -W 3 %s 2>&1", wan_ifname, num, target_addr);
		else
			snprintf(buf, sizeof(buf), "ping -I %s -c %d -W 3 %s 2>&1", wan_ifname, num, target_ip);
