...
#ifdef REBOOT_CHECK
char okMsg[MSG_MAX_LEN]={0};
char lastUrl[URL_MAX_LEN]={0};
...
void get_multiWan_info(char *argv,char *phy_interface,char *real_interface,int *wan_index)
{
	...
	getWanIfName(*wan_index, phy_interface,32);	
	if(wan_type ==PPPOE)
		getPPPIfName (*wan_index, real_interface,32);
	else
...
/*      IOCTL system call */
static int rtl865x_ioctl(char *name, unsigned int arg0, unsigned int arg1, unsigned int arg2, unsigned int arg3)
{
	  unsigned int args[4];
	  struct ifreq ifr;
	  int sockfd;
...
int rtl_get_wan_port_link_status(char *interface)
{
        unsigned int    ret;
        unsigned int    args[0];

        rtl865x_ioctl(interface, RTL8651_IOCTL_GETWANLINKSTATUS, (unsigned int)(args), 0, (unsigned int)&ret) ;
        return ret;
}
#ifdef CONFIG_IPV6
int addr6trans(addr6CfgParam_t *ipv6addr, char *straddr)
{
		addr6CfgParam_t ipaddr6;int i=0;
		memcpy(&ipaddr6, ipv6addr, sizeof(addr6CfgParam_t));
		for(i=0;i<8;i++)
			ipaddr6.addrIPv6[i] = htons(ipaddr6.addrIPv6[i]);
		if(!inet_ntop(AF_INET6, &ipaddr6.addrIPv6, straddr, INET6_ADDRSTRLEN)){
...
#endif
int multi_wan_name_get(request *wp)
{
	WANIFACE_T wanEntry={0};
	int i;
	char buf[128*WANIFACE_NUM];
	...
	for(i=1;i<=WANIFACE_NUM;i++)
	{
		...
		//printf("%s:%d wanEntry[%d].WanName=%s\n",__FUNCTION__,__LINE__,i,wanEntry.WanName);
		snprintf(optionbuf,sizeof(optionbuf),"<option value=\"%s\">%s</option>\
			",wanEntry.WanName,wanEntry.WanName);
		strcat(buf,optionbuf);
	}
	//printf("%s",buf);
	req_format_write(wp,buf);
	return 0;
}
int cmcc_init_multi_wan(request *wp)
{
	int i=0,j=0,napt=0,cmode=0,brmode=0,AddrMode=0,ipDhcp=0,dnsMode=0,mtu=0,itfGroup=0,lan_dhcp_mode=0,
		...
		itfenable=0,dslite_aftr_mode=0,dnsv6Mode=0,mcast_vid=0,is_main_wan=0,multiWanPortId=0,hwNat=0,
		hwNatFlag=0,hwNatIsGrey=0;
...
//printf("%s:%d\n",__FUNCTION__,__LINE__);
	for(i=1;i<=WANIFACE_NUM;i++)
	{
...
#ifdef CONFIG_IPV6
		#ifdef CONFIG_CMCC  // for CT UI display
		...
		if(!wanEntry.enable) IpProtocolType=0;
			...
			IpProtocolType=wanEntry.IpProtocol;
			if(IpProtocolType<1 || IpProtocolType>3)
				IpProtocolType=1;

			//printf("\n%s:%d IpProtocolType=%d\n",__FUNCTION__,__LINE__,IpProtocolType);
#else
			IpProtocolType=1;
#endif
		}
		...
		if(wanEntry.dsliteEnable) dslite_enable=1;
	...
	if(wanEntry.hw_nat_enabled==1)
		is_main_wan = 1;
	else
		is_main_wan = 0;
	//set mainwan init
#ifdef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT
	multiWanPortId=wanEntry.portId;
	if(wanEntry.hw_nat_enabled==1)
		hwNat = 1;
	else
		hwNat = 0;
	
	if((wanEntry.hw_nat_enabled==0)&&hwNatFlag==0)
		hwNatIsGrey=0;
	else if((wanEntry.hw_nat_enabled==0)&&hwNatFlag==1)
		hwNatIsGrey=1;
	else if(wanEntry.hw_nat_enabled==1)
		hwNatIsGrey=0;
#endif
		req_format_write(wp,"push(new it_nr(\"%s\"\n\
			, new it(\"upmode\", 0)\n\
			, new it(\"napt\", %d)\n\
			, new it(\"cmode\", %d)\n\
			, new it(\"brmode\", 0)\n\
			, new it(\"AddrMode\", %d)\n\
			, new it(\"pppUserName\", \"%s\")\n\
			, new it(\"pppPassword\", \"%s\")\n\
			, new it(\"pppAuth\", %d)\n\
			, new it(\"pppServiceName\", \"%s\")\n\
			, new it(\"pppACName\", \"%s\")\n\
			, new it(\"pppCtype\", %d)\n\
			, new it(\"ipDhcp\", %d)\n\
			, new it(\"ipAddr\", \"%s\")\n\
			, new it(\"remoteIpAddr\", \"%s\")\n\
			, new it(\"netMask\", \"%s\")\n\
			, new it(\"dgw\", %d)\n\
			, new it(\"v4dns1\", \"%s\")\n\
			, new it(\"v4dns2\", \"%s\")\n\
			...
			, new it(\"applicationtype\", %d)\n\
			, new it(\"disableLanDhcp\", %d)\n\
			, new it(\"IpProtocolType\", %d)\n\
			, new it(\"slacc\", %d)\n\
			, new it(\"staticIpv6\", %d)\n\
			, new it(\"Ipv6Addr\", \"%s\")\n\
			, new it(\"Ipv6Gateway\", \"%s\")\n\
			, new it(\"Ipv6Dns1\", \"%s\")\n\
			, new it(\"Ipv6Dns2\", \"%s\")\n\
			, new it(\"DSLiteRemoteIP\", \"%s\")\n\
			, new it(\"dslite_enable\", %d)\n\
			, new it(\"Ipv6PrefixLen\", %d)\n\
			, new it(\"itfenable\", %d)\n\
			, new it(\"iana\", %d)\n\
			, new it(\"iapd\", %d)\n\
			, new it(\"dslite_aftr_mode\", %d)\n\
			, new it(\"dslite_aftr_hostname\", \"\")\n\
			, new it(\"dnsv6Mode\", %d)\n\
			, new it(\"enable\", %d)\n\
			, new it(\"mcastVid\", %d)\n\
			, new it(\"isMainWan\", %d)\n\
			, new it(\"multiWanPortId\", %d)\n\
			, new it(\"hwNat\", %d)\n\
			, new it(\"hwNatIsGrey\", %d)\n\
			));\n",

		wanEntry.WanName,napt,cmode,AddrMode,wanEntry.pppUsername,wanEntry.pppPassword,wanEntry.pppAuth,
		wanEntry.pppServiceName,wanEntry.pppACName,wanEntry.pppCtype,ipDhcp,ipAddr,remoteIpAddr,
		netMask,wanEntry.dgw,v4dns1,v4dns2,dnsMode,wanEntry.vlan,
		wanEntry.vlanid,mtu,wanEntry.vlanpriority,itfGroup,wanEntry.applicationtype,disableLanDhcp,IpProtocolType,
		slacc,staticIpv6,Ipv6Addr,Ipv6Gateway,Ipv6Dns1,Ipv6Dns2,DSLiteRemoteIP,dslite_enable,
		Ipv6PrefixLen,itfenable,
	#ifdef CONFIG_IPV6
		wanEntry.ipv6DhcpReqAddrEnable,wanEntry.ipv6DhcpPdEnable,
	#else
		0,0,
	#endif
		dslite_aftr_mode,
		dnsv6Mode,wanEntry.enable, wanEntry.multicastVlan,is_main_wan,multiWanPortId,hwNat,hwNatIsGrey
	);
	}
	return 0;
}
...
{
	int internetWanIdx=0;
	int ipmodeIdx=0;
	int connTypeIdx=0; 
	int i=0,opmode=0;
	WANIFACE_T wanEntry={0};
	char wanIfname[IFNAMSIZE]={0};
	apmib_get( MIB_OP_MODE, (void *)&opmode);
	if(opmode==BRIDGE_MODE) return 1;

	/*jwj: Evan multi wan exist, only eth1 can get wan port phy link status.*/
	if (rtl_get_wan_port_link_status("eth1") < 0)
		return 2;
	
	//get first internet wan as wizard wan
	for(i=1;i<=WANIFACE_NUM;i++){
		getWanIfaceEntry(i,&wanEntry);
		if(wanEntry.enable==0 || wanEntry.cmode==0|| wanEntry.AddressType==BRIDGE) continue;
		if(wanEntry.applicationtype==APPTYPE_TR069_INTERNET || wanEntry.applicationtype==APPTYPE_INTERNET)
			break;
	}
	//if no internet wan, select first wan
	if(i>WANIFACE_NUM) i=1;
	internetWanIdx=i;

	getWanIfaceEntry(internetWanIdx,&wanEntry);
	wanEntry.enable=1;
	wanEntry.applicationtype=APPTYPE_TR069_INTERNET;	

	getEthWanIfName(internetWanIdx,wanIfname,sizeof(wanIfname));
...
#endif
	//printf("%s:%d wanIfname=%s\n",__FUNCTION__,__LINE__,wanIfname);
	cmcc_genWanName(internetWanIdx,&wanEntry);
	req_format_write(wp,"internetWanIdx=%d;\n",internetWanIdx);
	req_format_write(wp,"ipmodeIdx=%d;\n",ipmodeIdx);
	req_format_write(wp,"wanName=\"%s\";\n",wanEntry.WanName);
	req_format_write(wp,"connTypeIdx=%d;\n",connTypeIdx);
	return 0;
}
#endif
...
int getInfo(request *wp, int argc, char **argv)
{
...
#endif
#if defined(MULTI_WAN_SUPPORT)
	if(!strcmp(argv[0],("multi_wan_js_comment_start")))
	{
		req_format_write(wp,"");
		return 0;
	}
	else if(!strcmp(argv[0],("multi_wan_js_comment_end")))
	{
		req_format_write(wp,"");
		return 0;
	}
#ifdef CONFIG_CMCC
	if(!strcmp(argv[0],("cmcc_init_multi_wan"))){
		//printf("%s:%d\n",__FUNCTION__,__LINE__);
		cmcc_init_multi_wan(wp);
	//printf("%s:%d\n",__FUNCTION__,__LINE__);
		return 0;
	} 
	else if(!strcmp(argv[0],("initPageWizard"))){
		cmcc_init_wizard(wp);
		return 0;
	}else if (!strcmp(argv[0],("multi_wan_name_get"))) {
		multi_wan_name_get(wp);
		return 0;
	}
#endif
#else
	...
	if(!strcmp(argv[0],("mac_shaping_start"))){
		req_format_write(wp, "");
		return 0;
	}
	else if(!strcmp(argv[0],("mac_shaping_end"))){
		req_format_write(wp, "");
		return 0;
	}
#else
	...
	if(!strcmp(argv[0],("macfilter_dmac_start"))){
	#if defined(DST_MAC_FILTER)
		req_format_write(wp, "");
	#else
		req_format_write(wp, "<!--");
	#endif
	...
	else if(!strcmp(argv[0],("macfilter_dmac_end"))){
	#if defined(DST_MAC_FILTER)
		req_format_write(wp, "");
	#else
		req_format_write(wp, "-->");
	...
	if(!strcmp(argv[0],("macfilter_mode_support_start"))){
	#if defined(MACFILTER_MODE_SUPPORT)
		req_format_write(wp, "");
	#else
		req_format_write(wp, "<!--");
	#endif
	...
	else if(!strcmp(argv[0],("macfilter_mode_support_end"))){
	#if defined(MACFILTER_MODE_SUPPORT)
		req_format_write(wp, "");
	#else
		req_format_write(wp, "-->");
	...
	else if(!strcmp(argv[0],"otherMainWanNum")){
#ifdef MAIN_WAN_MAPPING
			int wanidx_bak=0,hwnat_enabled=0,main_wan_count=0,wan_enable=0;
			unsigned char wan_num = WANIFACE_NUM;
			apmib_get(MIB_WANIFACE_CURRENT_IDX,(void*)&wanidx_bak);
			for(i=WAN1;i<=wan_num;i++){
				//if(i==wan_index) continue;
				//printf("i=%d\n",i);
				apmib_set(MIB_WANIFACE_CURRENT_IDX,(void*)&i);
				apmib_get(MIB_WANIFACE_HW_NAT_ENABLED,(void*)&hwnat_enabled);
				apmib_get(MIB_WANIFACE_ENABLE,(void*)&wan_enable);
				if(wan_enable)
					main_wan_count+=hwnat_enabled;
			}
			if(wanidx_bak>=WAN1 && wanidx_bak<=wan_num)
				apmib_set(MIB_WANIFACE_CURRENT_IDX,(void*)&wanidx_bak);//printf("wanidx_bak:%d\n", wanidx_bak);
			snprintf(buffer,sizeof(buffer),"%d",main_wan_count);
			return req_format_write(wp,buffer);//printf("buffer:%s\n", buffer);
#else
			return req_format_write(wp,"%d",0);
#endif
			}
	else if(!strcmp(argv[0],"main_wan_support")){
#ifdef MAIN_WAN_MAPPING
	...
	else if(strcmp(argv[0],("ipv6_htm_cmd_start"))==0
		||strcmp(argv[0],("ipv6_htm_cmd_end"))==0
		||strcmp(argv[0],("ipv6_js_cmd_start"))==0
		||strcmp(argv[0],("ipv6_js_cmd_end"))==0
		||strcmp(argv[0],"ipv6_qos_start")==0
		||strcmp(argv[0],"ipv6_qos_end")==0
		){
		req_format_write(wp, "");
		return 0;
	}
#if defined(MULTI_WAN_SUPPORT)
	else if(!strcmp(argv[0],("ipv6_lan_js_comment_start")))
	{
		req_format_write(wp,"");
		return 0;
	}
	else if(!strcmp(argv[0],("ipv6_lan_js_comment_end")))
	{
		req_format_write(wp,"");
		return 0;
	}
	else if(!strcmp(argv[0],("ipv6_lan_html_comment_start")))
	{
		req_format_write(wp,"");
		return 0;
	}
	else if(!strcmp(argv[0],("ipv6_lan_html_comment_end")))
	{
		req_format_write(wp,"");
		return 0;
		}
	...
	else if(strcmp(argv[0],("ipv6_htm_cmd_end"))==0
		 || strcmp(argv[0],"ipv6_qos_end")==0){
			req_format_write(wp, "-->");
			return 0;
	}
	else if(strcmp(argv[0],("ipv6_js_cmd_start"))==0){
			req_format_write(wp, "/*");
			return 0;
	}
	else if(strcmp(argv[0],("ipv6_js_cmd_end"))==0){
			req_format_write(wp, "*/");
			return 0;
	}
	else if(!strcmp(argv[0],("ipv6_lan_js_comment_start")))
...
//	printf("get parameter=%s\n", argv[0]);
	...
	else if(!strcmp(name, "wlEnableProfile"))
	{
		...
		FILE *fp;
		char *ptr  = NULL;
		int profile_enabled_id, profileEnabledVal, profile_num_id, profileNumValue;
...
//printf("\r\n wlan_idx=[%d],vwlan_idx=[%d],__[%s-%u]\r\n",wlan_idx,vwlan_idx,__FILE__,__LINE__);	
		...
		apmib_get(profile_enabled_id, (void *)&profileEnabledVal);
		apmib_get(profile_num_id, (void *)&profileNumValue);

		if( (vwlan_idx == 0 || vwlan_idx == NUM_VWLAN_INTERFACE) 
			&& profileEnabledVal == 1 
			&& (wlan_mode == CLIENT_MODE) && profileNumValue>0 )
		{
...
//printf("\r\n buffer[%s],__[%s-%u]\r\n",buffer,__FILE__,__LINE__);			
	...
	else if ( !strcmp(name, "relay_dhcp_server_ip")) {
		if ( !apmib_get(MIB_DHCP_RELAY_SERVER_IP,  (void *)buffer) )
			return -1;
   		return req_format_write(wp, "%s", inet_ntoa(*((struct in_addr *)buffer)) );
	}	
	else if ( !strcmp(name, "access_relay_server_wan_idx")) {
		if ( !apmib_get(MIB_DHCP_RELAY_WAN_IDX, (void *)&intVal) )
			return -1;
		sprintf(buffer, "%d", intVal);
		return req_format_write(wp, "%s", buffer);
	}	
#endif
	else if ( !strcmp(name, "max_wan_num")) {	
	...
	else if ( !strcmp(name, "ntpServerIp3")) {
		if ( !apmib_get( MIB_NTP_SERVER_IP3,  (void *)buffer) )
			return -1;
		if (!memcmp(buffer, "\x0\x0\x0\x0", 4))
			return req_format_write(wp, "");
...
xxx
	...
	else if(!strcmp(name, "show_multiwan_qos")){
		#if defined (MULTI_WAN_QOS) && !defined(CONFIG_RTL_AP_PACKAGE)
			return req_format_write(wp,"1");
		#else
			return req_format_write(wp,"0");
		#endif
...
// for ap package of new ui , end
	...
	else if(!strcmp(name, "route_menu_onoff"))
	{
		...
		return req_format_write(wp, "menu.addItem(\"123Route Setup\", \"route.htm\", \"\", \"123Route Setup\");");
#else
		return req_format_write(wp,"");
#endif
	}
...
{
#ifdef CONFIG_APP_RTL_QUAGGA ||CONFIG_APP_PIMD ||CONFIG_APP_MROUTED
	 ...
	 req_format_write(wp, "dynamic_routing.addItem(\"Pimd Setup\", \"pimd.htm\", \"\", \"Pimd Setup\");");
 #endif
 #ifdef CONFIG_APP_MROUTED
	 req_format_write(wp, "dynamic_routing.addItem(\"Mrouted Setup\", \"mrouted.htm\", \"\", \"Mrouted Setup\");");
 #endif
return req_format_write(wp, "menu.makeLastSubmenu(dynamic_routing);");
	...
	else if ( !strcmp(name, "wan-ip"))
  {
	...
	else if(!strcmp(argv[0],"save_default_setting_comment_start"))
	{
#if defined(DOWNLOAD_UPLOAD_DEFAULT_SETTING)
		req_format_write(wp, "");
#else
		req_format_write(wp, "<!--");
#endif
	...
	else if(!strcmp(argv[0],"save_default_setting_comment_end"))
	{
#if defined(DOWNLOAD_UPLOAD_DEFAULT_SETTING)
		req_format_write(wp, "");
#else
	...
	else if(!strcmp(argv[0],"vpn_js_start"))
	{
#if defined(SINGLE_WAN_SUPPORT)
		req_format_write(wp, "");
#else
	...
	else if(!strcmp(argv[0],"vpn_js_end"))
	{
#if defined(SINGLE_WAN_SUPPORT)
		req_format_write(wp, "");
#else
	...
	else if(!strcmp(argv[0],"multi_wan_port_mode_promet_start")){
#ifdef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT
		req_format_write(wp, "<!--");
#else
		req_format_write(wp, "");
	...
	else if(!strcmp(argv[0],"multi_wan_port_mode_promet_end")){
#ifdef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT
		req_format_write(wp, "-->");
#else
		req_format_write(wp, "");
...
//		int i;
	...
	bzero(buffer,sizeof(buffer));	
		...
		return req_format_write(wp, "%d", wp->auth_flag);	
#else
		buffer[0]='\0';
	...
	else if(!strcmp(name, "vlan_wan_wifi_vap3_pri"))
	{
	...
	else if(!strcmp(name,"multi_phy_wan_qos_start")){
	...
	else if(!strcmp(name,"multi_phy_wan_qos_end")){
		#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
			req_format_write(wp, "");
		#else
			req_format_write(wp, "-->");
	...
	else if(!strcmp(name,"no_multi_phy_wan_qos_start")){
		#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
		req_format_write(wp, "<!--");
		#else
		req_format_write(wp, "");
	...
	else if(!strcmp(name,"no_multi_phy_wan_qos_end")){
		#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
			req_format_write(wp, "-->");
		#else
			req_format_write(wp, "");
...
#ifndef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT
	else if(!strcmp(name, "qos_mode")){
		if(!apmib_get( MIB_QOS_MODE, (void *)buffer)){
			return -1;
		}
		return req_format_write(wp, buffer);	
	}
	  ...
	  sprintf(buffer, "%d", intVal );
		...
		return req_format_write(wp, buffer);	}
#ifdef CONFIG_IPV6
	else if ( !strcmp(name, "Ipv6SmurfFlood")) {
		if ( !apmib_get( MIB_IPV6_DOS_SMURF_FLOOD, (void *)&intVal) )
			return -1;
		sprintf(buffer, "%d", intVal );
		return req_format_write(wp, buffer);	}
	else if ( !strcmp(name, "Ipv6IcmpTooBigFlood")) {
		if ( !apmib_get( MIB_IPV6_DOS_ICMP_TOO_BIG_FLOOD, (void *)&intVal) )
			return -1;
		sprintf(buffer, "%d", intVal );
		return req_format_write(wp, buffer);	}
	else if ( !strcmp(name, "Ipv6NSFlood")) {
		if ( !apmib_get( MIB_IPV6_DOS_NS_FLOOD, (void *)&intVal) )
			return -1;
		sprintf(buffer, "%d", intVal );
		return req_format_write(wp, buffer);	}
#endif
...
/*+++++added by Jack for TR-069 configuration+++++*/
...
#if defined(MULTI_WAN_SUPPORT)
	else if(!strcmp(name, "tr069_filetrans_start")) {
		req_format_write(wp, "");
		return 0;
	}else if(!strcmp(name, "tr069_filetrans_end")) {
		req_format_write(wp, "");
		return 0;
	}else if(!strcmp(name, "filetrans-intf-specify-0")) { /*disable radio*/
        unsigned char cVal = 0;
		if ( !apmib_get( MIB_CWMP_DLUL_INTF_SPECIFY_ENABLE, (void *)&cVal) )
			return -1;
		if(cVal==0){
			return req_format_write(wp,"checked");
		}else{
	...
	}else if(!strcmp(name, "filetrans-intf-specify-1")) { /*enable radio*/
        unsigned char cVal = 0;
		if ( !apmib_get( MIB_CWMP_DLUL_INTF_SPECIFY_ENABLE, (void *)&cVal) )
			return -1;
		if(cVal==1){
	...
	}else if(!strcmp(name, "tr069_get_dns_wanList")) {
		int i = 1, isSelected = 0;
		char ifname[32] = {0}, filename[32] = {0}, dns_intf[MAX_NAME_LEN]={0}, *ptr=NULL;
		FILE *fp = NULL;
		WANIFACE_T wanEntry={0}, *pWanEntry = &wanEntry;

		apmib_get(MIB_CWMP_DLUL_DNS_INTF, (void *)dns_intf);
		for(i=1; i<WANIFACE_NUM; i++)
		{
			getWanIfaceEntry(i, pWanEntry);
			if(!pWanEntry->enable) continue;

			snprintf(filename, sizeof(filename), "/var/wan_interface_%d", i);
			FILE *fp = fopen(filename ,"r");
			memset(ifname, 0x0, sizeof(ifname));
			isSelected = 0;
			if(fp)
			{
				fread(ifname, sizeof(ifname), sizeof(char), fp);
				fclose(fp);
			}else
				continue;

			//format ifname
			ptr = &ifname[strlen(ifname)-1];
			if(*ptr=='\n') 
				*ptr = '\0';
			if(strlen(dns_intf)!=0 && strcmp(dns_intf, ifname)==0)
				isSelected = 1;
			
			req_format_write(wp,"push(new it_nr(\"%s\"\n\
			, new it(\"wan_index\", %d)\n\
			, new it(\"isSelected\", %d)\n\
			, new it(\"wan_readable_name\", \"%s\")\n\
			, new it(\"interface_name\", \"%s\")\n));\n", 
			pWanEntry->WanName, i, isSelected, pWanEntry->WanName, ifname);
		}
		
		return 0;
	}else if(!strcmp(name, "tr069_get_data_wanList")) {
		int i = 1, isSelected = 0;
		char ifname[32] = {0}, filename[32] = {0}, data_intf[MAX_NAME_LEN]={0}, *ptr=NULL;
		FILE *fp = NULL;
		WANIFACE_T wanEntry={0}, *pWanEntry = &wanEntry;

		apmib_get(MIB_CWMP_DLUL_INTF, (void *)data_intf);
		for(i=1; i<WANIFACE_NUM; i++)
		{
			getWanIfaceEntry(i, pWanEntry);
			if(!pWanEntry->enable) continue;
			...
			if(*ptr=='\n') 
				*ptr = '\0';
			if(strlen(data_intf)!=0 && strcmp(data_intf, ifname)==0)
				isSelected = 1;
...
//		req_format_write(wp,"<tr><hr size=1 noshade align=top></tr>");
	...
	{SCHEDULE_T wlan_sched_mode;
	int schedule_Num=0,j=0;
	apmib_get(MIB_WLAN_SCHEDULE_TBL_NUM, (void *)&schedule_Num);

		if(schedule_Num > 0){
			for (j=1; j<2; j++) {
				*((char *)&wlan_sched_mode) = (char)j;
				apmib_get(MIB_WLAN_SCHEDULE_TBL, (void *)&wlan_sched_mode);
				if(
(wlan_sched_mode.eco & ECO_MODE_MASK)== ECO_MODE_1_BIT)
				{
					intVal=1;
				}else{
					intVal=0;
					}
				return req_format_write(wp, "%d",intVal);
			}
		}else{
			intVal=0;
			return req_format_write(wp, "%d",intVal);
...
// add for ds-lite
	...
	else if (!strcmp(name,"getPppAuthByLanStatus"))
	{
#ifdef GET_PPP_AUTH_BY_LAN
		FILE *fp=fopen("/var/getPppAuthFromLanStatus","r");
		char buff[128]={0};
		if(!fp){
			return req_format_write(wp,"Not Start");
		}
		if(fgets(buff,sizeof(buff),fp)!=NULL){
			fclose(fp);
			if(strstr(buff,"start")){
				return req_format_write(wp,"start");
			}else
			if(strstr(buff,"success")){
				return req_format_write(wp,"success");
			}else
			if(strstr(buff,"stop")){
				return req_format_write(wp,"stop");
			}			
		}
		else{
			fclose(fp);
	...
	else if(!strcmp(name,"getPppAuth_start"))
	{
#ifdef GET_PPP_AUTH_BY_LAN
		system("getPppAuthFromLan.sh start");
		return 0;
#endif
		return -1;
	}
	else if(!strcmp(name,"getPppAuth_stop"))
	{
#ifdef GET_PPP_AUTH_BY_LAN
		system("getPppAuthFromLan.sh stop");
		return 0;
#endif
		return -1;
	}
	else if(!strcmp(name,"show_elink"))
	{
	...
	{
		int exist;
		char version[ELINKSDK_PARAM_VERSION_LEN]={0};
		char config_file[128];
		
		if(rtl_elinksdk_is_alive(&exist) == ELINKSDK_FILE_NOT_EXIST){
				return req_format_write(wp,"未安装");
		}else{
			if(exist){
				snprintf(config_file, sizeof(config_file), "%s/%s", RTL_ELINKSDK_DIR, RTL_ELINKSDK_CONFIG);
				rtl_elinksdk_config_parse(config_file, ELINKSDK_PARAM_VERSION, version);
				return req_format_write(wp,version);
			}else{
				return req_format_write(wp,"未安装");
			}
		}
...
#endif
	else if(!strcmp(name, "syslog_display_level"))
	{
		int level=0;
		char buf[16] = {0};
		FILE *fp = NULL;

		fp = fopen(SCRLOG_DISPLAY_CONFFILE, "r");
		if(fp)
		{
		...
		return req_format_write(wp, "%d", level);
	}
...
#if defined(CONFIG_ISP_IGMPPROXY_MULTIWAN)&& defined(CONFIG_ISP_ON_PHY)
	else if(!strcmp(name,"onPhyIf")) 	sprintf(buffer,"%d",entry.onPhyIf);
	else if(!strcmp(name,"onPhyIf_comment_start"))	sprintf(buffer,"");
	else if(!strcmp(name,"onPhyIf_comment_end"))		sprintf(buffer,"");
	else if(!strcmp(name,"onPhyIf_jscomment_start"))	sprintf(buffer,"");
	else if(!strcmp(name,"onPhyIf_jscomment_end"))		sprintf(buffer,""); 
#else
	else if(!strcmp(name,"onPhyIf_jscomment_start"))	sprintf(buffer,"/*");
	else if(!strcmp(name,"onPhyIf_jscomment_end"))		sprintf(buffer,"*/"); 
	else if(!strcmp(name,"onPhyIf_comment_start"))	sprintf(buffer,"<!--");
...
#ifdef MAIN_WAN_MAPPING		
	else if(!strcmp(name,"setMainWan"))			snprintf(buffer,sizeof(buffer),"%d",entry.hw_nat_enabled);
	else if(!strcmp(name,"setMainWan_comment_start"))	snprintf(buffer,sizeof(buffer),"");
	else if(!strcmp(name,"setMainWan_comment_end"))		snprintf(buffer,sizeof(buffer),"");
	else if(!strcmp(name,"setMainWan_jscomment_start"))	snprintf(buffer,sizeof(buffer),"");
	else if(!strcmp(name,"setMainWan_jscomment_end"))		snprintf(buffer,sizeof(buffer),""); 
	else if(!strcmp(name,"wanIfName"))			{
		char wanIfName[IFNAMESIZE]={0};
		//printf("%s:%d wan_index=%d mappingWanIfIdx=%d\n",__FUNCTION__,__LINE__,wan_index,entry.mappingWanIfIdx);
		getWanIfName(wan_index,wanIfName,sizeof(wanIfName));
		if(entry.AddressType==PPPOE||entry.AddressType==PPTP||entry.AddressType==L2TP)
			getPPPIfName(wan_index,wanIfName,sizeof(wanIfName));
		//printf("%s:%d wanIfName=%s\n",__FUNCTION__,__LINE__,wanIfName);
		snprintf(buffer,sizeof(buffer),"%s",wanIfName);	
	}
	else if(!strcmp(name,"setNatEnable")) {
		if(entry.nat_disable) snprintf(buffer,sizeof(buffer),"0");
		else snprintf(buffer,sizeof(buffer),"1");
	}
	else if(!strcmp(name,"otherMainWanNum")){
		int wanidx_bak=0,hwnat_enabled=0,main_wan_count=0,wan_enable=0;
		apmib_get(MIB_WANIFACE_CURRENT_IDX,(void*)&wanidx_bak);
		for(i=WAN1;i<=wan_num;i++){
			if(i==wan_index) continue;
...
#else
	else if(!strcmp(name,"setMainWan"))			snprintf(buffer,sizeof(buffer),"0");
	else if(!strcmp(name,"setMainWan_comment_start"))	snprintf(buffer,sizeof(buffer),"<!--");
	else if(!strcmp(name,"setMainWan_comment_end"))		snprintf(buffer,sizeof(buffer),"-->");
	else if(!strcmp(name,"setMainWan_jscomment_start"))	snprintf(buffer,sizeof(buffer),"/*");
	else if(!strcmp(name,"setMainWan_jscomment_end"))		snprintf(buffer,sizeof(buffer),"*/"); 
	else if(!strcmp(name,"wanIfName"))			snprintf(buffer,sizeof(buffer),"");	
	else if(!strcmp(name,"setNatEnable"))   snprintf(buffer,sizeof(buffer),"1");
	else if(!strcmp(name,"otherMainWanNum"))		snprintf(buffer,sizeof(buffer),"0");
#endif
#ifdef CONFIG_IPV6
...
//dhcpv6 client
	else if(!strcmp(name,"enable_dhcpv6reqaddr")) 			sprintf(buffer,"%d",entry.ipv6DhcpReqAddrEnable);
	else if(!strcmp(name,"enable_dhcpv6pd"))				sprintf(buffer,"%d",entry.ipv6DhcpPdEnable);
	else if(!strcmp(name,"enable_dhcpv6RapidCommit"))		sprintf(buffer,"%d",entry.ipv6DhcpRapidCommitEnable);
//dslite
#ifdef CONFIG_DSLITE_SUPPORT
	else if(!strcmp(name,"dsliteMode")) 			sprintf(buffer,"%d",entry.dsliteMode);
//dslite aftr
	else if(!strcmp(name,"wan_dsliteAftr_0")) 		sprintf(buffer,"%04x",entry.addr6AftrParam.addrIPv6[0]);
	else if(!strcmp(name,"wan_dsliteAftr_1")) 		sprintf(buffer,"%04x",entry.addr6AftrParam.addrIPv6[1]);
	else if(!strcmp(name,"wan_dsliteAftr_2")) 		sprintf(buffer,"%04x",entry.addr6AftrParam.addrIPv6[2]);
	else if(!strcmp(name,"wan_dsliteAftr_3")) 		sprintf(buffer,"%04x",entry.addr6AftrParam.addrIPv6[3]);
	else if(!strcmp(name,"wan_dsliteAftr_4")) 		sprintf(buffer,"%04x",entry.addr6AftrParam.addrIPv6[4]);
	else if(!strcmp(name,"wan_dsliteAftr_5")) 		sprintf(buffer,"%04x",entry.addr6AftrParam.addrIPv6[5]);
	else if(!strcmp(name,"wan_dsliteAftr_6")) 		sprintf(buffer,"%04x",entry.addr6AftrParam.addrIPv6[6]);
	else if(!strcmp(name,"wan_dsliteAftr_7")) 		sprintf(buffer,"%04x",entry.addr6AftrParam.addrIPv6[7]);
	else if(!strcmp(name,"wan_dsliteAftr_prefixLen")) sprintf(buffer,"%d",entry.addr6AftrParam.prefix_len);
#if 0
	else if(!strcmp(name,"dsliteAftr")) 			sprintf(buffer,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
		entry.addr6AftrParam.addrIPv6[0],entry.addr6AftrParam.addrIPv6[1],entry.addr6AftrParam.addrIPv6[2],entry.addr6AftrParam.addrIPv6[3],
		entry.addr6AftrParam.addrIPv6[4],entry.addr6AftrParam.addrIPv6[5],entry.addr6AftrParam.addrIPv6[6],entry.addr6AftrParam.addrIPv6[7]);
	else if(!strcmp(name,"ipv6WanIp"))				sprintf(buffer,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
		entry.ipv6Addr.addrIPv6[0],entry.ipv6Addr.addrIPv6[1],entry.ipv6Addr.addrIPv6[2],entry.ipv6Addr.addrIPv6[3],
		entry.ipv6Addr.addrIPv6[4],entry.ipv6Addr.addrIPv6[5],entry.ipv6Addr.addrIPv6[6],entry.ipv6Addr.addrIPv6[7]);
	else if(!strcmp(name,"ipv6DefGW"))				sprintf(buffer,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
		entry.ipv6Gw.addrIPv6[0],entry.ipv6Gw.addrIPv6[1],entry.ipv6Gw.addrIPv6[2],entry.ipv6Gw.addrIPv6[3],
		entry.ipv6Gw.addrIPv6[4],entry.ipv6Gw.addrIPv6[5],entry.ipv6Gw.addrIPv6[6],entry.ipv6Gw.addrIPv6[7]);
#endif
	else if(!strcmp(name,"dslite_comment_start"))	sprintf(buffer,"");
	else if(!strcmp(name,"dslite_comment_end"))		sprintf(buffer,"");	
#else
	else if(!strcmp(name,"dslite_comment_start"))	sprintf(buffer,"<!--");
	else if(!strcmp(name,"dslite_comment_end"))		sprintf(buffer,"-->");	
#endif
#ifdef CONFIG_SIXRD_SUPPORT
	else if(!strcmp(name,"6rd_ipv6Addr_0")) 		sprintf(buffer,"%04x",entry.addr66rdParam.addrIPv6[0]);
	else if(!strcmp(name,"6rd_ipv6Addr_1")) 		sprintf(buffer,"%04x",entry.addr66rdParam.addrIPv6[1]);
	else if(!strcmp(name,"6rd_ipv6Addr_2")) 		sprintf(buffer,"%04x",entry.addr66rdParam.addrIPv6[2]);
	else if(!strcmp(name,"6rd_ipv6Addr_3")) 		sprintf(buffer,"%04x",entry.addr66rdParam.addrIPv6[3]);
	else if(!strcmp(name,"6rd_ipv6Addr_4")) 		sprintf(buffer,"%04x",entry.addr66rdParam.addrIPv6[4]);
	else if(!strcmp(name,"6rd_ipv6Addr_5")) 		sprintf(buffer,"%04x",entry.addr66rdParam.addrIPv6[5]);
	else if(!strcmp(name,"6rd_ipv6Addr_6")) 		sprintf(buffer,"%04x",entry.addr66rdParam.addrIPv6[6]);
	else if(!strcmp(name,"6rd_ipv6Addr_7")) 		sprintf(buffer,"%04x",entry.addr66rdParam.addrIPv6[7]);
	else if(!strcmp(name,"6rd_ipv6Addr_prefixLen")) sprintf(buffer,"%d",entry.addr66rdParam.prefix_len);
	else if(!strcmp(name,"6rd_wan_ip")) 			sprintf(buffer,"%s",inet_ntoa(*((struct in_addr *)entry.ipAddr)));
	else if(!strcmp(name,"6rd_ipv4Addr_netMask")) 	sprintf(buffer,"%d",entry.ip4MaskLen);
	else if(!strcmp(name,"6rd_BR_IP")) 				sprintf(buffer,"%s",inet_ntoa(*((struct in_addr *)entry.ip46rdBrAddr)));
	else if(!strcmp(name,"6rd_comment_start"))	sprintf(buffer,"");
	else if(!strcmp(name,"6rd_comment_end")) 	sprintf(buffer,""); 
#else
	else if(!strcmp(name,"6rd_comment_start"))	sprintf(buffer,"<!--");
	else if(!strcmp(name,"6rd_comment_end")) 	sprintf(buffer,"-->");	
#endif
	...
	else if(!strcmp(name,"ispMldProxyEnabled")) 
	{
		sprintf(buffer, "%d", entry.enableMLD);
//		printf("entry.enbleMLD:%d, [%s:%d]\n", entry.enableMLD, __FUNCTION__, __LINE__);
	}
	else if(!strcmp(name,"isp_mld_comment_start"))	sprintf(buffer,"");
	else if(!strcmp(name,"isp_mld_comment_end")) 	sprintf(buffer,"");
	else if(!strcmp(name,"isp_mld_jscomment_start"))	sprintf(buffer,"");
	else if(!strcmp(name,"isp_mld_jscomment_end")) 	sprintf(buffer,"");
#else
	else if(!strcmp(name,"isp_mld_jscomment_start"))	sprintf(buffer,"/*");
	else if(!strcmp(name,"isp_mld_jscomment_end")) 	sprintf(buffer,"*/");
	else if(!strcmp(name,"isp_mld_comment_start"))	sprintf(buffer,"<!--");
...
//dhcpv6 client
	else if(!strcmp(name,"ifname_dhcpv6c")) 		sprintf(buffer,"%s",entry.dhcp6cCfgParam.ifName);
	else if(!strcmp(name,"enable_dhcpv6pd"))		sprintf(buffer,"%d",entry.dhcp6cCfgParam.enabled & DHCP6C_PREFIX_ENABLE);
	else if(!strcmp(name,"interfacename_pd"))		sprintf(buffer,"%s",entry.dhcp6cCfgParam.dhcp6pd.ifName);
...
#endif
	else if(!strcmp(name,"ipv6_comment_start"))		sprintf(buffer,"");
	else if(!strcmp(name,"ipv6_comment_end"))		sprintf(buffer,"");
	else if(!strcmp(name,"ipv6_jscomment_start"))	sprintf(buffer,"");
	else if(!strcmp(name,"ipv6_jscomment_end"))		sprintf(buffer,"");
#else
	else if(!strcmp(name,"ipv6LinkType"))			sprintf(buffer,"%d",0);
	else if(!strcmp(name,"ipv6_comment_start"))		sprintf(buffer,"<!--");
	else if(!strcmp(name,"ipv6_comment_end"))		sprintf(buffer,"-->");
	else if(!strcmp(name,"ipv6_jscomment_start"))	sprintf(buffer,"/*");
	else if(!strcmp(name,"ipv6_jscomment_end"))		sprintf(buffer,"*/");
...
int getPortFwWanList(request *wp, int argc, char **argv){
	int wan_num=0,i=0;
	int var=0;//,enable=0;
	char bufferWanEnabled[32],bufferWantype[32],bufferServerType[32];
	char buffer[WAN_ITEM_MAX_LENGTH];
	...
	for(i=1;i<=wan_num;i++)
	{
		...
		*((char *)&entry) = (char)i;
		...
		sprintf(buffer,"wanListArray[%d] = new Array(\"%s\",\"WAN%d\");\n",i-1,bufferWanEnabled,i);
		req_format_write(wp, buffer);
	}
	return 0;
}
...
int ipv6_getWanList(request *wp, int argc, char **argv)
{
	...
	IPV6_ORIGIN_T	origin = IPV6_ORIGIN_AUTO;
...
#if defined(CONFIG_CMCC_QOS)
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
int getQosWanPort(request *wp, int argc, char **argv)
{
	int index, wan_index,num = 0;
	char buffer[512] = {0};
	QOSWANPORT_T wanport_entry;

	if(!apmib_get(MIB_QOS_WANPORT_TBL_NUM,(void *)&num)){
		printf("get MIB_QOS_WANPORT_TBL_NUM error!");
		return -1;
	}
	...
	for(index = 1; index <= num; index++){
		memset(&wanport_entry,'\0',sizeof(wanport_entry));
		*((char *)&wanport_entry) = (char)index;

		if((apmib_get(MIB_QOS_WANPORT_TBL,(void *)&wanport_entry)) == 0){
			printf("get MIB_QOS_WANPORT_TBL error!");
			return -1;
		}
		if(wanport_entry.WanPortInstanceNum != wan_index)
			continue;

		//printf("wanport:%d policy:%d !",wanport_entry.WanPortInstanceNum,wanport_entry.WanPortPlan);
		
		snprintf(buffer, sizeof(buffer), "up_shaping = %d; \n", wanport_entry.WanPortUpLinkSpeed);
		req_format_write(wp, buffer);

		snprintf(buffer, sizeof(buffer), "qos_policy = %d; \n", wanport_entry.WanPortPlan);
		req_format_write(wp, buffer);
	
		snprintf(buffer, sizeof(buffer), "dot1p_remark = %d; \n", wanport_entry.WanPortEnDot1pRemark);
		req_format_write(wp, buffer);

		snprintf(buffer, sizeof(buffer), "qos_enable = %d; \n", wanport_entry.WanPortEnable);
		req_format_write(wp, buffer);

		snprintf(buffer, sizeof(buffer), "en_force_weight = %d; \n", wanport_entry.WanPortEnforceWeight);
		req_format_write(wp, buffer);

		snprintf(buffer, sizeof(buffer), "dscp_remark = %d; \n", wanport_entry.WanPortEnDscpRemark);
		req_format_write(wp, buffer);
		
		snprintf(buffer, sizeof(buffer), "qos_wanport = %d; \n", wanport_entry.WanPortInstanceNum);
		req_format_write(wp, buffer);

		snprintf(buffer, sizeof(buffer), "qos_mode = \"%s\"; \n", wanport_entry.WanPortMode);
		req_format_write(wp, buffer);
	}
...
int getWanPortEnable(request *wp, int argc, char **argv)
{
	int index, wan_port,num = 0;
	char buffer[512] = {0};
	QOSWANPORT_T wanport_entry;
	...
	for(index = 1; index <= num; index++){
		...
		*((char *)&wanport_entry) = (char)index;
		...
		if(wanport_entry.WanPortEnable == 1)
			break;
	}
...
int getQoSWan(request *wp, int argc, char **argv)
{
	int wan_num = WANIFACE_NUM, wan_enable = 0;
	int i;
	char buffer[128] = {0};
	...
	for(i=1;i<=WANIFACE_NUM;i++)
	{
		...
		snprintf(buffer, sizeof(buffer), "wan_qos_policy[%d] = new Array(\"WAN%d\", \"%d\");\n", 
			i-1, i,wan_enable);
...
#endif
int getQosQueue(request *wp, int argc, char **argv)
{
	int index,num = 0;
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
	int index1;
	...
	if(!apmib_get(MIB_QOS_QUEUE_TBL_NUM, (void *)&num)){
		printf("get MIB_QOS_QUEUE_TBL_NUM error!");
		return -1;
	}
...
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
		if(queue_entry.QueueWanPort != wan_index)
			continue;
		index1 = (index-1)%MAX_PER_PORT_QOS_QUEUE_NUM;

		snprintf(buffer, sizeof(buffer), "qos_queue[%d] = new Array(\"%d\",\"%d\", \"%d\", \"%d\");\n", 
			index1, queue_entry.QueueInstanceNum, queue_entry.QueueEnable, queue_entry.Priority, queue_entry.Weight);
#else

		snprintf(buffer, sizeof(buffer), "qos_queue[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%d\");\n", 
			index-1, queue_entry.QueueInstanceNum, queue_entry.QueueEnable, queue_entry.Priority, queue_entry.Weight);
#endif
		//printf("buffer: %s\n", buffer);
		req_format_write(wp, buffer);
	}
...
int getQosClass(request *wp, int argc, char **argv)
{
	int index, class_number;
	int wan_port;
	char buffer[256] = {0};
	QOSCLASS_T class_entry;

	if (!apmib_get( MIB_QOS_CLASS_TBL_NUM, (void *)&class_number)){
...
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
		snprintf(buffer, sizeof(buffer), "qos_class[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%d\", \"%d\");\n", 
			index-1,class_entry.ClassInstanceNum, class_entry.ClassWanPort,  class_entry.ClassQueue, class_entry.DSCPMark, class_entry.Dot1pMark);
#else
		snprintf(buffer, sizeof(buffer), "qos_class[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%d\");\n", 
			index-1, class_entry.ClassInstanceNum, class_entry.ClassQueue, class_entry.DSCPMark, class_entry.Dot1pMark);
#endif
		//printf("buffer: %s\n", buffer);
		req_format_write(wp, buffer);
	}
...
int getQosType(request *wp, int argc, char **argv)
{
	int index, type_number, classid;
	int wan_port;
	char buffer[512] = {0};
	QOSTYPE_T type_entry;

	if (!apmib_get( MIB_QOS_TYPE_TBL_NUM, (void *)&type_number)){
		printf("get MIB_QOS_TYPE_TBL_NUM fail!\n");
		return -1;
	}

	if (!apmib_get( MIB_CURRENT_CLASSID, (void *)&classid)){
		printf("get MIB_CURRENT_CLASSID fail!\n");
		return -1;
	}

	for(index = 1; index <= type_number; index++){
		memset(&type_entry, '\0', sizeof(QOSTYPE_T));
		*((char *)&type_entry) = (char)index;

		if((apmib_get(MIB_QOS_TYPE_TBL, (void *)&type_entry)) == 0){
			printf("get MIB_QOS_TYPE_TBL fail!\n");
			return -1;
		}
		
		if(type_entry.TypeClassId != classid)
			continue;

#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
		snprintf(buffer, sizeof(buffer), "qos_type[%d] = new Array(\"%d\", \"%s\", \"%s\", \"%s\", \"%s\");\n", 
			type_entry.TypeInstanceNum-1, type_entry.TypeInstanceNum, 
			type_entry.TypeName, type_entry.TypeMin, 
			type_entry.TypeMax, type_entry.Protocol
		);
#else		
		snprintf(buffer, sizeof(buffer), "qos_type[%d] = new Array(\"%d\", \"%s\", \"%s\", \"%s\", \"%s\");\n", 
			index-1, type_entry.TypeInstanceNum, 
			type_entry.TypeName, type_entry.TypeMin, 
			type_entry.TypeMax, type_entry.Protocol
		);
#endif
...
int getQosUpCar(request *wp, int argc, char **argv)
{
	QOSCAR_T qosCar;
	unsigned int carNum = 0, index = 0, index2 = 0;
	unsigned int upCarMode = 0, downCarMode = 0;
	unsigned int mib_num = 0, mib_tbl = 0;
	unsigned int wan_port;
	char buffer[512] = {0};
	
	if(!apmib_get(MIB_QOS_CAR_TBL_NUM, (void *)&carNum)){
		printf("get MIB_QOS_CAR_TBL_NUM error!");
		return -1;
	}
#if 0
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
	if(!apmib_get(MIB_QOS_SHAPING_WAN_PORT, (void *)&wan_port)){
		printf("get MIB_QOS_SHAPING_WAN_PORT error!");
		return -1;
	}
#endif	
#endif
	for(index = 1; index <= carNum; index++){
		...
		*((char *)&qosCar) = (char)index;
		...
		if((apmib_get(MIB_QOS_CAR_TBL, (void *)&qosCar)) == 0){
			printf("get MIB_QOS_CAR_TBL error!");
			return -1;
		}
#if 0
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
		if(qosCar.CarWanPort != wan_port)
			continue;
#endif
#endif
		upCarMode = qosCar.SpeedLimitMode_UP;
		downCarMode = qosCar.SpeedLimitMode_DOWN;
		
		if(upCarMode == 0 && downCarMode == 0)
			return 1;
		
		break;
	}

	snprintf(buffer, sizeof(buffer), "qos_up_car_mode = %d; \n", upCarMode);
	req_format_write(wp, buffer);
	
	snprintf(buffer, sizeof(buffer), "qos_down_car_mode = %d; \n", downCarMode);
	req_format_write(wp, buffer);

	if(upCarMode == 1 || downCarMode == 1){		// iface
		for(index = OUT_BOUND; index <= IN_BOUND; index++){
			QOSCAR_IFACE_T qosCarIface;
			
			if(index == OUT_BOUND && upCarMode == 1){
				mib_num = MIB_QOS_CAR_UP_IFACE_TBL_NUM;
				mib_tbl = MIB_QOS_CAR_UP_IFACE_TBL;
			}
			else if(index == IN_BOUND && downCarMode == 1){
			...
			if(!apmib_get(mib_num, (void *)&carNum)){
				printf("get MIB_QOS_CAR_TBL_NUM %d error!", mib_num);
				return -1;
			}

			for(index2 = 1; index2 <= carNum; index2++){
				memset(&qosCarIface, '\0', sizeof(QOSCAR_IFACE_T));
				*((char *)&qosCarIface) = (char)index2;

				if((apmib_get(mib_tbl, (void *)&qosCarIface)) == 0){
					printf("get MIB_QOS_CAR_TBL %d error!", mib_tbl);
					return -1;
				}
#if 0
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
				if(qosCarIface.CarIfaceWanPort != wan_port || qosCarIface.CarIfaceCarId != 1)//currently every wan_port use 1st car tbl entry only
					continue;
#else
#endif
#endif
				if(qosCarIface.CarIfaceCarId != 1)	// temporarily use 1st car tbl entry only
					continue;
//#endif
				if(index == OUT_BOUND){
					snprintf(buffer, sizeof(buffer), "qos_up_car[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%s\", \"\");\n", 
						qosCarIface.CarIfaceInstanceNum-1, qosCarIface.CarIfaceInstanceNum, qosCarIface.CarIfaceCarId, qosCarIface.CarIfaceShaping, 
						qosCarIface.CarIfaceName);
				}
				else{
					snprintf(buffer, sizeof(buffer), "qos_down_car[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%s\", \"\");\n", 
						qosCarIface.CarIfaceInstanceNum-1, qosCarIface.CarIfaceInstanceNum, qosCarIface.CarIfaceCarId, qosCarIface.CarIfaceShaping, 
						qosCarIface.CarIfaceName);
				}
				//printf("[%s:%d] buffer: %s\n", __FUNCTION__, __LINE__, buffer);
	...
	if(upCarMode == 2 || downCarMode == 2){		// iface
		for(index = OUT_BOUND; index <= IN_BOUND; index++){
			QOSCAR_VLAN_T qosCarVlan;
			
			if(index == OUT_BOUND && upCarMode == 2){
				mib_num = MIB_QOS_CAR_UP_VLAN_TBL_NUM;
				mib_tbl = MIB_QOS_CAR_UP_VLAN_TBL;
			}
			else if(index == IN_BOUND && downCarMode == 2){
			...
			for(index2 = 1; index2 <= carNum; index2++){
				memset(&qosCarVlan, '\0', sizeof(QOSCAR_VLAN_T));
				*((char *)&qosCarVlan) = (char)index2;

				if((apmib_get(mib_tbl, (void *)&qosCarVlan)) == 0){
					printf("get MIB_QOS_CAR_TBL %d error!", mib_tbl);
					return -1;
				}
#if 0
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
				if(qosCarVlan.CarVlanWanPort != wan_port || qosCarVlan.CarVlanCarId != 1)
					continue;
#else
#endif
#endif
				if(qosCarVlan.CarVlanCarId != 1)	// temporarily use 1st car tbl only
					continue;
//#endif

				if(index == OUT_BOUND){
					snprintf(buffer, sizeof(buffer), "qos_up_car[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%d\", \"\");\n", 
						qosCarVlan.CarVlanInstanceNum-1, qosCarVlan.CarVlanInstanceNum, qosCarVlan.CarVlanCarId, qosCarVlan.CarVlanShaping, 
						qosCarVlan.CarVlanid);
				}
				else{
					snprintf(buffer, sizeof(buffer), "qos_down_car[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%d\", \"\");\n", 
					qosCarVlan.CarVlanInstanceNum-1, qosCarVlan.CarVlanInstanceNum, qosCarVlan.CarVlanCarId, qosCarVlan.CarVlanShaping, 
					qosCarVlan.CarVlanid);
				}
	...
	if(upCarMode == 3 || downCarMode == 3){		// ip range
		for(index = OUT_BOUND; index <= IN_BOUND; index++){
			QOSCAR_IP_T qosCarIp;
			
			if(index == OUT_BOUND && upCarMode == 3){
				mib_num = MIB_QOS_CAR_UP_IP_TBL_NUM;
				mib_tbl = MIB_QOS_CAR_UP_IP_TBL;
			}
			else if(index == IN_BOUND && downCarMode == 3){
			...
			for(index2 = 1; index2 <= carNum; index2++){
				...
				memset(&qosCarIp, '\0', sizeof(QOSCAR_IP_T));
				*((char *)&qosCarIp) = (char)index2;

				if((apmib_get(mib_tbl, (void *)&qosCarIp)) == 0){
					printf("get MIB_QOS_CAR_TBL %d error!", mib_tbl);
					return -1;
				}
#if 0
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
			if(qosCarIp.CarIpWanPort != wan_port || qosCarIp.CarIpCarId != 1)
				continue;
#else
#endif
#endif

				if(qosCarIp.CarIpCarId != 1)	// temporarily use 1st car tbl only
					continue;
//#endif

				ip_s = inet_ntoa(*((struct in_addr *)qosCarIp.CarIpStart));
				snprintf(str_ip_s, sizeof(str_ip_s), "%s", ip_s);
				
				ip_e = inet_ntoa(*((struct in_addr *)qosCarIp.CarIpEnd));
				snprintf(str_ip_e, sizeof(str_ip_e), "%s", ip_e);

				if(index == OUT_BOUND){
					snprintf(buffer, sizeof(buffer), "qos_up_car[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%s\", \"%s\");\n", 
						 qosCarIp.CarIpInstanceNum-1, qosCarIp.CarIpInstanceNum, qosCarIp.CarIpCarId, qosCarIp.CarIpShaping, str_ip_s, str_ip_e);
				}
				else{
					snprintf(buffer, sizeof(buffer), "qos_down_car[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%s\", \"%s\");\n", 
						 qosCarIp.CarIpInstanceNum-1, qosCarIp.CarIpInstanceNum, qosCarIp.CarIpCarId, qosCarIp.CarIpShaping, str_ip_s, str_ip_e);
				}
				//printf("[%s:%d] buffer: %s\n", __FUNCTION__, __LINE__, buffer);
				req_format_write(wp, buffer);
...
#if defined(CONFIG_MAC_SHAPING_QOS)
	if(upCarMode == 4 || downCarMode == 4){		// mac
		for(index = OUT_BOUND; index <= IN_BOUND; index++){
			QOSCAR_MAC_T qosCarMac;
			
			if(index == OUT_BOUND && upCarMode == 4){
				mib_num = MIB_QOS_CAR_UP_MAC_TBL_NUM;
				mib_tbl = MIB_QOS_CAR_UP_MAC_TBL;
			}
			else if(index == IN_BOUND && downCarMode == 4){
			...
			for(index2 = 1; index2 <= carNum; index2++){
				memset(&qosCarMac, '\0', sizeof(QOSCAR_MAC_T));
				*((char *)&qosCarMac) = (char)index2;

				if((apmib_get(mib_tbl, (void *)&qosCarMac)) == 0){
					printf("get MIB_QOS_CAR_TBL %d error!", mib_tbl);
					return -1;
				}
#if 0
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
				if(qosCarMac.CarMacWanPort != wan_port || qosCarMac.CarMacCarId != 1)
					continue;
#else
#endif
#endif

				if(qosCarMac.CarMacCarId != 1)	// temporarily use 1st car tbl only
					continue;
//#endif

				if(index == OUT_BOUND){
					snprintf(buffer, sizeof(buffer), "qos_up_car[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%02x:%02x:%02x:%02x:%02x:%02x\");\n", 
						qosCarMac.CarMacInstanceNum-1, qosCarMac.CarMacInstanceNum, qosCarMac.CarMacCarId, qosCarMac.CarMacShaping, 
						qosCarMac.CarMacAddr[0], qosCarMac.CarMacAddr[1], qosCarMac.CarMacAddr[2], 
						qosCarMac.CarMacAddr[3], qosCarMac.CarMacAddr[4], qosCarMac.CarMacAddr[5]);
				}
				else{
					snprintf(buffer, sizeof(buffer), "qos_down_car[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%02x:%02x:%02x:%02x:%02x:%02x\");\n", 
						qosCarMac.CarMacInstanceNum-1, qosCarMac.CarMacInstanceNum, qosCarMac.CarMacCarId, qosCarMac.CarMacShaping, 
						qosCarMac.CarMacAddr[0], qosCarMac.CarMacAddr[1], qosCarMac.CarMacAddr[2], 
						qosCarMac.CarMacAddr[3], qosCarMac.CarMacAddr[4], qosCarMac.CarMacAddr[5]);
				}
...
#elif defined(MULTI_WAN_QOS)
int getQoSWanTraffic(request *wp, int argc, char **argv)
{
	int wan_num = WANIFACE_NUM, wan_index = 0, v4Flag = 0, v6Flag = 0;
	WANIFACE_T WanIfaceEntry;
	char buffer[128] = {0};

	for(wan_index = 1; wan_index <= wan_num; wan_index++){
		memset(&WanIfaceEntry,0,sizeof(WANIFACE_T));
		*((char *)&WanIfaceEntry) = (char)wan_index;
		if(!apmib_get(MIB_WANIFACE_TBL, (void *)&WanIfaceEntry)){
			printf("%s.%d.get MIB_WANIFACE_TBL fail\n",__FUNCTION__,__LINE__);
		}
	
	#ifdef CONFIG_RTL_HW_QOS_SUPPORT
		v4Flag = WanIfaceEntry.enable;
	#else
		if((WanIfaceEntry.AddressType !=  BRIDGE) && WanIfaceEntry.enable)	/* not support sw bridge qos */
			v4Flag = 1;
		else
			v4Flag = 0;
	#endif
		...
		#if defined(CONFIG_RTL_HW_QOS_SUPPORT) && (defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F))
			v6Flag = WanIfaceEntry.ipv6Enable;
		#else
			if((WanIfaceEntry.ipv6Origin !=IPV6_ORIGIN_BRIDGE) && WanIfaceEntry.ipv6Enable)		/* not support sw bridge qos */
				v6Flag = 1;
			else
				v6Flag = 0;
		#endif
	
		snprintf(buffer, sizeof(buffer), "wan_qos_traffic[%d] = new Array(\"WAN%d\", \"%d\", \"%d\", \"%d\",\"%d\");\n", 
			wan_index-1, wan_index, v4Flag, WanIfaceEntry.ipv6Enable, WanIfaceEntry.wanQosEnable, WanIfaceEntry.wanQosV6Enable);
	#else
		snprintf(buffer, sizeof(buffer), "wan_qos_traffic[%d] = new Array(\"WAN%d\", \"%d\", \"0\", \"%d\", \"0\");\n", 
			wan_index-1, wan_index, v4Flag, WanIfaceEntry.wanQosEnable);
	#endif
		req_format_write(wp, buffer);


		snprintf(buffer, sizeof(buffer), "wan_bandwidth_array[%d] = new Array(\"%d\", \"%d\", \"%d\", \"%d\");\n", 
			wan_index-1, WanIfaceEntry.qosAutoUpLinkSpeed, WanIfaceEntry.qosManualUpLinkSpeed, WanIfaceEntry.qosAutoDownLinkSpeed, WanIfaceEntry.qosManualDownLinkSpeed);
		req_format_write(wp, buffer);
	}
...
int getQoSWanPolicy(request *wp, int argc, char **argv)
{
	int wan_num = WANIFACE_NUM, wan_index = 0, flag = 0;
	WANIFACE_T WanIfaceEntry;
	char buffer[128] = {0};
	...
	for(wan_index = 1; wan_index <= wan_num; wan_index++){
		...
		*((char *)&WanIfaceEntry) = (char)wan_index;
		...
		snprintf(buffer, sizeof(buffer), "wan_qos_policy[%d] = new Array(\"WAN%d\", \"%d\", \"%d\");\n", 
			wan_index-1, wan_index, WanIfaceEntry.enable && WanIfaceEntry.wanQosEnable, WanIfaceEntry.AddressType==BRIDGE ? 1 : 0);
...
#ifdef CONFIG_APP_MROUTED
if ( !strcmp(name, "mrouted_on")) {
 		if ( !apmib_get( MIB_MROUTED_ENABLE, (void *)&val) )
			return -1;
		sprintf(buffer, "%u", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "mroutedIfNum")) {
 		if ( !apmib_get( MIB_MROUTED_IF_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%u", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "mroutedTunnelNum")) {
 		if ( !apmib_get( MIB_MROUTED_TUNNEL_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%u", (int)val);
		req_format_write(wp, buffer);
		return 0;
	}
if ( !strcmp(name, "mroutedAltnetNum")) {
 		if ( !apmib_get( MIB_MROUTED_IF_ALTNET_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%u", (int)val);
		req_format_write(wp, buffer);
		return 0;
...
#ifdef CONFIG_APP_PIMD
if ( !strcmp(name, "pimd_on")) {
 		if ( !apmib_get( MIB_PIMD_ENABLE, (void *)&val) )
			return -1;
		sprintf(buffer, "%u", (int)val);
		req_format_write(wp, buffer);
		return 0;
...
if ( !strcmp(name, "bsr_candidate_priority")) {
 		if ( !apmib_get( MIB_PIMD_BSR_PRIORITY, (void *)&val) )
			return -1;
		if((int)val==256)
			req_format_write(wp, "");
		else{
...
if ( !strcmp(name, "rp_candidate_priority")) {
 		if ( !apmib_get( MIB_PIMD_RP_PRIORITY, (void *)&val) )
			return -1;
		if((int)val==256)
			req_format_write(wp, "");
		else{
...
if ( !strcmp(name, "rp_candidate_time")) {
 		if ( !apmib_get( MIB_PIMD_RP_TIME, (void *)&val) )
			return -1;
		if((int)val==0)
			req_format_write(wp, "");
		else{
...
if ( !strcmp(name, "pimd_ip_address")) {
 		if ( !apmib_get( MIB_PIMD_STATIC_RP_IP_ADDRESS, (void *)buffer) )
			return -1;
		if((int)buffer[0]==0)
			return req_format_write(wp,"");
		else
...
if ( !strcmp(name, "pimd_group_address")) {
 		if ( !apmib_get( MIB_PIMD_STATIC_RP_GROUP_ADDRESS, (void *)buffer) )
			return -1;
		if((int)buffer[0]==0)
			return req_format_write(wp,"");
		else
...
if ( !strcmp(name, "pimd_group_mask_len")) {
 		if ( !apmib_get( MIB_PIMD_STATIC_RP_GROUP_MASK_LEN, (void *)&val) )
			return -1;
		if((int)val==0)
			req_format_write(wp,"");
		else
...
if ( !strcmp(name, "PimdRpGroupNum")) {
 		if ( !apmib_get( MIB_PIMD_RP_GROUP_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%u", (int)val);
		req_format_write(wp, buffer);
		return 0;
...
if ( !strcmp(name, "PimdIfNum")) {
 		if ( !apmib_get( MIB_PIMD_IF_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%u", (int)val);
		req_format_write(wp, buffer);
		return 0;
...
if ( !strcmp(name, "isis_level")) {
	...
	if ( !strcmp(name, "l2tp_algonoff")) {
 		//if ( !apmib_get( MIB_ALG_L2TP_DISABLE, (void *)&val) )
			//return -1;
	...
	if ( !strcmp(name, "ipsec_algonoff")) {
 		//if ( !apmib_get( MIB_ALG_IPSEC_DISABLE, (void *)&val) )
			//return -1;
	...
	if ( !strcmp(name, "h323_algonoff")) {
 		//if ( !apmib_get( MIB_ALG_H323_DISABLE, (void *)&val) )
		//	return -1;
	...
	if ( !strcmp(name, "rtsp_algonoff")) {
 		//if ( !apmib_get( MIB_ALG_RTSP_DISABLE, (void *)&val) )
		//	return -1;
	...
	if ( !strcmp(name, "ftp_algonoff")) {
 		//if ( !apmib_get( MIB_ALG_FTP_DISABLE, (void *)&val) )
			//return -1;
	...
	if ( !strcmp(name, "pptp_algonoff")) {
 		//if ( !apmib_get( MIB_ALG_PPTP_DISABLE, (void *)&val) )
		//	return -1;
	...
	if ( !strcmp(name, "igmp_snoop_enabled")) {
 		//if ( !apmib_get( MIB_ALG_PPTP_DISABLE, (void *)&val) )
		//	return -1;

		bool enable;
		if(rtk_multicast_get_snoopy_enable(&enable) == 0)
			val=enable;
		else
			val=2;
	...
	if ( !strcmp(name, "igmp_proxy_enabled")) {
 		//if ( !apmib_get( MIB_ALG_PPTP_DISABLE, (void *)&val) )
		//	return -1;

		bool enable;
		if(rtk_multicast_get_proxy_enable(&enable) == 0)
			val=enable;
		else
			val=0;
	...
	if ( !strcmp(name, "mld_snoop_enabled")) {
 		//if ( !apmib_get( MIB_ALG_PPTP_DISABLE, (void *)&val) )
		//	return -1;

		bool enable;
		if(rtk_multicast_get_mld_snoopy_enable(&enable) == 0)
			val=enable;
		else
			val=2;
	...
	if ( !strcmp(name, "mld_proxy_enabled")) {
 		//if ( !apmib_get( MIB_ALG_PPTP_DISABLE, (void *)&val) )
		//	return -1;
		#ifdef  CONFIG_IPV6
		bool enable;
		if(rtk_multicast_get_mld_proxy_enable(&enable) == 0)
			val=enable;
		else
			val=0;
		#else
	...
	else if(!strcmp(name,"multiWanQosSupport")){
		#ifdef MULTI_WAN_QOS
			sprintf(buffer, "%d",1);
		#else
			sprintf(buffer, "%d",0);	
	...
	else if(!strcmp(name, "l2tpGetServIpByDomainName"))
	{
...
#if defined(CONFIG_CMCC_QOS)
	else if(!strcmp(name,"ispMultiPhyWan")){
		#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
		req_format_write(wp, "1");
		#else
		req_format_write(wp, "0");
...
#if defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
	else if(!strcmp(name,"qosWan")){
		if(!apmib_get(MIB_QOS_WAN_INDEX,(void *)&val))
			return -1;
...
#if 0
	else if(!strcmp(name,"qosShapingWanPort")){
		if(!apmib_get(MIB_QOS_SHAPING_WAN_PORT,(void *)&val))
			return -1;
...
#endif
	else if(!strcmp(name,"wan_port_numer")){
		sprintf(buffer,"%d",MAX_QOS_WANPORT_NUM);
		req_format_write(wp,buffer);
		return 0;
	}
	else if(!strcmp(name, "car_number") || !strcmp(name, "queue_number")){
		sprintf(buffer, "%d", MAX_PER_PORT_QOS_QUEUE_NUM);
		req_format_write(wp, buffer);
		return 0;
	}
	else if(!strcmp(name, "max_rate")){
	#if defined (CONFIG_RTL_8198)|| defined (CONFIG_RTL_8198C) || defined (CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_83XX_SUPPORT)
		sprintf(buffer, "%d", 1024000);
	#else
		sprintf(buffer, "%d", 102400);
	#endif
		req_format_write(wp, buffer);
		return 0;
	}
...
#else
	else if (!strcmp(name, "qosEnable")){
		if (!apmib_get(MIB_QOS_ENABLE, (void *)&val))
			return -1;
		...
		sprintf(buffer, "%d", MAX_QOS_QUEUE_NUM);
		req_format_write(wp, buffer);
		return 0;
	}
	else if(!strcmp(name, "max_rate")){
	...
	else if(!strcmp(name, "up_shaping")){
		if (!apmib_get(MIB_QOS_UPLINK_SPEED, (void *)&val))
			return -1;
	...
	else if(!strcmp(name, "qos_policy")){
		if (!apmib_get(MIB_QOS_PLAN, (void *)&val))
			return -1;
	...
	else if(!strcmp(name, "en_force_weight")){
		if (!apmib_get(MIB_QOS_ENFORCE_WEIGHT, (void *)&val))
			return -1;
	...
	else if(!strcmp(name, "dscp_remark")){
		if (!apmib_get(MIB_QOS_EN_DSCP_REAMRK, (void *)&val))
			return -1;
	...
	else if(!strcmp(name, "dot1p_remark")){
		if (!apmib_get(MIB_QOS_EN_DOT1P_REMARK, (void *)&val))
			return -1;
...
#endif
	else if(!strcmp(name, "down_shaping")){
		if (!apmib_get(MIB_QOS_DOWNLINK_SPEED, (void *)&val))
			return -1;
	...
	else if (!strcmp(name, "class_number")){
		if (!apmib_get( MIB_QOS_CLASS_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
	}
	else if (!strcmp(name, "type_number")){
		sprintf(buffer, "%d", MAX_QOS_TYPE_NUM);
		req_format_write(wp, buffer);
		return 0;
	}
	else if (!strcmp(name, "current_classid")){
		if (!apmib_get( MIB_CURRENT_CLASSID, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
	}
	else if (!strcmp(name, "current_typeid")){
		if (!apmib_get(MIB_CURRENT_TYPEID, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
	}
	else if (!strcmp(name, "uplink_qos_operation")){
		if (!apmib_get(MIB_UPLINK_QOS_OPERATION, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
	}
	else if (!strcmp(name, "current_car_up_id")){
		if (!apmib_get(MIB_CURRENT_CARID, (void *)&val))
			return -1;
		sprintf(buffer, "%d", ((val >> 4) & 0xF));
		req_format_write(wp, buffer);
		return 0;
	}
	else if (!strcmp(name, "current_car_down_id")){
		if (!apmib_get(MIB_CURRENT_CARID, (void *)&val))
			return -1;
		sprintf(buffer, "%d", (val & 0xF));
		req_format_write(wp, buffer);
		return 0;
	}
	else if (!strcmp(name, "up_shaping_operation")){
		if (!apmib_get(MIB_SHAPING_QOS_OEPRATION, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", ((val >> 4) & 0xF));
		req_format_write(wp, buffer);
		return 0;
	}
	else if (!strcmp(name, "down_shaping_operation")){
		if (!apmib_get(MIB_SHAPING_QOS_OEPRATION, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", (val & 0xF));
		req_format_write(wp, buffer);
	...
	else if(!strcmp(name, "mac_shaping")){
	#if defined(CONFIG_MAC_SHAPING_QOS)
		sprintf(buffer, "%d", 1);
	#else
		sprintf(buffer, "%d", 0);
	...
	else if(!strcmp(name, "defQueue")){
		unsigned int defQueue;
		if ( !apmib_get( MIB_QOS_DEF_QUEUE, (void *)&defQueue) )
			return -1;

		sprintf(buffer, "%u", defQueue);
		req_format_write(wp, buffer);
		return 0;
	}
	else if(!strcmp(name, "mainWanIndex")){
	#if defined(MAIN_WAN_MAPPING)
		val = get_main_wan_index();
		sprintf(buffer, "%d", val);	
	#else
		sprintf(buffer, "%d", 1);
	#endif
...
#if 0
	else if ( !strcmp(name, "wanQosEnable")) {
		WANIFACE_T WanIfaceEntry;
		int wan_index = 0 ;
		...
		sprintf(buffer, "%d",WanIfaceEntry.wanQosEnable & WanIfaceEntry.enable);
		req_format_write(wp, buffer);
		return 0;
	}
#endif
#if 0 //defined(CONFIG_IPV6)
	else if ( !strcmp(name, "wanQosV6Enable")) {
		WANIFACE_T WanIfaceEntry;
		int wan_index = 0 ;
		...
		sprintf(buffer, "%d",WanIfaceEntry.wanQosV6Enable & WanIfaceEntry.ipv6Enable);
		req_format_write(wp, buffer);
		return 0;
	}
#endif
#if 0
	else if ( !strcmp(name, "wanBrSupport")) {
		WANIFACE_T WanIfaceEntry;
		int wan_index = 0 ;
		...
		if(WanIfaceEntry.qosAutoUpLinkSpeed == 1){
				...
				sprintf(buffer, "%d", 1000000);
			#else
				sprintf(buffer, "%d", 100000);
			#endif
		}
		else{
			sprintf(buffer, "%d", WanIfaceEntry.qosManualUpLinkSpeed);
		}
		
		req_format_write(wp, buffer);
		return 0;	
	}
	else if ( !strcmp(name, "wanDownRate")) {
		WANIFACE_T WanIfaceEntry;
		int wan_index = 0 ;
		...
		if(WanIfaceEntry.qosAutoDownLinkSpeed == 1){
			...
			sprintf(buffer, "%d", WanIfaceEntry.qosManualDownLinkSpeed);
		}
		
		req_format_write(wp, buffer);
		return 0;	
	}
	else if ( !strcmp(name, "queueType")) {
		int wan_index = 0, queue_index = 0, direct = 0, flag = 0;
		int index = 0, entryNum = 0;
		QOSQUEUE_T queueEntry;
		WANIFACE_T WanIfaceEntry;
		char wan_dev[16] = {0};
		...
		if(apmib_get(MIB_QOS_QUEUE_TBL_NUM, (void *)&entryNum)){
			if(entryNum){
				for(index = 1; index<=entryNum; index++){
					...
					*((char *)&queueEntry) = (char)index;
					
					if (apmib_get(MIB_QOS_QUEUE_TBL, (void *)&queueEntry)){
						if(queueEntry.QueueInstanceNum == queue_index){
							sprintf(wan_dev, "%s", queueEntry.QueueWanInterface);
							if(wan_dev[0]){
								wan_index = wan_dev[3] - '0';	// WAN3
								...
								if(WanIfaceEntry.AddressType == BRIDGE){	// v4 hw bridge
									flag |= 1;
								}
								if(WanIfaceEntry.AddressType == PPPOE){		// v4 pppoe
									flag |= 2;
								}
							#if defined(CONFIG_IPV6)
								if(WanIfaceEntry.ipv6Enable){	// v6 hw routing or bridge
									if((WanIfaceEntry.ipv6Origin == IPV6_ORIGIN_BRIDGE)
	...
	else if(!strcmp(name, "validQueExist")){
	...
	else if ( !strcmp(name, "queueEnable")) {
		QOSQUEUE_T queueEntry;
		int queue_index = 0, entryNum = 0, index = 0, direct = 0;
		...
		if(!strcmp(argv[2],"first"))
			queue_index = 1;
		if(!strcmp(argv[2],"second"))
			queue_index = 2;
		if(!strcmp(argv[2],"third"))
			queue_index = 3;
		if(!strcmp(argv[2],"forth"))
			queue_index = 4;
		if(!strcmp(argv[2],"fifth"))
			queue_index = 5;
		if(!strcmp(argv[2],"sixth"))
			queue_index = 6;
		if(!strcmp(argv[2],"seventh"))
			queue_index = 7;
		if(!strcmp(argv[2],"eighth"))
			queue_index = 8;
		...
		if(apmib_get(MIB_QOS_QUEUE_TBL_NUM, (void *)&entryNum)){
			if(entryNum){
				for(index = 1; index<=entryNum; index++){
					...
					*((char *)&queueEntry) = (char)index;
					if (apmib_get(MIB_QOS_QUEUE_TBL, (void *)&queueEntry)){
						if(queueEntry.QueueEnable && (queueEntry.QueueInstanceNum == queue_index)){
							sprintf(buffer, "%d",1);
							req_format_write(wp, buffer);
							return 0;
						}	
...
#if defined(CONFIG_IPV6)
	else if (!strcmp(name, "queueV6Enable")) {
		QOSQUEUE_T queueEntry;
		int queue_index = 0, entryNum = 0, index = 0, direct = 0;
		char wanIface[8] = {0};
				...
				for(index = 1; index<=entryNum; index++){
					...
					*((char *)&queueEntry) = (char)index;
					if (apmib_get(MIB_QOS_QUEUE_TBL, (void *)&queueEntry)){
						...
						if(queueEntry.QueueEnable && (queueEntry.QueueInstanceNum == queue_index)){
							snprintf(wanIface, sizeof(wanIface), "%s", queueEntry.QueueWanInterface);
							
							if(wanIface[0]){
								int wanV6Status = 0, wanV6QosStatus = 0;;
								int wanIndex = wanIface[3] - '0';

								if(!apmib_set( MIB_WANIFACE_CURRENT_IDX, (void *)&wanIndex)){
									printf("[%s:%d] set wan interface item index fail!", __FUNCTION__, __LINE__);
									break;
								}

								if(!apmib_get( MIB_WANIFACE_IPV6_ENABLE, (void *)&wanV6Status)){
									printf("[%s:%d] get wan interface ipv6 enable fail!", __FUNCTION__, __LINE__);
									break;
								}

								if(!apmib_get( MIB_WAN_QOS_V6_ENABLE, (void *)&wanV6QosStatus)){
									printf("[%s:%d] get wan interface qos ipv6 enable fail!", __FUNCTION__, __LINE__);
									break;
								}
								
								if((wanV6Status == 1) && (wanV6QosStatus == 1)){
									sprintf(buffer, "%d",1);
									req_format_write(wp, buffer);
									return 0;
								}
...
#endif
	else if ( !strcmp(name, "upQueIdle")) {		// queue1 - queue 8
		QOSQUEUE_T queueEntry;
		int queue_index = 0, entryNum = 0, index = 0;
		...
		if(!strcmp(argv[1],"forth"))
			queue_index = 4;
		if(!strcmp(argv[1],"fifth"))
			queue_index = 5;
		if(!strcmp(argv[1],"sixth"))
			queue_index = 6;
		if(!strcmp(argv[1],"seventh"))
			queue_index = 7;
		if(!strcmp(argv[1],"eighth"))
			queue_index = 8;
					...
					*((char *)&queueEntry) = (char)index;
					if (apmib_get(MIB_QOS_QUEUE_TBL, (void *)&queueEntry)){
						if(queueEntry.QueueInstanceNum == queue_index){
							...
							if(!strncmp(queueEntry.QueueInterface, "WAN", 3))
								sprintf(buffer, "%d",0);
	...
	else if ( !strcmp(name, "downQueIdle")) {	// queue11 - queue 18
		QOSQUEUE_T queueEntry;
		int queue_index = 0, entryNum = 0, index = 0;
					...
					*((char *)&queueEntry) = (char)index;
					if (apmib_get(MIB_QOS_QUEUE_TBL, (void *)&queueEntry)){
						if(queueEntry.QueueInstanceNum == queue_index){
							...
							if(!strncmp(queueEntry.QueueInterface, "LAN", 3))
								sprintf(buffer, "%d",0);
...
#endif
#if 0 //defined(CONFIG_IPV6)
	else if( !strcmp(name, "wanV6Enable")){
		int wan_index = 0;
		WANIFACE_T WanIfaceEntry;
...
#if defined(CONFIG_RTL_HW_QOS_SUPPORT) && (defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F))
		sprintf(buffer, "%d",WanIfaceEntry.ipv6Enable);
#else
		//if((WanIfaceEntry.AddressType !=  BRIDGE) && WanIfaceEntry.ipv6Enable)	
		if((WanIfaceEntry.ipv6Origin !=IPV6_ORIGIN_BRIDGE) && WanIfaceEntry.ipv6Enable)		/* not support sw bridge qos */
			sprintf(buffer, "%d", 1);
		else
			sprintf(buffer, "%d", 0);
	...
	{		
		...
		sprintf(buffer, "%d",WanIfaceEntry.qosAutoUpLinkSpeed);
		req_format_write(wp, buffer);
		return 0;
	}		
	...
	{		
		...
		sprintf(buffer, "%d",WanIfaceEntry.qosAutoDownLinkSpeed);
		req_format_write(wp, buffer);
		return 0;
	}
	else if(!strcmp(name,"qosManualUplinkSpeed"))
	...
	{		
	...
	else if ( !strcmp(name, "shapingNum")) { 

		WANIFACE_T WanIfaceEntry;
		int shapingNum = 0, index = 0;

		for(index = WAN1; index <= WANIFACE_NUM; index++)
		{
			memset(&WanIfaceEntry, '\0', sizeof(WanIfaceEntry));
			if(!getWanIfaceEntry(index,&WanIfaceEntry))
			{
				printf("%s.%d.get waniface entry fail\n",__FUNCTION__,__LINE__);
				return -1;
			}
		...
		sprintf(buffer, "%d",shapingNum);
		req_format_write(wp, buffer);
		return 0;
	}
	else if ( !strcmp(name, "queueNum")) { 
...
#endif
	else if ( !strcmp(name, "macFilterModeEnabled")) {
		#ifdef MACFILTER_MODE_SUPPORT
		val = 1;
		#else
		val = 0;
	...
	else if(!strcmp(name,"accessInternetUserLimit")){
		if ( !apmib_get( MIB_LIMIT_USER_ENABLE, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
	}
	else if(!strcmp(name,"accessInternetUserLimitNum")){
		if ( !apmib_get( MIB_LIMIT_USER_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
...
*/
	...
	else if ( !strcmp(name, "ipRangeMultiWanSupport_start")) {
#ifndef IP_RANGE_MULTI_WAN_SUPPORT
		req_format_write(wp, "<!--");
#else
		req_format_write(wp, "");
#endif
	}
	else if( !strcmp(name, "ipRangeMultiWanSupport_end")){
#ifndef IP_RANGE_MULTI_WAN_SUPPORT
		req_format_write(wp, "-->");
#else
		req_format_write(wp, "");
	...
	else if ( !strcmp(name, "wlanVxdDisabled")) {
#ifdef UNIVERSAL_REPEATER
		if(wlan_idx==0){
...
//#define IP_RANGE_MULTI_WAN_SUPPORT
	...
	else if ( !strcmp(name, "isCTCOMSupport")) {
#ifdef CONFIG_CMCC	// ct-com
...
#ifdef HOME_GATEWAY
#ifdef DOS_SUPPORT
	else if ( !strcmp(name, "DOS_firewall_level")) {
		if ( !apmib_get( MIB_DOS_FIREWALL_LEVEL, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
...
#ifdef CONFIG_IPV6
	else if ( !strcmp(name, "ipv6DosEnabled")) {
		if ( !apmib_get( MIB_IPV6_DOS_ENABLE, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
...
//for 11N
	...
	else if(!strcmp(name,"vlan_binding_support")){
	...
	else if(!strcmp(name,"access_user_limit")){
		#if defined(CONFIG_RTL_ACCESS_INTERNET_USER_NUM_LIMIT)
		req_format_write(wp, "1");
		#else
		req_format_write(wp, "0");
	...
	#if defined(CONFIG_MULTIPLE_WAN_VLAN_BINDING)
	else if(!strcmp(name,"vlanBindingNum")){
		if ( !apmib_get( MIB_VLANBINDING_TBL_NUM, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
	...
	else if ( !strcmp(name, "vlanBindingEnabled")) {
		if ( !apmib_get( MIB_VLANBINDING_ENABLED, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
	}
	else if ( !strcmp(name, "hwnatlanvid")) {
		if ( !apmib_get( MIB_VLANBINDING_HWNAT_LANVID, (void *)&val) )
			return -1;
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
...
//### end
	...
	else if(!strcmp(name,"hwV6QosSupport")){
		#if defined (CONFIG_RTL_HW_QOS_SUPPORT) && defined(CONFIG_RTL_HARDWARE_IPV6_SUPPORT)
			req_format_write(wp, "1");
		#else
			req_format_write(wp, "0");
		#endif
	}
	else if(!strcmp(name,"swQosSupport_start"))
	{
		#ifndef CONFIG_RTL_HW_QOS_SUPPORT
		req_format_write(wp, "");
		#else
	...
	else if(!strcmp(name,"swQosSupport_end"))
	{
		#ifndef CONFIG_RTL_HW_QOS_SUPPORT
		req_format_write(wp, "");
		#else
...
/*	else if ( !strcmp(name, "StorageAnonAccessFtpEnable")) {
	...
	else if(!strcmp(name,"get_ppp_auth_by_lan_support")){
#ifdef GET_PPP_AUTH_BY_LAN
		return req_format_write(wp, "%s", "1");
#else
		return req_format_write(wp, "%s", "0");
	...
	{
#ifdef SUPER_NAME_SUPPORT
		sprintf(buffer,"%d",wp->auth_flag);
		return req_format_write(wp, buffer);
#else
		return req_format_write(wp,"0");
#endif
	}
	else if( !strcmp(name, "MULTI_USER_ENABLED"))
	{
#ifdef SUPER_NAME_SUPPORT
		return req_format_write(wp, "1");
#else
	...
	else if((strstr(name,".htm") || (strstr(name,".asp")))){
#ifdef SUPER_NAME_SUPPORT
		val = getHtmlReadAuth(name, wp->auth_flag);
#else
		val = 1;
#endif
		sprintf(buffer, "%d", val);
		return req_format_write(wp, buffer);
	}else if(!strcmp(name,"opModeTarget")){
#if defined(SUPER_NAME_SUPPORT) && defined(CONFIG_TAIGUO_CTCOM_OP_MODE_CHECK)
		return req_format_write(wp, "%s", "_top");
#else
		return req_format_write(wp, "%s", "_self");
#endif
...
#else
		sprintf(buffer,"%s", "sub_menu_tcpip.htm");
#endif
...
#ifdef MULTI_WAN_SUPPORT
#ifdef CONFIG_WLAN_UI_SUPPORT
		sprintf(buffer,"%s", "wlbasic.htm");
#else
		sprintf(buffer,"%s", "tcpiplan.htm");
#endif
	
#else
		sprintf(buffer,"%s", "wizard.htm");
#endif
		req_format_write(wp, buffer);	
		return 0;
	}	
	else if(!strcmp(name,"channel_sum")){
#ifdef AUTO_CHANNEL_CUSTOMIZE
		apmib_get(MIB_WLAN_CHANNEL_SUM,(void *)&val);
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
#endif
	...
	else if(!strcmp(name,"is_main_wan_support")){
#ifdef MAIN_WAN_MAPPING
		sprintf(buffer,"%d",1);
#else
		sprintf(buffer,"%d",0);
    ...
    else if(!strcmp(name,"has_cluster_mgmt")){
    ...
    else if(!strcmp(name,"enabled_cluster_mgmt")){
#ifdef CONFIG_USER_CLUSTER_MANAGE
        unsigned char enabled = 0; 
        apmib_get(MIB_CLUSTER_MANAGE_ENABLE, (void *)&enabled);
        if(enabled==1)
            sprintf(buffer, "%d", 1);
        else
            sprintf(buffer, "%d", 0);
	...
	else if(!strcmp(name,"internet_wan_upnp_enabled")){
#ifdef MULTI_WAN_SUPPORT
		int i;
		WANIFACE_T entry;
		WANIFACE_Tp pEntry;
		...
		for(i = 1; i <= WANIFACE_NUM; i++){
			...
			if(!getWanIfaceEntry(i,pEntry))
				return;
			if(pEntry->enable && (pEntry->cmode != IP_BRIDGE)&&
				(pEntry->applicationtype == APPTYPE_INTERNET || pEntry->applicationtype == APPTYPE_TR069_INTERNET)&&
				(pEntry->enableUPNP)){
				sprintf(buffer, "%d", i);
				req_format_write(wp, buffer);
				return 0;	
		...
		return 0;	
#else
		apmib_get(MIB_UPNP_ENABLED,&val);
	...
	else if(!strcmp(name,"cluster_periodic_enabled")){
#ifdef CONFIG_USER_CLUSTER_MANAGE
        unsigned char enabled = 0; 
        apmib_get(MIB_CLUSTER_MANAGE_PERIODIC_ENABLE, (void *)&enabled);
        if(enabled==1)
            sprintf(buffer, "%d", 1);
        else
            sprintf(buffer, "%d", 0);
	...
	else if(!strcmp(name,"cluster_periodic_interval")){
#ifdef CONFIG_USER_CLUSTER_MANAGE
        unsigned int interval = 0; 
        apmib_get(MIB_CLUSTER_MANAGE_PERIODIC_INTERVAL, (void *)&interval);
		...
		int siteAccess=0, pid=-1;
		char buf[128];
		pid=find_pid_by_name("timelycheck");
		if(pid){
			snprintf(buffer, sizeof(buffer), "kill -s SIGUSR1 %d > /dev/null", pid);
			system(buffer);
			sleep(1);
	...
	else if(!strcmp(name,"wired_dhcp_client_num"))
	{
		...
		//dhcpWiredClientList(wp, argc, argv);
	...
	{
		int enabled;
		apmib_get(MIB_RTL_LINK_SYNC,(void *)&enabled);
		if(enabled==1)
            sprintf(buffer, "%d", 1);
        else
            sprintf(buffer, "%d", 0);
	...
	else if(!strcmp(name,"aWiFi_support")){
		val = 1;
		return req_format_write(wp, "%d", val);	
	}
	else if(!strcmp(name,"internet_wan_aWiFi_enabled")){
		apmib_get(MIB_AWIFI_ENABLED, (void *)&val);
		if(val){
		#ifdef MULTI_WAN_SUPPORT
			apmib_get(MIB_AWIFI_WAN_IDX, (void *)&val);
		#endif
			sprintf(buffer, "%d",val);
		}else
			sprintf(buffer, "%d",val);
	...
	else if(!strcmp(name,"aWiFiTrustedMacNum"))
	{ //aWiFi
		apmib_get(MIB_AWIFI_TRUSTED_MAC_TBL_NUM, (void *)&val);
		return req_format_write(wp, "%d", val);	
	}
	else if(!strcmp(name,"aWiFiTrustedUrlNum"))
	{ //aWiFi
		apmib_get(MIB_AWIFI_TRUSTED_URL_TBL_NUM, (void *)&val);
		return req_format_write(wp, "%d", val);	
	}
#ifdef	CONFIG_APP_AWIFI_AUDIT_SUPPORT
	else if(!strcmp(name,"aWiFi_audit_support")){
		val = 1;
		return req_format_write(wp, "%d", val);	
	}else if(!strcmp(name,"aWiFi_audit_type")){
		apmib_get(MIB_AWIFI_AUDIT_TYPE, (void *)&val);
		return req_format_write(wp, "%d", val);	
	}
#endif
#endif
	else if(!strcmp(name,"multi_wan_port_mode"))
	{
#ifdef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT
		sprintf(buffer, "%d", 1);
#else
	...
	else if(!strcmp(name,"multi_wan_port_hwNat")){
#ifdef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT
		if(check_hwNat_is_enabled()==1)
			val=1;
		else
	...
	else if(!strcmp(name,"multi_wan_port_flag")){
		int portId;
#ifdef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT
		char portFlag[MAX_PHY_PORT_NUM]={0};

		for(portId=1;portId<=MAX_PHY_PORT_NUM;portId++)
		{
			portFlag[portId-1]=check_wan_logic_port_available(portId);
		}
		
		sprintf(buffer, ("%d|%d|%d|%d|%d"), portFlag[0], portFlag[1], portFlag[2], portFlag[3],portFlag[4]);
#else
		sprintf(buffer, ("%d|%d|%d|%d|%d"), 0, 0, 0, 0, 0);
#endif

		req_format_write(wp, buffer);
	}
	else if(!strcmp(name,"multi_wan_mapping_lan_port")){
		int wanPortId;
#ifdef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT
		char lanPortFlag[MAX_PHY_PORT_NUM]={0};

		for(wanPortId=1;wanPortId<=MAX_PHY_PORT_NUM;wanPortId++)
		{
			lanPortFlag[wanPortId-1]=get_lan_dev_idx_from_logic_port(wanPortId);
		}
		sprintf(buffer, ("%d|%d|%d|%d|%d"), lanPortFlag[0], lanPortFlag[1], lanPortFlag[2], lanPortFlag[3],lanPortFlag[4]);
#else
		sprintf(buffer, ("%d|%d|%d|%d|%d"), 0, 0, 0, 0, 0);
#endif
		req_format_write(wp, buffer);
	}
	else if(!strcmp(name,"multi_wan_check_lan_port_used")){
		int lanPortId;
#ifdef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT
		char lanDevFlag[MAX_PHY_PORT_NUM]={0};
		const char* LANDEVNAME[] = {"eth0", "eth2", "eth3", "eth4", "eth5"};

		for(lanPortId=1;lanPortId<=MAX_PHY_PORT_NUM;lanPortId++)
		{
			lanDevFlag[lanPortId-1]=check_lan_dev_exist(LANDEVNAME[lanPortId-1]);
		}
		sprintf(buffer, ("%d|%d|%d|%d|%d"), lanDevFlag[0], lanDevFlag[1], lanDevFlag[2], lanDevFlag[3],lanDevFlag[4]);
#else
		sprintf(buffer, ("%d|%d|%d|%d|%d"), 0, 0, 0, 0, 0);
#endif
		req_format_write(wp, buffer);
...
int getDHCPModeCombobox(request *wp, int argc, char **argv)
{
...
#elif defined(CONFIG_APP_DHCPRELAY)
	if(lan_dhcp_mode == 0){
 		return req_format_write(wp,"<option selected value=\"0\"><script>dw(tcpip_lan_dhcp_disabled)</script></option>"
 							"<option value=\"1\"><script>dw(tcpip_lan_dhcp_client)</script></option>"
 							 "<option value=\"2\"><script>dw(tcpip_lan_dhcp_server)</script></option>"
 							 "<option value=\"3\"><script>dw(tcpip_lan_dhcp_relay)</script></option>"
 							 );
      	  }
	if(lan_dhcp_mode == 1){
 		return req_format_write(wp,"<option  value=\"0\"><script>dw(tcpip_lan_dhcp_disabled)</script></option>"
	...
	if(lan_dhcp_mode == DHCP_LAN_RELAY){
 		return req_format_write(wp,"<option  value=\"0\"><script>dw(tcpip_lan_dhcp_disabled)</script></option>"
 							"<option  value=\"1\"><script>dw(tcpip_lan_dhcp_client)</script></option>"
 							 "<option  value=\"2\"><script>dw(tcpip_lan_dhcp_server)</script></option>"
 							 "<option selected value=\"3\"><script>dw(tcpip_lan_dhcp_relay)</script></option>"
 							 );
      	  }
#else
 	if(lan_dhcp_mode == 0){
