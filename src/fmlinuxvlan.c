...
#define DEFAULT_RESERVE_PASSTHROUGH_VID 100
...
void formVlan(request *wp, char *path, char *query)
{
	...
	if(strTmp[0])
	{
		...
		if(vlan_id==DEFAULT_RESERVE_PASSTHROUGH_VID || vlan_id==DEFAULT_NAT_LAN_VID || vlan_id==DEFAULT_NAT_WAN_VID)
		{
			sprintf(errBuf, "VLAN id %d , %d and %d are for internal use!",
				DEFAULT_NAT_WAN_VID, DEFAULT_NAT_LAN_VID,DEFAULT_RESERVE_PASSTHROUGH_VID);
			goto setErr;
		}		
