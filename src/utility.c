...
int getWlAuthResult(char *interface, int *pStatus)
{
    ...
    if (iw_get_ext(skfd, interface, SIOCGIWRTLAUTH, &wrq) < 0){
    	close( skfd );
	return -1;
	}
    close( skfd );
...
#endif
	if (entry->ServiceList&X_CT_SRV_IPTV)
...
int getMultiWanApplicationType(unsigned int ServiceList,char* bufferServerType,char lenth)
{
	...
	switch(ServiceList)
	{
		...
		case X_CT_SRV_IPTV:
			...
			break;
		case X_CT_SRV_TR069_BRIDGE:
			strcpy(bufferServerType,"TR069_Bridge");
			break;
		case X_CT_SRV_TR069_INTERNET:
...
int set_reinit_flag(int flag)
{
	int flag_value=0;
	apmib_get(MIB_MODULES_REINIT_FLAG,(void*)&flag_value);
	flag_value|=flag;
	apmib_set(MIB_MODULES_REINIT_FLAG,(void*)&flag_value);
	return 0;
}
#endif
#ifdef CONFIG_CMCC
...
#ifdef SUPER_NAME_SUPPORT
int getHtmlReadAuth(char *name, int login_level){
	int auth_mode;
	
#ifdef CONFIG_TAIGUO_CTCOM_OP_MODE_CHECK
	int op_mode;
	if(!apmib_get(MIB_OP_MODE, (void *)&op_mode)){
		printf("get MIB_OP_MODE fail\n");
		return 1;
	}

	auth_mode = getHtmlAuth(name, login_level, op_mode);
#else
	auth_mode = getHtmlAuth(name, login_level);
#endif
...
int getHtmlWriteAuth(char *reference, int login_level){		
	char *p = NULL;
	char buf[100]={0};
	int auth_mode;
	...
	{	
		...
		auth_mode = getHtmlAuth(buf, login_level, op_mode);
#else
		auth_mode = getHtmlAuth(buf, login_level);
#endif
		if(auth_mode == AUTH_WRITE)
			return 1;
		else
			return 0;
	}else
