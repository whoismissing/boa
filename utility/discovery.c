...
u_int32_t km_auto_switch_yiaddr = 0;
...
int creat_pppoe_untagged_wan_iface()
{	
	system("echo \"add eth1 pppoe napt\" > /proc/wan_vlan");
	system("echo \"vlan add eth1 8 untagged\" > /proc/wan_vlan");
	return 0;
}
int clean_pppoe_untaged_wan_iface()
{
	system("echo \"wan-mapping flush eth1\" > /proc/wan_vlan");
	system("echo \"vlan flush eth1\" > /proc/wan_vlan");
	system("echo \"remove eth1\" > /proc/wan_vlan");
	return 0;
}

int recover_eth1_wan()
{
	char cmdBuf[256]={0};
	FILE *fp=NULL;
	if((fp=fopen("/var/eth1_wan", "r"))==NULL)
		return 0;
...
int  discovery_ppp(char *wan_if)
{
	int i, ses;
	int ret = -1;
	int create_flag=0, clean_flag=0;
	
#ifndef CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT	
#ifdef MULTI_WAN_SUPPORT
	if(access("/var/wan1_info",0)!=0)
	{
		creat_pppoe_untagged_wan_iface();
		create_flag=1;
	}
#endif
#endif
...
int waitForOffer(int sock_fd, int timeout, unsigned char *myEth)
{
    ...
    km_auto_switch_yiaddr = dhcpPkt.data.yiaddr;
		return 1;
	}
...
int sendDiscovery(int sock_fd, struct dhcpPacket *dhcpPkt, unsigned char *myEth, struct sockaddr_ll *dest)
{
	...
	while (1)
	{
		...
		//printf("\n%s:%d##padiAttempts=%d\n",__FUNCTION__,__LINE__,padiAttempts);
