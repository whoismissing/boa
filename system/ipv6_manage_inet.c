...
void set_radvdwanconnect(RA_INFO_ITEM_Tp ra_data)
{
		char ipv6dnsstr[256];
		char tmpdnsstr[64];
		char ipv6str[512];
		int i=0,dnsupdate=0,prefixupdate=0;
		if(ra_data->slaacFail == 0)
		{
			if(memcmp(&last_recv_data.prefix,&ra_data->prefix,sizeof(struct in6_addr)))
			{
				prefixupdate=1;
				memcpy(&last_recv_data.prefix,&ra_data->prefix,sizeof(struct in6_addr));
		...
		{
			prefixupdate=0;
			memset(&ra_data->prefix,0,sizeof(struct in6_addr));
		}
#ifdef	CONFIG_IPV6_RA_RDNSS_SUPPORT 
		for(i=0;i<(ra_data->dnsinfo.length-1)/2;i++)
		{
			sprintf(tmpdnsstr,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x	 ",
			ra_data->dnsinfo.in6addr_dns[i].s6_addr16[0],ra_data->dnsinfo.in6addr_dns[i].s6_addr16[1],ra_data->dnsinfo.in6addr_dns[i].s6_addr16[2],ra_data->dnsinfo.in6addr_dns[i].s6_addr16[3],
			ra_data->dnsinfo.in6addr_dns[i].s6_addr16[4],ra_data->dnsinfo.in6addr_dns[i].s6_addr16[5],ra_data->dnsinfo.in6addr_dns[i].s6_addr16[6],ra_data->dnsinfo.in6addr_dns[i].s6_addr16[7]);
			strcat(ipv6dnsstr,tmpdnsstr);
			dnsupdate = dnsupdate || memcmp(&last_recv_data.dnsinfo.in6addr_dns[i],&ra_data->dnsinfo.in6addr_dns[i],sizeof(struct in6_addr));
		}
		if(dnsupdate)
		{
			memcpy(&last_recv_data.dnsinfo,&ra_data->dnsinfo,sizeof(struct rdnss_info));
		}
#endif
		sprintf(ipv6str,"sysconf ipv6_radvd ra_dns=%s",ipv6dnsstr);
		if(prefixupdate || dnsupdate )
		{
			system(ipv6str);
		}
		memset(tmpdnsstr,0,sizeof(tmpdnsstr));
		memset(ipv6dnsstr,0,sizeof(ipv6dnsstr));
		memset(ipv6str,0,sizeof(ipv6str));
		dnsupdate=0;prefixupdate=0;
		if(last_recv_data.icmp6_managed!=ra_data->icmp6_managed
		|| last_recv_data.icmp6_other!=ra_data->icmp6_other)
		{
			restart_dhcp6c(ra_data);
			last_recv_data.icmp6_managed=ra_data->icmp6_managed;
			last_recv_data.icmp6_other=ra_data->icmp6_other;
		}
		return 0;
}
...
int handle_ra(struct msghdr * pMsg)
{
	...
	if(recv_data.lifetime != 0)
	{
		if(access(RECV_RA_INVALID,0)==0)
			system("rm "RECV_RA_INVALID);
	}
	if(last_recv_data.icmp6_managed==recv_data.icmp6_managed
		&& last_recv_data.icmp6_other==recv_data.icmp6_other
		&& last_recv_data.slaacFail==recv_data.slaacFail
		&& (last_recv_data.lifetime == recv_data.lifetime || recv_data.lifetime != 0))
		{//the same as last, do nothing
			return 0;
		}
	if(last_recv_data.lifetime != recv_data.lifetime && recv_data.lifetime == 0)
	{
		system("echo \"update\" > "RECV_RA_INVALID);
		if(access(DHCP6PD_CONF_FILE,0)==0)
		{
			system("killall radvd 2> /dev/null");
			system("rm -f /var/run/radvd.pid 2> /dev/null");
			return 0;
	...
	return 0;
#endif
	if(recv_data.lifetime)
	{
		set_radvdwanconnect(&recv_data);
	}
	return 0;
}

void restart_dhcp6c(RA_INFO_ITEM_Tp raData)
{
	FILE *fp = NULL;
	int fh;
...
/*
 * fix ipv6 ce-router logo lan_rfc3736 item 11 fail that 
 * use DUID_LL methoud to generate DUID and keep 
 * the DUID_LL after restarting we not use DUID_LLT 
 * [RFC 7084],L-8
