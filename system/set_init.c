...
#ifdef CONFIG_APP_THERMAL_CONTROL
	system("killall -9 ther_control");
#endif /* CONFIG_APP_THERMAL_CONTROL */
}
...
void mount_jffs2_partition()
{
	...
	if(access("/jffs2/cwmp_config",F_OK) != 0) {
		...
		printf("Create cwmp_config dictionary!\n");
	}

#ifdef CONFIG_FWD_UNIFIED_SUPPORT_PLUGIN
	if(access(PLUGIN_INST_DEF_PATH,F_OK) != 0) {
		snprintf(cmdbuf, sizeof(cmdbuf), "mkdir %s", PLUGIN_INST_DEF_PATH);
		system(cmdbuf);
		printf("Newly create jffs2 plugins dictionary %s!\n", PLUGIN_INST_DEF_PATH);
	}
	snprintf(buf, sizeof(buf), "%s/%s", PLUGIN_INST_DEF_PATH, PLUGIN_INSTALL_DIRNAME);
	if(access(buf, F_OK) != 0) {
		snprintf(cmdbuf, sizeof(cmdbuf), "mkdir %s", buf);
		system(cmdbuf);
		printf("Newly create jffs2 plugins install info dictionary %s!\n", buf);
	}
#endif
...
#ifdef CONFIG_APP_THERMAL_CONTROL
int start_thermal_control()
{
	system("mkdir -p /var/ther;cp -f /etc/ther.conf /var/ther/conf");
	system("ther_control &");
	return 0;
}
#endif /* CONFIG_USER_THERMAL_CONTROL */
...
int setinit(int argc, char** argv)
{
	...
	int rtl_link_uptype = 0;
	int rtl_link_opmode = 0;
#endif
	...
	system("echo 1 > /proc/br_subnet_from_wan_to_cpu");
        ...
        if(rtl_link_enabled){
            if(!isFileExist(RTL_LINK_PID_FILE)){
                ...
                system("elink -m1 -d2");
#else
                apmib_get(MIB_RTL_LINK_UPTYPE, (void *)&rtl_link_uptype);
                apmib_get(MIB_RTL_LINK_GW_MODE, (void *)&rtl_link_opmode);
                snprintf(tmpBuff, sizeof(tmpBuff), "andlink -m1 -d2 -u%d -o%d", rtl_link_uptype, rtl_link_opmode);
                system(tmpBuff);
#endif
            }
        }
...
{
    int i=0, j=0, wlan_disabled=0, intValue, opmode, wisp_idx;
    char tmpbuff[16],tmpbuff1[16], tmpBuff[512];
    char *token=NULL, *savestr1=NULL;
    int led_on;
#ifdef CONFIG_SMART_REPEATER
    ...
    apmib_get(MIB_RTL_LINK_LED_ON, (void *)&led_on);
    if(led_on){
        for(i=0; i<NUM_WLAN_INTERFACE; i++){
