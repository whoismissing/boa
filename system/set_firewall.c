...
//dip match
...
#if defined(CONFIG_RTL_HW_QOS_SUPPORT)
	sprintf(cmd,"echo restore %s dscp remark > /proc/rtl_hw_qos_config",pInterface);
	system(cmd);

	sprintf(cmd,"echo restore %s vlan remark > /proc/rtl_hw_qos_config",pInterface);
	system(cmd);

	sprintf(cmd,"echo disable dscp remark > /proc/rtl_hw_qos_config");
	system(cmd);

	sprintf(cmd,"echo disable vlan remark > /proc/rtl_hw_qos_config");
	system(cmd);

	#if defined(CONFIG_RTL_HARDWARE_IPV6_SUPPORT)
	sprintf(cmd,"echo aclpri flush %s > /proc/rtl_hw_qos_config",pInterface);
	system(cmd);
	
	sprintf(cmd,"echo aclpri flush %s > /proc/rtl_hw_qos_config",br_interface);
	system(cmd);

	sprintf(cmd,"echo restore %s v6Dscp remark > /proc/rtl_hw_qos_config",pInterface);
	system(cmd);

	sprintf(cmd,"echo disable v6Dscp remark > /proc/rtl_hw_qos_config");
	system(cmd);
	#endif
#endif
	...
	for (j=0;j<RTL_MAX_HW_NETIF_NUM;j++)
	{
		for (k=0;k<MAX_QOS_RULE_NUM;k++)
		{
				...
				*((char *)&entry) = (char)qosConfigInfo[j][k].index;
					...
					(entry.remark_dscp!=ADVANCED_IPQOS_DEF_CHAR_VALUE))
				{
			...
			#if defined(CONFIG_RTL_HW_QOS_SUPPORT) && !defined(CONFIG_RTL_83XX_QOS_SUPPORT)
				#if defined(CONFIG_RTL_HARDWARE_IPV6_SUPPORT)
					if(strcmp(str_iptables, Ip6tables) == 0){
						if((entry.remark_vlan_pri!=ADVANCED_IPQOS_DEF_CHAR_VALUE) &&
						(entry.remark_dscp!=ADVANCED_IPQOS_DEF_CHAR_VALUE)){
							sprintf(command,"echo remark netif %s mark %d v6 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,entry.remark_dscp,entry.remark_vlan_pri,NULL_STR);
						}
						else if(entry.remark_dscp!=ADVANCED_IPQOS_DEF_CHAR_VALUE){
							#if defined(SET_DFAULT_VLANREMARK)
							sprintf(command,"echo remark netif %s mark %d v6 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,entry.remark_dscp,0,NULL_STR);
							#else
							sprintf(command,"echo remark netif %s mark %d v6 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,entry.remark_dscp,entry.remark_vlan_pri,NULL_STR);
							#endif
						}
							...
							sprintf(command,"echo remark netif %s mark %d v6 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,0,entry.remark_vlan_pri,NULL_STR);
							#else
							sprintf(command,"echo remark netif %s mark %d v6 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,entry.remark_dscp,entry.remark_vlan_pri,NULL_STR);
							#endif
						}
				...
				#endif
					{
						if((entry.remark_vlan_pri!=ADVANCED_IPQOS_DEF_CHAR_VALUE) &&
						(entry.remark_dscp!=ADVANCED_IPQOS_DEF_CHAR_VALUE)){
							sprintf(command,"echo remark netif %s mark %d v4 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,entry.remark_dscp,entry.remark_vlan_pri,NULL_STR);
						}
						else if(entry.remark_dscp!=ADVANCED_IPQOS_DEF_CHAR_VALUE){
							#if defined(SET_DFAULT_VLANREMARK)
							sprintf(command,"echo remark netif %s mark %d v4 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,entry.remark_dscp,0,NULL_STR);
							#else
							sprintf(command,"echo remark netif %s mark %d v4 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,entry.remark_dscp,entry.remark_vlan_pri,NULL_STR);
							#endif
						}
							...
							#if defined(SET_DFAULT_DSCPREMARK)
							sprintf(command,"echo remark netif %s mark %d v4 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,0,entry.remark_vlan_pri,NULL_STR);
							#else
							sprintf(command,"echo remark netif %s mark %d v4 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,entry.remark_dscp,entry.remark_vlan_pri,NULL_STR);
							#endif
						}
					...
					if(strcmp(str_iptables, Ip6tables) == 0){
							...
							sprintf(command,"echo remark netif %s mark %d v6 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,0,0,NULL_STR);
						#elif defined(SET_DFAULT_DSCPREMARK)
							sprintf(command,"echo remark netif %s mark %d v6 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,0,255,NULL_STR);
						#elif defined(SET_DFAULT_VLANREMARK)
							sprintf(command,"echo remark netif %s mark %d v6 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,255,0,NULL_STR);
						#else
							sprintf(command,"echo remark netif %s mark %d v6 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,255,255,NULL_STR);
						#endif
					}
					else{
						
						#if defined(SET_DFAULT_DSCPREMARK) && defined(SET_DFAULT_VLANREMARK)
							sprintf(command,"echo remark netif %s mark %d v4 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,0,0,NULL_STR);
						#elif defined(SET_DFAULT_DSCPREMARK)
							sprintf(command,"echo remark netif %s mark %d v4 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,0,255,NULL_STR);
						#elif defined(SET_DFAULT_VLANREMARK)
							sprintf(command,"echo remark netif %s mark %d v4 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,255,0,NULL_STR);
						#else
							sprintf(command,"echo remark netif %s mark %d v4 %d %d > /proc/rtl_hw_qos_config %s", qosConfigInfo[j][k].netif,qosConfigInfo[j][k].mark,255,255,NULL_STR);
						#endif
					}
				#else
					#if defined(SET_DFAULT_DSCPREMARK) && defined(SET_DFAULT_VLANREMARK)
				...
				#if defined(CONFIG_RTL_HW_QOS_SUPPORT) && defined(CONFIG_RTL_HARDWARE_IPV6_SUPPORT)
				if(entry.mode & QOS_RESTRICT_IPV6){
					if(strcmp(entry.ip6_src,"")){
						unsigned char str_match[128] = {0};
						int len=0;
						int matched=0;
						...
						sprintf(str_ipv6, "%s", entry.ip6_src);

						if(inet_pton6(str_ipv6, &ip6addr.s6_addr32) == 0)
						{
							printf("invalid internet v6 address! [%s]:[%d].\n",__FUNCTION__,__LINE__);
							return -1;
						}
						
						if((entry.bandwidth > 0) && (qosConfigInfo[j][k].netif!=br_interface)){
							matched ++;
							len+=sprintf(str_match+len, "ipv6 src %x %x %x %x",ip6addr.s6_addr32[0], ip6addr.s6_addr32[1], 
								ip6addr.s6_addr32[2], ip6addr.s6_addr32[3]);
							acl_interface =br_interface;
						}

						if((entry.bandwidth_downlink > 0) && (qosConfigInfo[j][k].netif==br_interface)) {
							matched ++;
							len+=sprintf(str_match+len, "ipv6 dst %x %x %x %x",ip6addr.s6_addr32[0], ip6addr.s6_addr32[1],
								ip6addr.s6_addr32[2], ip6addr.s6_addr32[3]);
							
							/*qos interface and acl interface not the same one*/
							acl_interface =pInterface;
						}

						if(len==0){
							printf("not support match rule!\n");
							continue;
						}
						else{
							//echo aclpri netif qosnetif aclnetif bridgedFalg matchrule mark markvalue >
							sprintf(command,"echo aclpri netif %s %s 0 %s mark %d > /proc/rtl_hw_qos_config %s ",qosConfigInfo[j][k].netif, acl_interface,str_match,qosConfigInfo[j][k].mark,NULL_STR);
							//printf("%s\n", command);
							system(command);
						}
					}
...
#endif
#ifdef CONFIG_IPV6
int setIpv6DoS(unsigned long enabled, unsigned long ipv6_enabled, int op)
{
	char cmdBuffer[500];
	unsigned int *dst, *mask;
	unsigned int synsynflood=0;
	...
	unsigned int pipicmpflood=0;
	unsigned int blockTime=0;
	unsigned int Ipv6SmurfFlood=0,Ipv6IcmpTooBigFlood=0,Ipv6NSFlood=0;
	struct in_addr curIpAddr={0}, curSubnet={0};
	...
	apmib_get(MIB_DOS_PIPICMP_FLOOD, (void *)&pipicmpflood);
	apmib_get(MIB_DOS_BLOCK_TIME, (void *)&blockTime);
	apmib_get(MIB_IPV6_DOS_SMURF_FLOOD, (void *)&Ipv6SmurfFlood);
	apmib_get(MIB_IPV6_DOS_ICMP_TOO_BIG_FLOOD, (void *)&Ipv6IcmpTooBigFlood);
	apmib_get(MIB_IPV6_DOS_NS_FLOOD, (void *)&Ipv6NSFlood);
	getInAddr("br0", IP_ADDR_T, (void *)&curIpAddr);
	getInAddr("br0", NET_MASK_T, (void *)&curSubnet);
	//apmib_get(MIB_IP_ADDR,  (void *)ipbuf);
	dst = (unsigned int *)&curIpAddr;
	...
	if(op==2){
  		sprintf(cmdBuffer, "echo \" 2 %X %X %ld %d %d %d %d %d %d %d %d %d %d %ld %d %d %d\" >  /proc/enable_dos", 
			htonl(*dst), htonl(*mask), enabled, synsynflood, sysfinflood, sysudpflood, sysicmpflood, 
			pipsynflood, pipfinflood, pipudpflood, pipicmpflood, blockTime,0,
			ipv6_enabled,Ipv6SmurfFlood,Ipv6IcmpTooBigFlood,Ipv6NSFlood);
  		  
  	}else{
  		sprintf(cmdBuffer, "echo \" 0 %X %X %ld %d %d %d %d %d %d %d %d %d %d %ld %d %d %d\" >  /proc/enable_dos", 
			(htonl(*dst) & 0xFFFFFF00), htonl(*mask), enabled, synsynflood, sysfinflood, sysudpflood,
			sysicmpflood, pipsynflood, pipfinflood, pipudpflood, pipicmpflood, blockTime,0,
			ipv6_enabled,Ipv6SmurfFlood,Ipv6IcmpTooBigFlood,Ipv6NSFlood);
...
#if defined (QOS_OF_TR069) && !defined(CONFIG_CMCC_QOS)
#define LAN_FLAG	0x1
#define WAN_FLAG	0x2
#if 1
#ifndef max
           ...
           if(class_entry.Protocol == 6 || class_entry.Protocol == 17 || class_entry.Protocol == -1 || 
			...
			if(class_entry.VLANIDCheck!=-1)
			{
				if((class_entry.VLANIDCheck>=0)&&(class_entry.VLANIDCheck<4095))
				{
					...
					sprintf(str_vid,"%s %s %s %s %d",match,"vlanid",invert_flag,"--svid-value",class_entry.VLANIDCheck);
				}
			}
...
#ifdef CONFIG_IPV6
void setIpv6RulesWithOutDevice(int opmode)
{
	int intVal=0, natEnabled=0;
	int intVal1=0;
	int intVal2 = 0;
	...
	int my_wan_type = 0;
	unsigned long	dos_enabled = 0;
	unsigned long	ipv6_dos_enabled = 0;
	char wan_type[8];
	...
	RunSystemCmd("/tmp/firewall_igd", "echo", "1", NULL_STR);//disable fastpath when port filter is enabled
	apmib_get(MIB_DOS_ENABLED, (void *)&dos_enabled);
	apmib_get(MIB_IPV6_DOS_ENABLE, (void *)&ipv6_dos_enabled);
	setIpv6DoS(dos_enabled,ipv6_dos_enabled, opmode);
}
...
RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, _protocol, _udp,  dport, "1900:1900", in, pInterface, _dest, WanIpAddr, jump,DROP, NULL_STR);
    ...
    apmib_get(MIB_RTL_LINK_GW_MODE, (void *) &intVal);
    if(intVal){
        //5683: coap port
...
#ifdef  CONFIG_APP_MINIUPNPD //miniupnpd
	apmib_get(MIB_UPNP_ENABLED, (void *)&intVal);
	if(intVal){	//add chain and rule	
//add nat/filter rule in chain	
		RunSystemCmd(NULL_FILE, Iptables, _table, nat_table, NEW, "MINIUPNPD", NULL_STR);
		RunSystemCmd(NULL_FILE, Iptables, _table, nat_table, NEW, "MINIUPNPD-POSTROUTING", NULL_STR);
		RunSystemCmd(NULL_FILE, Iptables, NEW,  "MINIUPNPD", NULL_STR);	
		
		RunSystemCmd(NULL_FILE, Iptables, _table, nat_table, ADD, PREROUTING, in, pInterface, jump, "MINIUPNPD", NULL_STR);
		RunSystemCmd(NULL_FILE, Iptables, _table, nat_table, ADD, POSTROUTING, out, pInterface, jump,"MINIUPNPD-POSTROUTING", NULL_STR);
		RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, in, pInterface, NOT, out, pInterface, jump, "MINIUPNPD", NULL_STR);	
	}
#endif  //CONFIG_APP_MINIUPNPD
...
#ifdef CONFIG_IPV6
int setFirewallIpv6tablesRules(int argc, char** argv)
{
	int opmode=-1;
	struct in_addr wanaddr;
	char IpAddr[30], *strIp;
	...
	printf("Init Ipv6Firewall Rules....\n");
	apmib_get(MIB_OP_MODE, (void *)&opmode);
	setIpv6RulesWithOutDevice(opmode);
...
{	
	...
	if (opmode == GATEWAY_MODE){
		...
		//qos
		#if (defined (QOS_OF_TR069) || defined (MULTI_WAN_QOS) || defined(CONFIG_CMCC_QOS)) && !defined(CONFIG_RTL_ISP_MULTI_PHY_WAN_SUPPORT)
		if ( apmib_get( MIB_QOS_ENABLE, (void *)&qosFlag) == 0)
		#else
		...
		qosFlag = 0;	// support hw ipv6 ip address classification 
		#endif
		...
		if(intVal ==1 && intVal_num>0){
			for (index=1; index<=intVal_num; index++) {
				...
				*((char *)&port_entry) = (char)index;
				...
				#ifdef CONFIG_IPV6
				//port not differentiate ipv6 or ipv4
