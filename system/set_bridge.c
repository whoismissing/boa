	...
	for (i=1; i<=entrynum; i++) 
	{
		*((char *)&entry) = (char)i;
		...
		//if(enabled==1)
		{
			...
			//		printf("#####%s:%s:%d enable = %d, sdhcp_entry.dhcpConSPInstNum = %d, entry.InstanceNum = %d\n",
			//			__FILE__,__FUNCTION__,__LINE__, sdhcp_entry.dhcpRsvdIpEntryEnabled, sdhcp_entry.dhcpConSPInstNum, entry.InstanceNum);
					
					if(sdhcp_entry.dhcpRsvdIpEntryEnabled==0 ||sdhcp_entry.dhcpConSPInstNum != entry.InstanceNum)
						continue;
...
/*gateway &  lan dns*/
		...
		int tr098_set_dns_flag = 0;
		apmib_get(MIB_LAN_DNS1, (void *)&addr);
		if(addr.s_addr>0)
		{
			tr098_set_dns_flag |= 1;
			sprintf(line_buffer,"opt dns %s\n", inet_ntoa(addr));
			write_line_to_file(DHCPD_CONF_FILE, 2, line_buffer);
		}
		apmib_get(MIB_LAN_DNS2, (void *)&addr);
		if(addr.s_addr>0)
		{
			tr098_set_dns_flag |= 1;
			sprintf(line_buffer,"opt dns %s\n", inet_ntoa(addr));
		...
		apmib_get(MIB_LAN_DNS3, (void *)&addr);
		if(addr.s_addr>0)
		{
			tr098_set_dns_flag |= 1;
			sprintf(line_buffer,"opt dns %s\n", inet_ntoa(addr));
...
int setbridge(char *argv)
{
	...
	int hw_br_vid_same_num = 0, k = 0, temp_vid = 0, repeat_flag = 0;
	int hw_bridge_num = 0;
...
//for pass thru
	...
	for (i = 0; i < 5; i++){
		temp_vid = rtl_hw_vlan_info[i].vlan_vid;
		repeat_flag = 0;
		//if vlan id, already checked, skip it;	
		for (k = i-1; k >=0; k--){
			if((rtl_hw_vlan_info[k].vlan_enabled == 1)&&((rtl_hw_vlan_info[k].vlan_bridge == 1)))
			{
				if (temp_vid == rtl_hw_vlan_info[k].vlan_vid){
					repeat_flag = 1;
					break;
				}
			}
		...
		if (!repeat_flag){
			for(j = i+1; j < 5; j++){
				if((rtl_hw_vlan_info[j].vlan_enabled == 1)&&((rtl_hw_vlan_info[j].vlan_bridge == 1)))
				{
					if (temp_vid == rtl_hw_vlan_info[j].vlan_vid)
						hw_br_vid_same_num++;
				}
			}
		}
	}
	hw_bridge_num =  hw_bridge_num - hw_br_vid_same_num;
...
//printf("\r\n iface_name=[%s],__[%s-%u]\r\n",iface_name,__FILE__,__LINE__);
	...
	/*
	* fix ipv6 ready logo addr test fail that dut lan port can't 
	* send out dad_ns packet when reboot
	*/
	sleep(2);
#endif
#if defined(CONFIG_RTL_ULINKER)
