...
#define DHCP6C_PID_FILE_FORMAT "/var/run/dhcp6c_wan%d.pid"
#define DHCP6C_CONF_FILE_FORMAT "/var/ipv6/dhcp6c_wan%d.conf"
unsigned char * getConWanIface(WAN_DATA_Tp wan_p)
{
	...
	|| wan_p->wan_data.wan_type == L2TP
		...
		return wan_p->wan_data.virtual_wan_iface;
	else
		return wan_p->wan_data.wan_iface;
}
...
int format_wan_data(BASE_DATA_Tp pdata,BASE_DATA_Tp orig_data)
{
	...
	int wan_wifi_idx;
#endif
	int wan_index = orig_data->wan_idx;
    ...
    WANp->wan_data.app_type = WanIfaceEntry.applicationtype;
#ifdef CONFIG_IPV6
	WANp->wan_data.ipv6Enable = WanIfaceEntry.ipv6Enable;
#endif
	if(getWanIfName(WANp->base.wan_idx, wanIfName,32) < 0)
		return 0;
	...
	{
		sprintf(WANp->wan_data.wan_iface,"%s",wanIfName);
		sprintf(WANp->wan_data.real_iface,"%s",wanIfName);
		GET_REAL_IFNAME(WANp->wan_data.real_iface);
	}
	else if(opmode == WISP_MODE)
	{
		apmib_get(MIB_WISP_WAN_ID,&wan_wifi_idx);
		sprintf(WANp->wan_data.wan_iface,"wlan%d-vxd",wan_wifi_idx);
		sprintf(WANp->wan_data.real_iface,"wlan%d-vxd",wan_wifi_idx);
	}
#else
	sprintf(WANp->wan_data.wan_iface,"%s",wanIfName);
	sprintf(WANp->wan_data.real_iface,"%s",wanIfName);	
	...
	if(getPPPIfName(WANp->base.wan_idx, wanIfName,sizeof(wanIfName))<0){
	...
	if(	WANp->wan_data.wan_type == PPPOE
		...
		sprintf(WANp->wan_data.virtual_wan_iface,"%s",wanIfName);

	snprintf(cmdBuf,sizeof(cmdBuf),"echo %s>/var/wan_interface_%d",getConWanIface(WANp),WANp->base.wan_idx);
	//fprintf(stderr,"%s:%d cmdBuf=%s\n",__FUNCTION__,__LINE__,cmdBuf);
	system(cmdBuf);

	if((WanIfaceEntry.enable) && (WanIfaceEntry.applicationtype==APPTYPE_INTERNET || WanIfaceEntry.applicationtype==APPTYPE_TR069_INTERNET))
	{
		snprintf(cmdBuf,sizeof(cmdBuf),"echo %s>/var/internet_wan_%d",getConWanIface(WANp),WANp->base.wan_idx);
		system(cmdBuf);
	}
...
{ 
	...
	WAN_DATA_T wan_info={0};
	char dhcp6cPidFileName[128]={0};
	char dhcp6cConfFileName[128]={0};
	...
	if((pfile = fopen(WAN_LINK_STATUS_FILE,"r+"))!= NULL)
	{
		...
		{	
		 ...
		 if (link_down_restart_dhcp6c==0) {/*kill dhcp6c daemon and restart  if exists*/
			for(i=1;i<=WANIFACE_NUM;i++){		
				base.wan_idx=i;			
				getWanIfaceEntry(base.wan_idx,&WanIfaceEntry);
				if(!WanIfaceEntry.enable){
					//printf("%s.%d. wan %d not enable!\n",__FUNCTION__,__LINE__,base.wan_idx);
					continue;
				}
				
				if(!WanIfaceEntry.ipv6Enable){ // need ipv6 enable
					continue;	
				}
				...
				if(IPV6_LINKTYPE_IP != WanIfaceEntry.ipv6LinkType) // only need to check ipoe+dhcp6c	
					continue;	

				snprintf(dhcp6cPidFileName,sizeof(dhcp6cPidFileName),DHCP6C_PID_FILE_FORMAT,base.wan_idx);
				snprintf(dhcp6cConfFileName,sizeof(dhcp6cConfFileName),DHCP6C_CONF_FILE_FORMAT,base.wan_idx);

				if(isFileExist(dhcp6cPidFileName)){
					pid = getPid_fromFile(dhcp6cPidFileName);
					if (pid > 1)
					{
						kill(pid,15);
						//printf("%s.%d. kill dhcp6c,pid=%d\n",__FUNCTION__,__LINE__,pid);		
					}
					unlink(dhcp6cPidFileName);
				} else {		
					//printf("%s.%d. %s not exist!\n",__FUNCTION__,__LINE__,dhcp6cPidFileName);
				}

				//flush v6 global address
				snprintf(tmpStr,sizeof(tmpStr),"ip -6 address flush dev %s scope global",getConWanIface(&wan_info));
				//printf("%s.%d %s \n",__FUNCTION__,__LINE__,tmpStr);
				system(tmpStr);

				// ipv6 disable->enable to send RS
				snprintf(tmpStr,sizeof(tmpStr),"echo 1 >/proc/sys/net/ipv6/conf/%s/disable_ipv6",getConWanIface(&wan_info));
				system(tmpStr);
				snprintf(tmpStr,sizeof(tmpStr),"echo 0 >/proc/sys/net/ipv6/conf/%s/disable_ipv6",getConWanIface(&wan_info));
				system(tmpStr);

				/*restart dhcp6c daemon*/
				snprintf(tmpStr,sizeof(tmpStr),"dhcp6c -I %d -c %s -p %s %s",base.wan_idx,dhcp6cConfFileName,dhcp6cPidFileName,getConWanIface(&wan_info));
				//printf("%s.%d. %s\n",__FUNCTION__,__LINE__,tmpStr);
				system(tmpStr);
				}
...
//printf("\r\n line_buffer=[%s],__[%s-%u]\r\n",line_buffer,__FILE__,__LINE__);
		...
		start_upnp_igd();//restart miniigd/miniupnpd
		return 1;
	}
...
#ifdef CONFIG_IPV6
void ipv6wan_connect(int argc, char** argv)
{
#ifndef CONFIG_IPV6_CE_ROUTER_SUPPORT
	/* need to modify*/
	setFirewallIpv6tablesRules(0, NULL);
#endif
}
#endif
...
void checkDhcp6pd()
{
	...
	//printf("%s:%d\n",__FUNCTION__,__LINE__);
	int val=0;
	apmib_get(MIB_IPV6_DHCP_PD_ENABLE,&val);
	if(val==0)
		return;
	//printf("%s:%d\n",__FUNCTION__,__LINE__);
...
void radvd_reconfig()
{
	printf("start create radvd configure\n");
		...
		sprintf(tmpBuf,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d",
				htons(dhcp6_pd.addr6[0]),htons(dhcp6_pd.addr6[1]),
				htons(dhcp6_pd.addr6[2]),htons(dhcp6_pd.addr6[3]),
				htons(dhcp6_pd.addr6[4]),htons(dhcp6_pd.addr6[5]),
				htons(dhcp6_pd.addr6[6]),htons(dhcp6_pd.addr6[7]),
				64);
		fprintf(fp, "prefix %s\n", tmpBuf);				
		fprintf(fp, "{\n");	
		fprintf(fp, "	AdvOnLink on;\n");	
		...
		fprintf(fp, "AdvRetransTimer %u;\n", radvdCfgParam.interface.AdvRetransTimer);
		fprintf(fp, "AdvCurHopLimit %d;\n", radvdCfgParam.interface.AdvCurHopLimit);
		if(access("/var/dhcp6pd_need_update",0)==0 && access(DHCP6PD_CONF_INVALID,0)<0 && access(RECV_RA_INVALID,0)<0)
		{
			fprintf(fp, "AdvDefaultLifetime %d;\n", radvdCfgParam.interface.AdvDefaultLifetime);			
		}else
		{
		...
		//add prefix
		if(radvdCfgParam.interface.prefix[0].enabled > 0 && access("/var/dhcp6pd_need_update",0)==0 && access(DHCP6PD_CONF_INVALID,0)<0)
		{
			sprintf(tmpBuf,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%d",
					htons(dhcp6_pd.addr6[0]),htons(dhcp6_pd.addr6[1]),
					htons(dhcp6_pd.addr6[2]),htons(dhcp6_pd.addr6[3]),
	...
	//		printf("%s %d: prefix is %s.\n", __FUNCTION__, __LINE__, tmpBuf);
		...
		apmib_get(MIB_IPV6_ULA_ENABLE,&val);
		if(val==1 && access("/var/dhcp6pd_need_update",0)==0 && access(DHCP6PD_CONF_INVALID,0)<0)
		{
		...
		if(isFileExist(DNSV6_ADDR_FILE))
		{
			if((fs=fopen("/var/dns6.conf","r")))
			{				
				memset(tmpStr, 0, sizeof(tmpStr));
				while(fgets(tmpBuf, sizeof(tmpBuf), fs)!=NULL)
				{
				
					if(strstr(tmpBuf,"nameserver"))
					{
						strcat(tmpStr, tmpBuf+strlen("nameserver")+1);
						strcat(tmpStr, " ");
						if(strlen(tmpStr)>=1)
		...
		if(isFileExist(DNSV6_ADDR_FILE))
		{
			...
			{				
				...
				while(fgets(tmpBuf, sizeof(tmpBuf), fs)!=NULL)
				{
					if(strstr(tmpBuf,"search"))
					{
						strcat(tmpStr, tmpBuf+strlen("search")+1);
						strcat(tmpStr, " ");
						if(strlen(tmpStr)>=1)
						{
							//tmpStr[strlen(tmpStr)-1]=0;
...
void start_upnp_igd(){
	...
	RunSystemCmd(NULL_FILE, "killall", "-15", "miniupnpd", NULL_STR);
#else
	return;
#endif

	int intValue = 0, wantype, opmode, wisp_id, upnp_wanType;
	char lan_interface[32] = {0};
#if defined(CONFIG_DYNAMIC_WAN_IP)
	int  mib, wan_ip_dynamic = 0;
#endif

	if(!apmib_get(MIB_UPNP_ENABLED, (void *)&intValue) || !intValue) return;
	if(!apmib_get(MIB_WAN_DHCP,(void *)&wantype)) return;	
	if(!apmib_get(MIB_OP_MODE,(void *)&opmode)) return;
	if(!apmib_get(MIB_WISP_WAN_ID,(void *)&wisp_id)) return;
	
	switch(wantype){
	case DHCP_DISABLED:
	case DHCP_CLIENT:
	case PPPOE:
		upnp_wanType = wantype;
		break;
	case PPTP:
	case L2TP:
#if defined(CONFIG_DYNAMIC_WAN_IP)
		mib = ((wantype == PPTP) ? MIB_PPTP_WAN_IP_DYNAMIC: MIB_L2TP_WAN_IP_DYNAMIC);
		if(!apmib_get(mib, (void *)&wan_ip_dynamic)) return;
		if(wan_ip_dynamic != STATIC_IP){
			upnp_wanType = DHCP_CLIENT;
			break;
		}
#endif	
		upnp_wanType = wantype;
	...
	snprintf(lan_interface, sizeof(lan_interface), "%s", "br0");
#ifdef CONFIG_APP_MINIIGD
	start_miniigd(upnp_wanType, opmode, wisp_id, lan_interface);
#elif defined(CONFIG_APP_MINIUPNPD) 
	start_miniupnpd(upnp_wanType, opmode, wisp_id, lan_interface);
#endif
}

#ifdef CONFIG_APP_MINIIGD
void start_miniigd(int wantype, int sys_opmode, int wisp_id, char *lan_interface)
{
	char tmp1[16]={0};
	char tmp2[16]={0};
...
#elif defined(CONFIG_APP_MINIUPNPD) 
void start_miniupnpd(int wan_type, int op_mode, int wisp_id, char *lan_interface){
	char tmp1[16]={0};
	char tmp2[16]={0};

	sprintf(tmp1, "%d", wan_type);
	sprintf(tmp2, "wlan%d", wisp_id);	
	sleep(1);
	if(op_mode==WISP_MODE)
	{
		...
		//strcat(tmp2, "-vxd");
#endif			
		RunSystemCmd(NULL_FILE, "miniupnpd", "-e", tmp1, "-a", lan_interface, "-y", tmp2, NULL_STR); 
	}else		
		RunSystemCmd(NULL_FILE, "miniupnpd", "-e", tmp1, "-a", lan_interface,NULL_STR);						
}	
#endif
...
void start_ddns(void)
{
	...
	unsigned char ddns_port[MAX_PORT_LEN];
	...
	if(ddns_onoff == 1)
	{
		...
		RunSystemCmd(NULL_FILE, "echo", "Start DDNS daemon", NULL_STR);
		...
		apmib_get( MIB_DDNS_PORT,  (void *)ddns_port);
		if(ddns_type == 0) // 0:ddns; 1:tzo
			RunSystemCmd(NULL_FILE, "ddns_inet", "-x", "dyndns", ddns_user_name, ddns_password, ddns_domanin_name,ddns_port, NULL_STR);
		else if(ddns_type == 1)
			RunSystemCmd(NULL_FILE, "ddns_inet", "-x", "tzo", ddns_user_name, ddns_password, ddns_domanin_name,ddns_port, NULL_STR);		
		else if(ddns_type == 2) 
			RunSystemCmd(NULL_FILE, "ddns_inet", "-x", "noip", ddns_user_name, ddns_password, ddns_domanin_name , NULL_STR);
		else if(ddns_type == 3)
			RunSystemCmd(NULL_FILE, "ddns_inet", "-x", "oray", ddns_user_name, ddns_password, ddns_domanin_name, NULL_STR);
	}
...
void start_ntp(void)
{
	...
	unsigned int i;
	char	ntp_server[NTP_SERVER_NUM][40];
	char cmd[128];
	...
	if(ntp_onoff == 1)
	{
		...
		snprintf(cmd,sizeof(cmd), "ntp_inet -x ");
		
		for(i=0; i<NTP_SERVER_NUM; i++)
		{
			if(i<2)
				apmib_get( MIB_NTP_SERVER_IP1+i, (void *)ntp_server[i]);
			else
				apmib_get( MIB_NTP_SERVER_IP3+i-2, (void *)ntp_server[i]);

			if(ntp_server[i])
				strcat(cmd,ntp_server[i]);
...
#else
	snprintf(script_file, sizeof(script_file), "%s", "/usr/share/udhcpc/dhcpc.sh");/*deconfig script path*/
	snprintf(deconfig_script, sizeof(deconfig_script), "/var/udhcpc/%s.deconfig", iface);/*deconfig script path*/
#endif
...
int start_wan(int wan_mode, int sys_op, char *wan_iface, char *lan_iface, int wisp_id, int act_source)
{
	...
	{
		int internet_wan_disabled=0;
		apmib_get(MIB_INTERNET_WAN_DISABLED, (void *)&internet_wan_disabled);
		//printf("\n%s:%d internet_wan_disabled=%d\n",__FUNCTION__,__LINE__,internet_wan_disabled);
		if(internet_wan_disabled)
		{
