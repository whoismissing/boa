...
#ifdef CONFIG_IPV6_CE_ROUTER_SUPPORT
		if(radvdCfgParam.interface.prefix[0].prefix_mode == 1){
			sprintf(tmpStr, "AdvDefaultLifetime 0;\n");
				write(fh, tmpStr, strlen(tmpStr));
		}
		else
#endif
...
#ifdef CONFIG_IPV6_CE_ROUTER_SUPPORT
	if(radvdCfgParam.interface.prefix[0].prefix_mode == 0 || radvdCfgParam.interface.prefix[0].prefix_mode == 2)
	{
#endif
		/*prefix 1*/
...
#ifdef CONFIG_IPV6_CE_ROUTER_SUPPORT
/*
 * fix ipv6 ce-router logo wan_rfc3633 item 11 fail that 
 * and forward_rfc7084 item 2 fail that forward between 
 * lan and wan interfacece should be enabled after router 
 * completed addr and pd acquisition. 
 * and By default, an IPv6 CE router that has no default router(s) on
...
*/
			...
			system("echo 1 > /proc/sys/net/ipv6/conf/br0/forwarding");
#endif
		}
	}
...
void set_staticv6firewall()
{
	...
	sprintf(ipv6dns,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
	...
	sprintf(ipv6Str,"sysconf ipv6_static ipv6_dns=%s",ipv6dns);
	system(ipv6Str);
}
void set_wanv6()
{
		...
		sprintf(tmpStr,"echo 0 > /proc/sys/net/ipv6/conf/all/forwarding 2> /dev/null");
		system(tmpStr);
		if(!apmib_get(MIB_IPV6_ORIGIN_TYPE,&val)){	
			fprintf(stderr, "get mib %d error!\n", MIB_IPV6_ORIGIN_TYPE);
			return;			
...
#if defined(CONFIG_IPV6_MANAGE_INET)
...
void set_lanv6()
{
	...
	if(enable_ula)
	{
		...
		if(strstr(tmpBuf, "0000:0000:0000:0000")==NULL)
		system(tmpBuf);
	}
#endif
	return;
