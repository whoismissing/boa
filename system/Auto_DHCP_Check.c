...
void set_lan_dhcpc(char *iface)
{
	...
	int intValue=0;
#endif
	snprintf(script_file, sizeof(script_file), "%s", "/usr/share/udhcpc/dhcpc.sh");
	sprintf(deconfig_script, "/var/udhcpc/%s.deconfig", iface);/*deconfig script path*/
	sprintf(pid_file, "/var/run/udhcpc-%s.pid", iface); /*pid path*/
...
static void down_up_lan_interface(void)
{	
	deauth_wlan_clients();	
...
static int switch_dhcpc_dhcpd(void)
{
	...
	//printf("\n%s:%d tempStatus=%d\n",__FUNCTION__,__LINE__,tempStatus);
	if(tempStatus == 1) //if front-end router's DHCP is enabled ,this router's DHCPD should be disabled
	{
		...
		if(!isFileExist("/var/run/udhcpc-br0.pid"))
		{
			set_lan_dhcpc("br0");
		}
	...
	else			   //if front-end router's DHCP is disabled and this router's DHCP is not running,this router's DHCP should be enabled
	{
		if(isFileExist("/var/run/udhcpc-br0.pid"))
		{
			pid=getPid_fromFile("/var/run/udhcpc-br0.pid");
			if(pid>0)
			{
				snprintf(cmdBuf, sizeof(cmdBuf), "kill -9 %d", pid);
				system(cmdBuf);
				
				unlink("/var/run/udhcpc-br0.pid");
			}
		}
