...
{
  var wanType = form.wanType.options[form.wanType.selectedIndex].value ;
  if(form.pppoeNumber)
  ...
  if (form.lte4g_build &&form.lte4g_build.value == 1 && form.lte4g_enable &&form.lte4g_enable.value == 1)
	wanType = 1;
  ...
  else if ( wanType == 3){ //pppoe wanType
...
//----------------------Fourth pppoe info check  End----------------------------------		  
  ...
  else if ( wanType == 4){ //pptp wanType
   ...
   /*-- keith: add l2tp support. 20080515  */
   else if ( wanType == 6){ //l2tp wanType
...
function updateType(form, wlan_id, isctcom)
{
	var mode_selected=0;
	var Type_selected=0;
	var index_channelbound=0;
  updateChan(form, wlan_id);
  updateIputState(form, wlan_id,isctcom);
  if(isctcom==1)
...
function checkIpAddr(field, msg)
{
  if (field.value=="") {
   ...
   if ( !checkDigitRange(field.value,1,1,223) ) {
      alert(msg+util_gw_ipaddr_1strange1);
      field.value = field.defaultValue;
      field.focus();
      return false;
   }  
   ...
   if(getDigit(field.value,1)==127){
      alert(msg+util_gw_ipaddr_1strange2);
      field.value = field.defaultValue;
      field.focus();
      return false;
   }
...
function checkMacAddr_is_zero(field)
{
	...
	if(field.value == "000000000000")
	{
		field.focus();
		field.select();
		alert(tcpip_lan_macaddr_invlaid_value3);
		return false;
	}
	return true;
}
...
function checkMacAddr_is_muticast(field)
{
	...
	if(parseInt(field.value.substr(0, 2), 16) & 0x01 != 0)
	{
		field.focus();
		field.select();
		alert(tcpip_lan_macaddr_invlaid_value4);
		return false;
	}
	return true;
}
...
function checkMacAddrString_is_legal(field)
{
...
function checkMacAddrString_is_zero(field)
{
	if(!field) return false;
	if(field.value == "00:00:00:00:00:00")
	{
		field.focus();
		field.select();
		alert(tcpip_lan_macaddr_invlaid_value3);
...
function checkMacAddrString_is_broadcast(field)
{
	if(!field) return false;
	if(field.value == "ff:ff:ff:ff:ff:ff" || field.value == "FF:FF:FF:FF:FF:FF")
	{
		field.focus();
		field.select();
		alert(tcpip_lan_macaddr_invlaid_value1);
...
function checkMacAddrString(field)
{
	return (checkMacAddrString_is_legal(field) && checkMacAddrString_is_zero(field) && checkMacAddrString_is_broadcast(field) && checkMacAddr_is_muticast(field));
}
...
function rtk_sendRequest(stateChangeFunc,url)
{
	var ajaxReq=rtk_createRequest();
	if(!ajaxReq)	return;

	ajaxReq.open("Get",url+"?t="+rtk_random_str(),true);
	ajaxReq.onreadystatechange=stateChangeFunc;
	ajaxReq.send(null);
	return ajaxReq;
}
